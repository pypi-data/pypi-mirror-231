...
class DjangoSimpleExportAdmin(DjangoChangelistToolbarAdmin, admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        return [
            path(
                "django-simple-export/<code>/",
                self.admin_site.admin_view(self.django_simple_export_admin_export_view),
                name="{0}_{1}_simple_export".format(app_label, model_name),
            ),
        ] + super().get_urls()

    def django_simple_export_admin_export_view(self, request, code):
        django_simple_exports = (
            self.django_simple_export_admin_get_django_simple_exports()
        )
        settings = django_simple_exports.get(code, None)
        if not settings:
            msg = _("No export setings for: %s.") % code
            return self.django_simple_export_admin_redirect_to_changelist_with_error_message(
                request, msg
            )
        if not self.django_simple_export_admin_has_perm(
            request, settings.get("permissions", [])
        ):
    ...
    def django_simple_export_admin_redirect_to_changelist_with_error_message(
        self, request, msg
    ):
    ...
    def django_simple_export_admin_do_export(self, request, settings):
        ...
        if settings.get("show-header", True):
            for col_index, field_setting in enumerate(settings["fields"]):
                label = self.django_simple_export_admin_get_field_label(
                    field_setting, items
                )
        ...
        for loop_index, item in enumerate(items):
            for col_index, field_setting in enumerate(settings["fields"]):
                value = self.django_simple_export_admin_get_field_value(
                    item, field_setting, loop_index
                )
                ...
                if not isinstance(real_col_index, (tuple, list, set)):
                    ...
                    for tmp_index in range(len(values)):
                        worksheet.cell(
                            row_index + 1, real_col_index[tmp_index]
                        ).value = values[tmp_index]
                    # split value into multi cells end
                self.django_simple_export_admin_calc_footer_value(
                    footer_values, col_index, field_setting, value
                )
            row_index += 1
        # write footer
        for col_index, field_setting in enumerate(settings["fields"]):
    ...
    def django_simple_export_admin_calc_footer_value(
        self, footer_values, col_index, field_setting, value
    ):
    ...
    def get_changelist_toolbar_buttons(self, request):
        ...
        for (
            code,
            settings,
        ) in self.django_simple_export_admin_get_django_simple_exports().items():
            if not self.django_simple_export_admin_has_perm(
                request, settings.get("permissions", [])
            ):
                continue
            ...
            url = reverse(
                "admin:{0}_{1}_simple_export".format(app_label, model_name), args=[code]
            )
            if export_filtered:
