...
def build_kpis_df(aggregated_products, aggregated_reviews, source_dict, kpi_folder_path):
    """Creates KPIs excel files for collect analysis.

    Args:
        aggregated_products (list): list of aggregated products dictionary.
        aggregated_reviews (list): list of aggregated reviews dictionary.
        source_dict (dict): dictionary with information from the source.
        kpi_folder_path (str): path to the KPIs folder.
    
    Returns:
        df, pandas dataframe of KPIs.
    """
        
    reviews_df = pd.DataFrame(aggregated_reviews)
    products_df = pd.DataFrame(aggregated_products)[['url', 'product_name', 'product_brand', 'n_reviews']]
    reviews_value_count_df = pd.DataFrame(reviews_df.url.value_counts()).reset_index()
    reviews_value_count_df.columns = ['url', 'n_saved_reviews']

    kpi_df = pd.merge(products_df, reviews_value_count_df, on='url')
    kpi_df.to_excel(
        os.path.join(
            kpi_folder_path,
