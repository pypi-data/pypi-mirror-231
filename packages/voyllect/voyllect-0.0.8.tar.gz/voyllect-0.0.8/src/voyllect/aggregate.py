...
def build_kpis_df(aggregated_products, aggregated_reviews, source_dict, kpis_folder_path):
    ...
    subset = list(reviews_df.columns.drop('collect_date').drop('id').drop('writer_information_dict').drop('review_other_rating_dict').drop('product_attribute_dict'))
    reviews_df = \
    ...
    kpis_df = pd.merge(products_df, reviews_value_counts_df, on='url')
