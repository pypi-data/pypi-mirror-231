...
class ApiLoggingMiddleware(MiddlewareMixin):
    ...
    def __handle_response(self, request, response):
        ...
        try:
            ...
            return
        user, err = get_request_user(request)
        if err:
            return
        info = {
            ...
            'response_code': response.data.get('err'),
            ...
            'json_result': {"err": response.data.get('err'), "msg": response.data.get('msg')},
        }
        # 定义正则表达式模式
        pattern = re.compile(r"^/api/([^/_]+)")
        # 使用正则表达式匹配路径
        match = pattern.match(request.request_path)
        # 获取匹配的参数
        if match:
        ...
        if not operation_log.request_modular and settings.API_MODEL_MAP.get(path, None):
            operation_log.request_modular = settings.API_MODEL_MAP[path]
            operation_log.save()
