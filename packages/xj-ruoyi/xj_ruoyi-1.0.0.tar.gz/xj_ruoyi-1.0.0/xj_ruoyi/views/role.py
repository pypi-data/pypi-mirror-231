...
from ..models import Role, Menu, MenuButton, Dept
from .dept import DeptSerializer
from .menu import MenuSerializer
from .menu_button import MenuButtonSerializer
from ..utils.json_response import SuccessResponse, DetailResponse
from ..utils.serializers import CustomModelSerializer
from ..utils.utility_method import parse_integers, build_tree
...
class RoleViewSet(CustomModelViewSet):
    ...
    def data_scope(self, request):
        is_superuser = request.user.is_superuser
        role_queryset = Role.objects.filter(user__id=request.user.id).values_list('data_range', flat=True)
        if is_superuser:
            data = [
                {
                    "value": 0,
    ...
    def data_scope_dept(self, request):
        ...
        if request.query_params.get('is_tree', ""):
            data = build_tree(list(queryset), "parent", "id")
...
class RoleApi:
    @staticmethod
    def bind_user_role(user, role_ids):
        """
        批量绑定用户角色信息
        :param user:用户模型
