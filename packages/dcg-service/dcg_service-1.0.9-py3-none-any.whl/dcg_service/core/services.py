...
class PriceCatalog(ServiceBase):
    ...
    def get_fee_details(self, include_tax=False):
        ...
        obj.service_price_type = obj.service_price_type.upper() if obj.service_price_type else obj.service_price_type
        ...
        if obj.service_price_type == ServicePrice.FLAT:
            ...
            response['total_commission'] = self.get_commission_details(flat, self._total_price, units)
            ...
            response['total_price'] = self._total_price
            response['total_commission'] = self.get_commission_details(tier_group, self._total_price, units)
    ...
    def tier_bundle_price_generator(self, tiers):
        ...
        for obj in tiers:
            ...
            self._total_commission += self.get_commission_details(obj, obj.total_price, unit)
    ...
    def get_commission_details(self, service_obj, service_price, units):
        """
        ...
        self._total_commission = total_commission * units
...
class CouponMixin(ServiceBase):
    ...
    @classmethod
    def get_quantity_discount_value(cls, coupon, coupon_code_list):
        if (
                not coupon.offered_service_coupon
        ) or (
                not coupon.offered_service_quantity
        ):
            return

        offered_coupon = coupon.offered_service_coupon
        is_applicable = bool(
            offered_coupon.is_internal_use and
            (datetime.strptime(
                offered_coupon.start_date, '%Y-%m-%d'
            ).date() <= datetime.now().date() <= datetime.strptime(
                offered_coupon.end_date, '%Y-%m-%d').date())
        )
        if not is_applicable:
            return

        service_details = PriceCatalog(
            service_id=coupon.offered_service_id,
            units=coupon.offered_service_quantity
        )
        offered_service_details = service_details.get_fee_details()
        offered_service_details['quantity'] = coupon.offered_service_quantity
        offered_service_details['service_id'] = coupon.offered_service_id

        offered_coupon.coupon_type = offered_coupon.coupon_type.upper(
        ) if offered_coupon.coupon_type else offered_coupon.coupon_type

        if offered_coupon.coupon_type == CouponMixin.PERCENTAGE_COUPON and is_applicable:
            coupon_value = round((offered_coupon.discount_value / 100) * service_details.total_price, 2)
            offered_coupon.coupon_value = min([coupon_value, service_details.total_price])

        elif offered_coupon.coupon_type == CouponMixin.FLAT_COUPON and is_applicable:
            coupon_value = offered_coupon.discount_value
            offered_coupon.coupon_value = coupon_value * coupon.offered_service_quantity

        if getattr(offered_coupon, 'coupon_value', None) <= service_details.total_price:
            coupon.is_quantity_coupon = True
            coupon.offered_service_details = service_details.get_fee_details()
            coupon.offered_service_details.service_id = coupon.offered_service_id
            coupon.offered_service_details.quantity = coupon.offered_service_quantity
            coupon.coupon_value = offered_coupon.coupon_value
            coupon.offered_coupon_detail = offered_coupon
            coupon.offered_coupon_detail.coupon_id = offered_coupon.id
            if coupon.short_code.upper() in coupon_code_list:
                coupon.is_auto_apply = False
            return coupon
    ...
    def get_coupon_details(self, coupon_code=None, **kwargs):
        ...
        for c_code in coupon_code_list:
            coupons_to_apply += list(
                filter(
                    lambda coup: (coup.short_code and coup.short_code.upper() == c_code.upper()
                                  ) and (coup.short_code and coup.short_code.upper() not in availed_coupons
                                         ),
                    service_coupons
                )
            )
        ...
        for coupon in service_coupons:
            if coupon.is_auto_apply and (
                    (coupon.id not in [c.id for c in coupons_to_apply]) and (
                    coupon.short_code and coupon.short_code.upper() not in availed_coupons)
            ):
        ...
        for coupon in coupons_to_apply:
            is_applicable = bool(
                (
                        self.units >= coupon.actual_service_quantity
                ) and (
                        datetime.strptime(
                            coupon.start_date, '%Y-%m-%d'
                        ).date() <= datetime.now().date() <= datetime.strptime(coupon.end_date, '%Y-%m-%d'
                                                                               ).date()
                )
            )
            coupon.coupon_type = coupon.coupon_type.upper() if coupon.coupon_type else coupon.coupon_type
            if coupon.coupon_type == CouponMixin.PERCENTAGE_COUPON and is_applicable:
            ...
            elif coupon.coupon_type == CouponMixin.FLAT_COUPON and is_applicable:
                coupon_value = coupon.discount_value
                coupon.coupon_value = coupon_value * self.units
                if total_price >= coupon_value:
                    filtered_coupon_list.append(coupon)
            elif coupon.coupon_type == CouponMixin.QUANTITY_COUPON and is_applicable:
                coupon = self.get_quantity_discount_value(coupon, coupon_code_list)
                if coupon:
                    filtered_coupon_list.append(coupon)
        ...
        if coupons_to_apply:
            ...
            if coupon_to_be_apply and coupon_to_be_apply.coupon_type == CouponMixin.QUANTITY_COUPON:
                offered_service_details = getattr(coupon_to_be_apply, 'offered_service_details', None)
                offered_service_coupon = getattr(coupon_to_be_apply, 'offered_coupon_detail', None)
                self.offered_service_details = offered_service_details.to_dict() if offered_service_coupon else dict()
                self.offered_service_coupon = offered_service_coupon.to_dict() if offered_service_coupon else dict()
                self.is_quantity_coupon = getattr(coupon_to_be_apply, 'is_quantity_coupon', False)
                self._total_discount = getattr(coupon_to_be_apply, 'coupon_value', 0)
                self._coupon_type = coupon_to_be_apply.coupon_type
                self._discount_value = coupon_to_be_apply.discount_value
                self._coupon_code = coupon_to_be_apply.short_code.upper()
                self._coupon_id = coupon_to_be_apply.id
                self.actual_service_id = coupon_to_be_apply.actual_service_id
                self.is_auto_apply_coupon = coupon_to_be_apply.is_auto_apply
            elif coupon_to_be_apply and (coupon_to_be_apply.coupon_value <= total_price):
                self._total_discount = coupon_to_be_apply.coupon_value
                self._coupon_type = coupon_to_be_apply.coupon_type
                self._discount_value = coupon_to_be_apply.discount_value
                self._coupon_code = coupon_to_be_apply.short_code.upper()
                self._coupon_id = coupon_to_be_apply.id
        ...
        coupon_results['coupon_type'] = getattr(self, '_coupon_type', None)
        coupon_results['is_quantity_coupon'] = getattr(self, 'is_quantity_coupon', False)
        coupon_results['offered_service_details'] = getattr(self, 'offered_service_details', dict())
        coupon_results['offered_service_coupon'] = getattr(self, 'offered_service_coupon', dict())
        coupon_results['discount_value'] = getattr(self, '_discount_value', None)
        coupon_results['is_auto_apply'] = getattr(self, 'is_auto_apply_coupon', False)
        coupon_results['actual_service_id'] = getattr(self, 'actual_service_id', None)
