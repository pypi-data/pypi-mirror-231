...
class RespondersRebid(Hand):
    ...
    def _after_weak_opening(self):
        ...
        #     bid = self.next_level_bid(self.opener_bid_one.denomination, '3316')
    ...
    def _weak_opening_and_competitive_auction(self):
        ...
        if self._has_seventeen_and_three_card_support_can_bid_game():
            bid = self.suit_bid(4, self.opener_bid_one.denomination, '3319')
        elif self._can_rebid_openers_suit():
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3564')
        else:
            bid = Pass('3321')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _after_nt_opening(self):
        ...
        if self._opener_has_responded_to_stayman():
            ...
            bid = self.bid_to_game(self.opener_bid_two.denomination, '3323')
        else:
            bid = Pass('3324')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _strong_after_nt_opening(self):
        ...
        if self.suit_length(self.opener_bid_two.denomination) >= 3:
            bid = self.next_level_bid(self.opener_bid_two.denomination, '3325')
    ...
    def _after_stayman_four_card_major(self):
        ...
        if self.opener_bid_one.name == "1NT":
            if self.suit_length(self.opener_bid_two.denomination) >= 5:
                bid = self.bid_to_game(self.opener_bid_two.denomination, '3330')
            else:
    ...
    def _after_stayman_level_two_four_card_major(self):
        ...
        if self.opener_bid_two.level == 3:
            level = 4
        bid = self.suit_bid(level, self.opener_bid_two.denomination, '3331', True)
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _after_stayman_level_three_four_card_major(self):
        ...
        if self.suit_length(self.opener_bid_two.denomination) >= 4:
            bid = self.suit_bid(4, self.opener_bid_two.denomination, '3332')
        else:
    ...
    def _after_stayman_two_nt_rebid(self):
        ...
        if self.hcp >= 13 and self.nt_level <= 3:
            ...
            if self.hearts in self.opponents_suits:
                bid = self.spade_bid(4, '3488')
            else:
                bid = self.heart_bid(4, '3488')
        else:
            bid = Pass('3345')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _fewer_than_ten_points_after_two_club_opening(self):
        """Return bid if fewer than 10 points after 2C opening."""
        if self.suit_length(self.opener_bid_two.denomination) >= 3:
            if self.opener_bid_two.denomination.is_major:
                bid = self.bid_to_game(self.opener_bid_two.denomination, '3358')
            else:
                bid = self.next_level_bid(self.opener_bid_two.denomination, '3359')
        elif self.is_semi_balanced and self.nt_level <= 3:
    ...
    def _opener_has_jumped_or_bid_at_level_three(self):
        ...
        if self.opener_bid_one.denomination == self.bid_one.denomination:
            if (self.hand_value_points(self.opener_bid_one.denomination) >= 8 and
                ...
                bid = self.next_level_bid(self.opener_bid_one.denomination, '3367', )
            else:
                bid = Pass('3368')
    ...
    def _no_jump_bid(self):
        ...
        if self._has_fewer_than_nine_points_and_passed_or_one_nt():
            ...
            bid = self.suit_bid(4, self.opener_bid_two.denomination, '3373')
    ...
    def _no_jump_bid_support_for_opener(self):
        ...
        if self.hand_value_points(self.opener_bid_one.denomination) <= 9:
            ...
            bid = self.nt_bid(nt_level, '3385')
                ...
                level_allowed):
            bid = self.suit_bid(level + 1, self.opener_bid_one.denomination, '3386')
        else:
    ...
    def _no_jump_bid_support_for_opener_strong(self):
        ...
        if self._intermediate_has_seven_car_major():
            ...
            bid = self.suit_bid(raise_level, self.opener_bid_one.denomination, '3389')
        else:
            bid = Pass('3568')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _opener_has_supported_responder_jump(self):
        ...
        if self.longest_suit.is_minor and self._can_support_openers_major():
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3415')
        elif (self.hcp >= 9 or
    ...
    def _responder_long_suit_weak(self):
        ...
        if self._own_suit_is_minor_stoppers_in_other_suits_level_two():
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3429')
    ...
    def _opener_jumped(self):
        ...
        if self.hcp >= 20 and self.suit_length(self.opener_bid_one.denomination) >= 2:
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3433')
    ...
    def _opener_jumped_support_strong(self):
        ...
        if self._balanced_and_opener_bid_minor():
            ...
            bid = self.bid_to_game(self.opener_bid_one.denomination, '3441', True)
        elif self.support_points(self.opener_bid_one.denomination) >= 15:
            bid = self.bid_to_game(self.opener_bid_one.denomination, '3442', True)
        elif self._opener_has_jumped_and_can_support_competitive():
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3443')
        elif (self.opener_bid_one.denomination == self.opener_bid_two.denomination and
              ...
              self.hcp >= 11):
            bid = self.bid_to_game(self.opener_bid_one.denomination, '3444', True)
    ...
    def _opener_changed_suit_after_nt_intermediate(self):
        ...
        if self._no_support_own_suit_is_minor_with_stops():
            ...
            bid = self.suit_bid(4, suit_to_bid, '3461')
        else:
            bid = Pass('3462')
        self.tracer(__name__, inspection(), bid, self.trace)
        return bid
    ...
    def _three_suits_bid_support_opener(self):
        ...
        if self._can_support_minor_but_is_semi_balanced(suit_to_bid, suit_support_points):
            ...
            bid = self.next_level_bid(suit_to_bid, '3474')
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3484')
    ...
    def _opener_broken_barrier_weak(self):
        ...
        if self.hcp >= 15 and self.suit_length(self.opener_bid_one.denomination) >= 4:
            ...
            bid = self.suit_bid(4, self.opener_bid_one.denomination, '3508')
            ...
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3516')
        elif (self.shape[0] >= 6 and
    ...
    def _opener_nt_after_suit_response(self):
        ...
        if self.opener_bid_two.name == '3NT':
            ...
            bid = self.next_level_bid(self.second_suit, '3532')
                ...
                self._has_shortage()):
            bid = self.bid_to_game(self.opener_bid_one.denomination, '3533')
    ...
    def _opener_nt_after_suit_response_strong(self):
        ...
        if self._has_seven_card_suit_and_fourteen_points():
            ...
            bid = self.suit_bid(3, self.opener_bid_one.denomination, '3546')
    ...
    def _opener_has_passed(self):
        ...
        if self._has_four_card_support_at_level_three():
            bid = self.next_level_bid(self.opener_bid_one.denomination, '3555')
    ...
    def _can_rebid_openers_suit(self):
        ...
        trial_bid = self.next_level_bid(self.opener_bid_one.denomination, '3564')
    ...
    def _has_twelve_points_and_opener_has_jumped(self):
        ...
        result = ((self.hcp >= 12 or self.is_jump(self.bid_one, self.opener_bid_two)) and
                  self.nt_level <= 3 and
                  self.stoppers_in_bid_suits)
        return result
    ...
    def _has_nine_points_and_opening_bid_is_major(self):
