...
def ai_tagger(
    ...
    if backend != "openai":
        ...
        for tag in tags:
            ...
            self.dataset = self.dataset.add_column(name=f"__ai_tag__{tag.replace(' ', '_')}", column=labels)
            logger.info(f"Tagged with tag {tag}. Pausing to cool down before issuing more requests...")
            time.sleep(30)
