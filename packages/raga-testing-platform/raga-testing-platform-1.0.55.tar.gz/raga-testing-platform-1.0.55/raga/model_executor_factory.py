...
class ModelExecutor:
    ...
    def execute(self, init_args, execution_args, data_frame:Dataset):
        ...
        try:
            ...
            role = 'arn:aws:iam::596708924737:role/test-s3'
            ...
            role, auth = ModelExecutor.get_role_and_auth(data_frame)
    ...
    @classmethod
    def get_role_and_auth(cls, data_frame:Dataset):
...
class ModelExecutorFactory:
    @classmethod
    def get_model_executor(cls, test_session:TestSession, model_name:str, version:int):
...
def model_validation(model, model_name, raga_repo_path, model_path):
    supported_python_versions = model.get("supportedPythonVersion")
    supported_platforms = model.get("supportedPlatforms")
    local_platform = get_machine_platform()
    local_python_version = get_python_version()
    if supported_python_versions and supported_platforms:
        supported_platforms = [platform.strip() for platform in supported_platforms.split(',')]
        supported_python_versions = format_python_versions(supported_python_versions)
        if local_platform not in supported_platforms:
            raise PlatformError(f"{PLATFORM_NOT_SUPPORT.format(local_platform, supported_platforms)}")
        if local_python_version not in supported_python_versions:
            raise PythonVersionError(f"{PYTHON_NOT_SUPPORT.format(local_python_version, supported_python_versions)}")
    else:
        raise ModelExecutorFactoryException(MODEL_OR_PYTHON_NOT_FOUND)
