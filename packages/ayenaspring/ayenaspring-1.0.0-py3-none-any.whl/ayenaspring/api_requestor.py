...
import ayenaspring
from ayenaspring import error, util, version
from ayenaspring.openai_response import OpenAIResponse
from ayenaspring.util import ApiType
...
def _requests_proxies_arg(proxy) -> Optional[Dict[str, str]]:
    ...
    if proxy is None:
        ...
        raise ValueError(
            "'ayenaspring.proxy' must be specified as either a string URL or a dict with string URL under the https and/or http keys."
        )
...
def _make_session() -> requests.Session:
    if ayenaspring.requestssession:
        if isinstance(ayenaspring.requestssession, requests.Session):
            return ayenaspring.requestssession
        return ayenaspring.requestssession()
    if not ayenaspring.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; ayenaspring always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(ayenaspring.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or ayenaspring.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(ayenaspring.api_type)
        )
        self.api_version = api_version or ayenaspring.api_version
        self.organization = organization or ayenaspring.organization
    ...
    def request_headers(
        ...
        if ayenaspring.app_info:
            user_agent += " " + self.format_app_info(ayenaspring.app_info)
        ...
        ua = {
            ...
            "publisher": "ayenaspring",
            "uname": uname_without_node,
        }
        if ayenaspring.app_info:
            ua["application"] = ayenaspring.app_info
    ...
    def _prepare_request_raw(
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "OpenAI bindings. Please contact us through our help center at help.ayenaspring.com for "
                "assistance." % (method,)
            )
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(ayenaspring.proxy),
            "timeout": timeout,
        }
        try:
...
async def aiohttp_session() -> AsyncIterator[aiohttp.ClientSession]:
    user_set_session = ayenaspring.aiosession.get()
    if user_set_session:
