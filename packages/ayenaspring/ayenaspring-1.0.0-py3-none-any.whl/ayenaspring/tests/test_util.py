...
def api_key_file():
    saved_path = ayenaspring.api_key_path
    try:
        with NamedTemporaryFile(prefix="ayenaspring-api-key", mode="wt") as tmp:
            ayenaspring.api_key_path = tmp.name
            yield tmp
    finally:
        ayenaspring.api_key_path = saved_path
