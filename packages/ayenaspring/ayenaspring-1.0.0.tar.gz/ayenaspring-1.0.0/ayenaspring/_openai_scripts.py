...
import ayenaspring
from ayenaspring import version
from ayenaspring.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    ayenaspring.debug = True
    if args.api_key is not None:
        ayenaspring.api_key = args.api_key
    if args.api_base is not None:
        ayenaspring.api_base = args.api_base
    if args.organization is not None:
        ayenaspring.organization = args.organization
    if args.proxy is not None:
        ayenaspring.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                ayenaspring.proxy['https'] = proxy
            elif proxy.startswith('http'):
                ayenaspring.proxy['http'] = proxy

    try:
        args.func(args)
    except ayenaspring.error.OpenAIError as e:
        display_error(e)
        return 1
