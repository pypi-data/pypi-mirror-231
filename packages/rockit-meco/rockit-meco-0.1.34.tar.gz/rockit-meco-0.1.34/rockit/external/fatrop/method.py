...
class FatropMethod(ExternalMethod):
    ...
    def transcribe_phase1(self, stage, **kwargs):
        ...
        self.artifacts.append(LibraryArtifact("fatrop_driver", "lib"))
    ...
    def transcribe_phase2(self, stage, build_dir_abs=None,**kwargs):
        ...
        for sampl in self.samplers.keys():
            self.codegen.add(Function("sampler_"+ str(sampl), [stage.u, stage.x, self.stage_params_sym, self.global_params_sym], [densify(placeholders(self.samplers[sampl], preference =['expose']))]).expand())
        ...
        if self.mode == "fatropy":
            ...
            subprocess.run(["cmake","-S", "interface","-B", os.path.join("interface","build")], cwd=self.build_dir_abs)
    ...
    def to_function(self, stage, name, args, results, *margs):
        ...
        class FatropCallBack(Callback):
            ...
            def has_codegen(sefl):
                return True
        try:
            helper0 = Function("helper0", args, [*self.U_gist, *self.X_gist, *self.P_stage_gist, self.P_global_gist], {"allow_free":True})
        except:
            helper0 = Function("helper0", args, [*self.U_gist, *self.X_gist, *self.P_stage_gist, self.P_global_gist])

        if helper0.has_free():
            helper0_free = helper0.free_mx()
            [controls, states] = cs.substitute([cs.horzcat(*self.U_gist), cs.horzcat(*self.X_gist)], helper0_free, self.initial_value(stage, helper0_free))
        else:
            [controls, states] = [cs.horzcat(*self.U_gist), cs.horzcat(*self.X_gist)]
        if self.mode == "fatropy":
            ...
            res_u, res_x = self.fatrop_solver(cs.horzcat(*self.P_stage_gist), self.P_global_gist, controls, states)
        elif self.mode == "interface":
            res_x, res_u, _, _ = self.fatrop_solver(self.P_global_gist, cs.horzcat(*self.P_stage_gist), densify(states), densify(controls))

        ret = Function(name, args, cs.substitute(results, [*self.U_gist, *self.X_gist], [*cs.horzsplit(res_u),*cs.horzsplit(res_x)]) , *margs)
    ...
    def initial_value(self, stage, expr):
        ...
        helper_in = [self.P_global_gist,cs.horzcat(*self.P_stage_gist),states,controls,variables, stage.T]
        helper = Function("helper", helper_in, expr)
        ret = [s.toarray(simplify=True) for s in helper(self.global_params_value, self.stage_params_value, self.X0, self.U0, 0, 0)]
        if not lst :
            return ret[0]
        return ret
    ...
    @property
    def gist(self):
        return vertcat(ExternalMethod.gist.fget(self), self.P_global_gist, *self.P_stage_gist, self.T_gist)
