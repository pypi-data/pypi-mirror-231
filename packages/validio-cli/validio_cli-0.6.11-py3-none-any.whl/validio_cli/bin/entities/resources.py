...
async def move(
    ...
    namespace: str = typer.Option(
        ..., "--namespace", "-n", help="Namespace where the resources currently reside"
    ),
...
async def do_move(
    ...
    if namespace == target_namespace:
        raise Exception("target namespace should be different from source namespace")

    if not file.is_file():
        raise Exception(f"'{file.absolute()}' is not a file")

    obj = json.loads(file.read_text())
    if type(obj) is not dict:
        raise Exception(f"expected json object in resource file: got {type(obj)}")

    if "resources" not in obj:
        raise Exception("json object in file should contain a 'resources' key")

    resources_obj_raw = obj["resources"]
    if type(resources_obj_raw) is not dict:
        raise Exception(
    ...
    for resource_type in DiffContext.fields():
        ...
        if type(resource_names) is not list:
            raise Exception(
    ...
    resources_to_move = await get_resources_to_move(
        namespace=namespace,
        client=client,
        resources_obj_raw=resources_obj_raw,
    )
