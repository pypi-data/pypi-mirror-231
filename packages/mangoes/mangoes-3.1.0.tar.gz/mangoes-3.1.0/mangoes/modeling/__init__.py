# -*- coding: utf-8 -*-
"""
The modeling subpackage provides functionality for using transformer models for various tasks, including feature
extraction, pretraining, and a number of fine-tuning tasks. It is a interface into the Huggingface transformers and
tokenizers libraries (https://huggingface.co/transformers/ and https://huggingface.co/docs/tokenizers/python/latest/),
meant to provide an easy-to-use API for using transformers for contextual embeddings or training/inference.

The main component of the subpackage are the task classes. Each possible task is implemented as a class that inherits
from :class:`mangoes.modeling.transformer_base.TransformerModel`. This API provides full pipeline functionality for each
task, from raw textual inputs to inference, and training, saving, and loading. Each class has a tokenizer and model as
attributes.
...
The API for these task classes include 5 main functions:
    - __init__():
        ...
        transformer model (while initializing possible fine-tuning head(s)) for use in fine-tuning, or loading a
        pretrained base transformer model and trained classification heads.
        Arguments into this method can be paths to local directories containing pretrained tokenizers/models, or
    ...
    - train():
        ...
        hyper-parameters they want. See examples in the use-cases sections of the documentation, or in the demo
        notebooks.
    ...
    - generate_outputs():
        ...
        the base model, in addition to possible fine tuning head layer outputs. Furthermore, information on how the
        input was tokenized is returned as well. This method is useful when users want to analyze the
...
"""
from mangoes.modeling.enhanced_models import *
from mangoes.modeling.transformer_base import *
from mangoes.modeling.finetuning import *
