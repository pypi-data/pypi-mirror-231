...
class TransformerModelForCoreferenceResolutionBase(transformers.PreTrainedModel):
    """
    ...
    def __init__(self, pretrained_model_or_config, max_span_width=30, ffnn_hidden_size=1000, top_span_ratio=0.4,
                 ...
                 coref_dropout=0.3, **base_model_keyword_args):
        if isinstance(pretrained_model_or_config, transformers.PretrainedConfig):
            base_config = pretrained_model_or_config
        elif isinstance(pretrained_model_or_config, transformers.PreTrainedModel):
            base_config = pretrained_model_or_config.config
        else:
            base_config = transformers.AutoConfig.from_pretrained(pretrained_model_or_config)
        if base_config.task_specific_params is None:
            # used for saving/loading model
            base_config.task_specific_params = {"max_span_width": max_span_width,
            ...
            max_span_width = base_config.task_specific_params["max_span_width"] \
                if "max_span_width" in base_config.task_specific_params else max_span_width
            ffnn_hidden_size = base_config.task_specific_params["ffnn_hidden_size"] \
                if "ffnn_hidden_size" in base_config.task_specific_params else ffnn_hidden_size
            top_span_ratio = base_config.task_specific_params["top_span_ratio"] \
                if "top_span_ratio" in base_config.task_specific_params else top_span_ratio
            max_top_antecendents = base_config.task_specific_params["max_top_antecendents"] \
                if "max_top_antecendents" in base_config.task_specific_params else max_top_antecendents
            use_metadata = base_config.task_specific_params["use_metadata"] \
                if "use_metadata" in base_config.task_specific_params else use_metadata
            metadata_feature_size = base_config.task_specific_params["metadata_feature_size"] \
                if "metadata_feature_size" in base_config.task_specific_params else metadata_feature_size
            genres = base_config.task_specific_params["genres"] \
                if "genres" in base_config.task_specific_params else genres
            max_training_segments = base_config.task_specific_params["max_training_segments"] \
                if "max_training_segments" in base_config.task_specific_params else max_training_segments
            coref_depth = base_config.task_specific_params["coref_depth"] \
                if "coref_depth" in base_config.task_specific_params else coref_depth
            coref_dropout = base_config.task_specific_params["coref_dropout"] \
                if "coref_dropout" in base_config.task_specific_params else coref_dropout
        super().__init__(base_config)
        if isinstance(pretrained_model_or_config, transformers.PretrainedConfig):
            self.features_model = transformers.AutoModel.from_config(pretrained_model_or_config,
                                                                     **base_model_keyword_args)
        elif isinstance(pretrained_model_or_config, transformers.PreTrainedModel):
            self.features_model = pretrained_model_or_config
        else:
            self.features_model = transformers.AutoModel.from_pretrained(pretrained_model_or_config,
                                                                         **base_model_keyword_args)
        base_emb_size = base_config.hidden_size
        self.span_attend_projection = nn.Linear(base_emb_size, 1)
        span_embedding_dim = (base_emb_size * 3) + metadata_feature_size
        ...
        self.fast_antecedent_projection = nn.Linear(span_embedding_dim, span_embedding_dim)
        ...
        self.slow_antecedent_projection = nn.Linear(span_embedding_dim * 2, span_embedding_dim)
        # metadata embeddings
        ...
        self.segment_dist_embeddings = nn.Embedding(num_embeddings=max_training_segments,
                                                    embedding_dim=metadata_feature_size)
        self.emb_cluster_size = nn.Embedding(10, embedding_dim=metadata_feature_size)
        self.cluster_score_ffnn = nn.Sequential(
            nn.Linear(3 * span_embedding_dim + metadata_feature_size,
                      ffnn_hidden_size),
            nn.ReLU(),
            nn.Dropout(coref_dropout),
            nn.Linear(ffnn_hidden_size, 1),
        )
        self.cm_dropout = nn.Dropout(p=coref_dropout)
    ...
    def forward(self, input_ids, attention_mask, sentence_map, speaker_ids=None, genre=None, gold_starts=None,
        ...
        base_outputs = self.features_model(input_ids, attention_mask, output_attentions=output_attentions,
                                           output_hidden_states=output_hidden_states, return_dict=return_dict,
                                           **extra_inputs)
        mention_doc = base_outputs[0]  # [num_seg, max_seg_len, emb_len]
        ...
        for i in range(self.coref_depth):
            ...
            cluster_merging_scores = self.cluster_merging(top_span_emb, top_antecedents, top_antecedent_scores)
        ...
        if gold_ends is not None and gold_starts is not None and cluster_ids is not None:
            ...
            loss = torch.sum(loss)
            loss += torch.sum(
                self.softmax_loss(top_antecedent_scores + torch.cat([dummy_scores, cluster_merging_scores], 1),
                                  top_antecedent_labels))
        else:
            loss = None
            top_antecedent_scores += torch.cat([dummy_scores, cluster_merging_scores], 1)
        if not return_dict:
    ...
    def cluster_merging(self, top_span_emb, top_antecedent_idx, top_antecedent_scores):
        ...
        span_to_cluster_id = torch.zeros(num_top_spans, dtype=torch.long, device=self.device)  # id 0 as dummy cluster
        cluster_emb = torch.zeros(max_num_clusters, span_emb_size, dtype=torch.float,
                                  device=self.device)  # [max num clusters, emb size]
        num_clusters = 1  # dummy cluster
        cluster_sizes = torch.ones(max_num_clusters, dtype=torch.long, device=self.device)
        ...
        for i in merge_order.tolist():
            ...
            cluster_size_emb = F.dropout(self.emb_cluster_size(antecedent_cluster_size), p=self.coref_dropout,
            ...
            cluster_scores = torch.squeeze(self.cluster_score_ffnn(pair_emb), 1)
            ...
            if antecedent_cluster_id > 0 and curr_span_cluster_id > 0:
                ...
                _merge_clusters(cluster_emb, cluster_sizes, antecedent_cluster_id, curr_span_cluster_id)
...
def _merge_span_to_cluster(cluster_emb, cluster_sizes, cluster_to_merge_id, span_emb):
...
def _merge_clusters(cluster_emb, cluster_sizes, cluster1_id, cluster2_id):
    """ Merge cluster1 to cluster2 """
    cluster1_size, cluster2_size = cluster_sizes[cluster1_id].item(), cluster_sizes[cluster2_id].item()
    cluster_emb[cluster2_id] = (cluster_emb[cluster1_id] * cluster1_size + cluster_emb[cluster2_id] * cluster2_size) / (
