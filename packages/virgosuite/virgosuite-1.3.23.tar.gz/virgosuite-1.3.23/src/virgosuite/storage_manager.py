...
def load_sfdb09(
    ...
    # Skipping last element so that intervall is open -> freq_range [start_freq, end_freq)
    fft = sliced_fft_data.fillna(0)[:-1, :].values
    periodogram = sliced_regressive_data["periodogram"].fillna(0)[:-1, :].values
    ar_spectrum = sliced_regressive_data["ar_spectrum"].fillna(0)[:-1, :].values
    td_attributes = sliced_td_attributes.to_pandas()
    ti_attributes = ti_attributes.to_pandas()
    time_axis = sliced_fft_data.time.values
    frequency_axis = sliced_fft_data.frequency[:-1].values
