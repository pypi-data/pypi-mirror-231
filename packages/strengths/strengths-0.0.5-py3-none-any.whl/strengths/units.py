...
_units_labels_dict = {
    ...
    "volume"   : ["kL", "L", "dL", "cL", "mL", "µL", "nL", "pL", "fL"]
    }
...
class UnitValue :
    ...
    :param units: units of the variable or None
    :type units: Units, str or None
    """

    def __init__(self, value, units=None) :

        if type(units) == str or type(units)==Units :
            if type(units) == str :
            ...
            if isnumber(value) :
                ...
                raise ValueError("when units are not None, value must be a number.")
        elif isnone(units) :
            if type(value) == str :
                v = parse_unitvalue(value)
                self.units = v.units
                self.value = v.value
            else :
                raise ValueError("when units are None, value must be a str.")
        else :
            raise ValueError("units must be a str ot Units.")

    @property
    def value(self) :
        """
        Numerical value of the pysical quantity (number).
        """
...
class UnitArray :
    ...
    def __init__(self, value, units, check_value=True) :
        """
        constructor
        """
                ...
                for i in range(len(self._value)) :
                    ...
                    if type(self._value[i]) == UnitValue :
                        ...
                        if self._value[i].units.sys != self.units.sys :
                            self._value[i] = self._value[i].convert(units)
    ...
    def convert(self, u) :
        ...
        for i in range(len(self.value)) :
            v[i] = convert_value(self.value[i], self.units.sys, su_dst, self.units.dim)

        return UnitArray(v, Units(su_dst, self.units.dim))
...
def unitarray_to_dict(v) :
    ...
    return {"value" : list(v.value), "units" : str(v.units)}

def unitarray_from_dict(d) :
    """
