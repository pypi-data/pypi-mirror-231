...
def _get_normal_data(investment_ids: list, normal_data_point_settings: DataFrame) -> dict:
    ...
    if normal_data_point_settings is not None and not normal_data_point_settings.empty:
        ...
        if os.getenv("FEED_ID"):
            resp = _split_request(investment_ids, data_point_list)
        else:
            postbody = {
...
def _split_request(investment_ids: list, data_point_list: list) -> Dict[str, Any]:
    split_size = 1000
    all_resp :dict= dict()
    all_resp["investments"]= []
    split_lists = [investment_ids[i:i + split_size] for i in range(0, len(investment_ids), split_size)]
    for item in split_lists:
        postbody = {
            "datapoints": data_point_list,
            "investments": list(map(lambda x: {"id": x}, item)),
        }
        resp = _request_investment_data(postbody)
        data_points = resp.get("datapoints", [])
        investments = resp.get("investments", [])
        all_resp["datapoints"] = data_points
        all_resp["investments"].extend(investments)
    return all_resp
