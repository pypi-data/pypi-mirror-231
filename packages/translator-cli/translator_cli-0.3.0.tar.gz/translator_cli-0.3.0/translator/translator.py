...
from translator.consts import CONFIG, ENGINES, APP_NAME, GPT_ENGINES
from translator.utils import (
    language_pair_is_available,
    get_language_code,
    build_chatgpt_translation_prompt,
    build_chatgpt_translation_prompt_from_list,
    build_chatgpt_system_directive,
...
class Translator:
    def __init__(self, engine_list=None, verbosity=1):
        ...
        if "deepl" in self.engine_list:
            try:
                self.deepl_translator = deepl.Translator(CONFIG["deepl"])
            except Exception:
        ...
        if "gpt3" in self.engine_list:
            try:
                self.chatgpt3 = ChatGPTPrompt(
                    api_key=CONFIG["openai"], engine="gpt-3.5-turbo-16k"
                )
            except Exception:
        ...
        if "gpt4" in self.engine_list:
            try:
                self.chatgpt4 = ChatGPTPrompt(api_key=CONFIG["openai"], engine="gpt-4")
            except Exception:
    ...
    def translate(
        ...
        try:
            if language_pair_is_available(engine, input_lang, output_lang) is False:
                ...
                logger.debug(
                    f"Language pairs for {engine}:\n{ts.get_languages(engine)}"
                )
