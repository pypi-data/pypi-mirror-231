...
def get_prompt_for_ppts(texts: List[str], input_lang: str, output_lang: str) -> str:
    ...
    prompt_v1 = (
        ...
        "to your prompt. Of course, all the text in the presentation belongs to the same context "
        "and probably shares a common topic. Furthermore, contents within the same slide will probably be more closely related to each other than "
        "those in different slides; contents sharing a shape will be even more related, and so on. In the next three lines I will provide you an example "
        "of what I want you to do. First line will be an informative header, indicating the origin and target languages. Second line will be the input I "
        "would be providing. Third line would be the translated content you should be generating. After the example, and separated by two linebreaks, I "
...
def structure_equals(lst1, lst2):
    """
    (GPT4 generated code)
    ...
    for elem1, elem2 in zip(lst1, lst2):
        if isinstance(elem1, list) or isinstance(elem2, list):
            if not structure_equals(elem1, elem2):
                return False
