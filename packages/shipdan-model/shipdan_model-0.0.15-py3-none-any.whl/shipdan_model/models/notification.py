...
class MainNoticeUserLog(models.Model):
    user = models.ForeignKey('accounts.User', related_name='main_notice_logs', on_delete=models.CASCADE)
...
class FCMDevice(models.Model):
    ...
    user = models.ForeignKey('accounts.User', related_name='fcm_devices', blank=True, null=True,
...
class AirbridgeDevice(models.Model):
    ...
    user = models.ForeignKey('accounts.User', related_name='airbridge_devices', blank=True, null=True,
                             on_delete=models.CASCADE)
...
class AmazonSNSSubscription(models.Model):
    ...
    user = models.ForeignKey(
        'accounts.User', related_name='amazon_sns_subscriptions', on_delete=models.CASCADE, null=True, default=None
    )
    arn = models.CharField(max_length=200, null=True, default=None, unique=True)
    attributes = models.JSONField(default=dict)
...
class PushRoutine(models.Model):
    ...
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name='push_routines')
    is_active = models.BooleanField(default=True)
    days = ArrayField(models.IntegerField(choices=DAYS))
    sent_at = ArrayField(models.TimeField())
    amazon_sns_subscription = models.OneToOneField(AmazonSNSSubscription, on_delete=models.SET_NULL, default=None,
                                                   null=True, related_name='push_routine')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
