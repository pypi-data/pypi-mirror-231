...
# todo 솔직히 아는 것 없어서 다 가져옴 - 검수 요망
class DietOrderSurvey(models.Model):
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name='diet_order_surveys')
...
class DietOrderSurveyComplete(models.Model):
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name='diet_order_surveys_completed')
    diet_order = models.OneToOneField(
        'food_payment.FoodOrder', related_name='surveys_completed', on_delete=models.CASCADE
    )
...
class DayDietSurveyComplete(models.Model):
    user = models.ForeignKey('accounts.User', on_delete=models.CASCADE, related_name='day_diet_surveys_completed')
    day_diet = models.OneToOneField('food_payment.OrderDayDietLog', related_name='survey_completed',
...
class DietOrderSurveyRelation(models.Model):
    ...
    diet_order = models.ForeignKey('food_payment.FoodOrder', related_name='survey_relations', on_delete=models.CASCADE)
...
class DietMealSurveyRelation(models.Model):
    ...
    day_diet = models.ForeignKey('food_payment.OrderDayDietLog', related_name='meal_survey_relations',
                                 on_delete=models.CASCADE)
...
class DietFoodSurveyRelation(models.Model):
    ...
    day_diet_food = models.ForeignKey('food_payment.OrderDayDietFoodLog', related_name='survey_relations',
                                      on_delete=models.CASCADE)
...
class FoodProductSurveyRelation(models.Model):
    ...
    food_product = models.ForeignKey('food.FoodProduct', related_name='survey_relations', on_delete=models.CASCADE)
