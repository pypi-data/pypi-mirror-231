...
from shipdan_model.models.food import (
...
class OperationProfile(models.Model):
    ...
    user = models.OneToOneField(
        'accounts.User', on_delete=models.SET_NULL, related_name='operation_profile',
        null=True
    )
    company = models.ForeignKey(
        'food.FoodCompany', on_delete=models.SET_NULL, related_name='operators', null=True, blank=True
    )
...
class OperationCompanyInfo(models.Model):
    company = models.OneToOneField(
        'food.FoodCompany', on_delete=models.SET_NULL, related_name='company_info',
        null=True
    )
...
class FoodProductChangeRequest(models.Model):
    ...
    food_product = models.ForeignKey(
        'food.FoodProduct', on_delete=models.SET_NULL, related_name='change_requests',
        null=True
    )
    status = models.IntegerField(choices=REQUEST_STATUS)
    requester = models.ForeignKey(
        'accounts.User', on_delete=models.SET_NULL, related_name='change_requests_from',
        null=True
    )
    approver = models.ForeignKey(
        'accounts.User', on_delete=models.SET_NULL, related_name='change_requests_to',
        null=True
    )
...
class FoodProductChangeRequestContent(models.Model):
    ...
    allergies = models.ManyToManyField('health.Allergy', )
...
class SalesProduct(models.Model):
    ...
    food_product = models.ForeignKey('food.FoodProduct', related_name='sales_products', on_delete=models.SET_NULL,
                                     null=True)
...
class OrderReleaseOrderInformation(models.Model):
    """ 쉽단 Order와 사방넷 발주 model Relation """

    order = models.OneToOneField('payment.Order', related_name='release_order_info', on_delete=models.CASCADE)
