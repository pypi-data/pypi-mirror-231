...
def plot_confusion_matrix(X, y, model, class_dict, title="", test_size=0.2, seed=0, filepath=None, display=True):
    ...
    for i, md in enumerate([model, DummyClassifier()]):
        ...
        for j, (cm_name, cm, _X, _y) in enumerate([("Train", cm_train, X_train, y_train),
                                                   ("Test", cm_test, X_test, y_test)]):
            ax = axs[i, j]
            ...
            sns.heatmap(cm, annot=annots, fmt='', square=True, xticklabels=labels, ax=ax,
                        yticklabels=labels, linewidths=1, cmap=cmap, cbar=False, linecolor='black')
...
def plot_regression_pred(X, y, models, y_label="", title="", test_size=0.2,
    ...
    plt.gca().add_patch(Rectangle((len(y_train) - 0.5, _y.min()), len(y_test), (_y.max() - _y.min()),
...
def plot_batch_results(df, metric_name, title="", filepath=None, display=True,
                       ignore_first_n=0, ignore_last_n=0):
    """
    ...
    _df = df.sort_values(by=y_label, ascending=False)
    _df = _df.iloc[ignore_first_n : (_df.shape[0] - ignore_last_n)]

    sns.scatterplot(data=_df, x=x_label, y=y_label, s=100, hue="Model", legend=False)
