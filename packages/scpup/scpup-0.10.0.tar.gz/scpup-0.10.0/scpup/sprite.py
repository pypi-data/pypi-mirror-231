...
def eau_collide(sprite1, sprite2) -> bool:
...
class EauSprite:
  ...
  @classmethod
  def __init_subclass__(cls, /, *paths, **kwargs):
    ...
    ```python
    class SomeSprite(EauSprite, 'path', 'to', 'image', 'image.png'): ...
    ...
    Args:
      *paths:
        The path segments under `<root>/assets/images/` of where the image of
        the subclass exist
    """
    super().__init_subclass__(**kwargs)
    EauSprite.subclasses[cls.__name__] = partial(cls, *paths)
  ...
  def do(self, action: scpup.EauAction, /, **__) -> None:
    """Action handler placeholder so that subclasses can use it if needed."""
    pass

  def clamp(self, rect: pygame.Rect | None = None):
  ...
  def on_collition(self, *args, **kwargs) -> None:
    ...
