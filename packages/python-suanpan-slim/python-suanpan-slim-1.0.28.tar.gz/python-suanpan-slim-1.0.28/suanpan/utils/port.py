...
def _get_open_ports():
    ...
    class MIB_TCPROW(ctypes.Structure):
        _fields_ = [('dwState', DWORD), ('dwLocalAddr', DWORD),
                    ('dwLocalPort', DWORD), ('dwRemoteAddr', DWORD),
                    ('dwRemotePort', DWORD)]
    ...
    class MIB_TCPTABLE(ctypes.Structure):
        _fields_ = [('dwNumEntries', DWORD), ('table', MIB_TCPROW * ANY_SIZE)]
    ...
    class MIB_UDPTABLE(ctypes.Structure):
        _fields_ = [('dwNumEntries', DWORD), ('table', MIB_UDPROW * ANY_SIZE)]
...
helper = PortHelper(int(g.portStart), int(g.portEnd))
...
def register_server(node_port, real_port):
    ...
    url = f"{protocol}://localhost:{g.port}/app/service/register"
    param = {
        "appId": g.appId,
        "nodeId": g.nodeId,
        "userId": g.userId,
