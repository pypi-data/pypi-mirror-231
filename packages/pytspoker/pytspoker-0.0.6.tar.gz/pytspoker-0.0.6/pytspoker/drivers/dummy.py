...
from pytspoker.voice import Voice
import time

def buildDriver(proxy):
    '''
    Builds a new instance of a drivers and returns it for use by the drivers
    proxy.

    @param proxy: Proxy creating the drivers
    @type proxy: L{drivers.DriverProxy}
    '''
...
class DummyDriver(object):
    ...
    properties, and sequencing responsibilities of a drivers implementation.

    @ivar _proxy: Driver proxy that manages this instance
    @type _proxy: L{drivers.DriverProxy}
    ...
    def destroy(self):
        '''
        Optional method that will be called when the drivers proxy is being
    ...
    def startLoop(self):
        '''
        Starts a blocking run loop in which drivers callbacks are properly
        invoked.
    ...
    def say(self, text):
        ...
        to the notifications on behalf of the drivers.

        When starting to output an utterance, the drivers must inform its proxy
        that it is busy by invoking L{drivers.DriverProxy.setBusy} with a flag
        of True. When the utterance completes or is interrupted, the drivers
        inform the proxy that it is no longer busy by invoking
        L{drivers.DriverProxy.setBusy} with a flag of False.
    ...
    def stop(self):
        '''
        Stops any current output. If an utterance was being spoken, the drivers
    ...
    def getProperty(self, name):
        ...
        voices: List of L{voice.Voice} objects supported by the drivers
