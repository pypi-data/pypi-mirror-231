...
def redo_success_determination(modeladmin, request, queryset):
    ...
    msg = _("{count} result items' success value changes").format(
        count=len(changed_items)
    )
    modeladmin.message_user(request, msg)


redo_success_determination.short_description = _(
    "Redo success determinaion on selected items"
)


def recompute_schedule_code(modeladmin, request, queryset):
    changed_items = update_schedules_code(queryset.al())
    msg = _("{count} schedule items's code value changed.").format(
        count=len(changed_items)
    )
    modeladmin.message_user(request, msg)
