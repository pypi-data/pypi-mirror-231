...
class ThirdSeatDeclarer(ThirdSeat):
    ...
    def _select_card_if_void(self) -> Card:
        ...
        # cprintf'{dashboard.tricks_needed=}', MODULE_COLOUR)
        # cprintf'{player.declarers_tricks=}', MODULE_COLOUR)
        # cprintf'{dashboard.sure_tricks.count=}', MODULE_COLOUR)
        # cprintf'{dashboard.probable_tricks.count=}', MODULE_COLOUR)
        # cprintf'{dashboard.possible_tricks.count=}', MODULE_COLOUR)
        # cprintf'{dashboard.sure_tricks.cards=}', MODULE_COLOUR)
        # cprintf'{dashboard.probable_tricks.cards=}', MODULE_COLOUR)
        # cprintf'{dashboard.possible_promotions.cards=}', MODULE_COLOUR)
        ...
        for suit_name, suit in suits.items():
            if player.suit_cards[suit_name]:
                ...
                # cprintf'{suit} {len(opponents_cards)}', MODULE_COLOUR)
                if opponents_cards:
