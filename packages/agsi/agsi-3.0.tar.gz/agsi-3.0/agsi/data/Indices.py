...
class bsi(Indices):
    ...
    def __calculate_bsi__(self):
        red_band = self.src.read(self.band_identifier['Red']).astype(float)
        nir_band = self.src.read(self.band_identifier['NIR']).astype(float)
        green_band = self.src.read(self.band_identifier['Green']).astype(float)
