...
from tool.FileTool import remove_path
...
class TreeType:
    ...
    @classmethod
    def options(cls):
        return [TreeType.SOURCE_ROOT, TreeType.SDKROOT, TreeType.GROUP, TreeType.ABSOLUTE, TreeType.DEVELOPER_DIR, TreeType.BUILT_PRODUCTS_DIR]
...
class PBXProjectHelper:
    ...
    def add_folder(self, target_name: str, path: str, parent: PBXObject, excludes=None):
        ...
        for child in os.listdir(path):
            ...
            if os.path.isfile(full_path) or filename in _SPECIAL_FOLDERS:
                ...
                self.add_file(target_name, full_path, parent, tree=TreeType.SOURCE_ROOT)
            else:
    ...
    def add_lan(self, target_name, lanpath, lan_name):
        ...
        file_ref_obj = PBXObject("", {'isa': "PBXFileReference", 'name': lan_name, 'path': f'{lan_name}.lproj/InfoPlist.strings', 'sourceTree': TreeType.GROUP})
