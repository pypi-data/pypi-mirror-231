...
def Trial_Version(cur, trial_version):
    try:
        c = cur.cursor()
        c.execute(trial_version)
        items = c.fetchall()
        ...
        if Check_Table(con, """ SELECT name FROM sqlite_master """) != "RaDin":
            ...
            while True:
                ...
                if (prompt == id):
                    ...
                    insert_table = """ CREATE TABLE IF NOT EXISTS RaDin (name text, nToE integer, ninth integer, matric integer, id integer); """           
                    insert_values = f""" INSERT INTO RaDin (name, nToE, ninth, matric, id) VALUES ("month", 0, 0, 0, {prompt}) """            
            ...
            if Check_ID(con, """ SELECT id From RaDin where name = 'month' """) == id:
                ...
                while True:
                    ...
                    if (user_input == "e"):
                        ...
                        trial_version = """ SELECT name FROM sqlite_master """    
                        if (Trial_Version(con, trial_version) >= 5):
                            print("You can't insert more class in trial version.")
