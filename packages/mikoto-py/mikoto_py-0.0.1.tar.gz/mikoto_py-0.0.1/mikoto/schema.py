...
class SpaceService:
    ...
    def on_create(self, fn: Callable[[Space], None]):
        @self.client.on("spaces/onCreate")
        async def handler(data):
            await fn(TypeAdapter(Space).validate_python(data))
        return handler

    def on_update(self, fn: Callable[[Space], None]):
        @self.client.on("spaces/onUpdate")
        async def handler(data):
            await fn(TypeAdapter(Space).validate_python(data))
        return handler

    def on_delete(self, fn: Callable[[Space], None]):
...
class MemberService:
    ...
    def on_create(self, fn: Callable[[Member], None]):
        @self.client.on("members/onCreate")
        async def handler(data):
            await fn(TypeAdapter(Member).validate_python(data))
        return handler

    def on_update(self, fn: Callable[[Member], None]):
        @self.client.on("members/onUpdate")
        async def handler(data):
            await fn(TypeAdapter(Member).validate_python(data))
        return handler

    def on_delete(self, fn: Callable[[Member], None]):
...
class UserService:
    ...
    def on_create(self, fn: Callable[[User], None]):
        @self.client.on("users/onCreate")
        async def handler(data):
            await fn(TypeAdapter(User).validate_python(data))
        return handler

    def on_update(self, fn: Callable[[User], None]):
        @self.client.on("users/onUpdate")
        async def handler(data):
            await fn(TypeAdapter(User).validate_python(data))
        return handler

    def on_delete(self, fn: Callable[[User], None]):
...
class MessageService:
    ...
    def on_create(self, fn: Callable[[Message], None]):
        @self.client.on("messages/onCreate")
        async def handler(data):
            await fn(TypeAdapter(Message).validate_python(data))
        return handler

    def on_update(self, fn: Callable[[Message], None]):
        @self.client.on("messages/onUpdate")
        async def handler(data):
            await fn(TypeAdapter(Message).validate_python(data))
        return handler

    def on_delete(self, fn: Callable[[MessageDeleteEvent], None]):
...
class RoleService:
    ...
    def on_create(self, fn: Callable[[Role], None]):
        @self.client.on("roles/onCreate")
        async def handler(data):
            await fn(TypeAdapter(Role).validate_python(data))
        return handler

    def on_update(self, fn: Callable[[Role], None]):
        @self.client.on("roles/onUpdate")
        async def handler(data):
            await fn(TypeAdapter(Role).validate_python(data))
        return handler

    def on_delete(self, fn: Callable[[Role], None]):
