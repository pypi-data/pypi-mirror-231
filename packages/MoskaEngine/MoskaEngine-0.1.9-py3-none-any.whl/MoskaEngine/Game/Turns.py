...
class Turn(ABC):
    """
    Abstract base class for game turns.

    Defines the interface for a game turn, which consists of a `__call__` method
    and a `play` method. Subclasses must implement these methods.
    """
...
class InitialPlay(_PlayToPlayer):
    ...
    def check_single_or_multiple(self):
        """
        Check if the cards in the turn are either a single card or multiple cards of the same rank.

        Returns:
            bool: True if the cards are either a single card or multiple cards of the same rank, False otherwise.
        """
...
class PlayToSelfFromDeck(_PlayToPlayer):
    """
    A class representing a turn where a player plays cards from their own deck to themselves.

    Attributes:
    - player (AbstractPlayer): The player who is playing the cards.
    - target (AbstractPlayer): The player who is the target of the cards (in this case, the same as the player).
    - cards (List[Card]): The cards being played.

    Methods:
    - __call__(self, player : AbstractPlayer, target : AbstractPlayer, cards : List[Card]): Plays the cards to the player.
    """
    def __call__(self, player : AbstractPlayer, target : AbstractPlayer, cards : List[Card]):
        assert utils.check_signature([AbstractPlayer,AbstractPlayer,list],[player,target,cards]), "Incorrect input signature"
        self.player = player
...
class PlayToSelf(PlayToOther):
    """
    A class representing a turn where a player plays to themselves.
    Inherits from the PlayToOther class.
    """
    pass
...
class PlayFallFromHand(Turn):
    ...
    def __call__(self, player: AbstractPlayer, play_fall: Dict[Card, Card]):
        """
        Executes a turn for the given player with the specified cards to play and fall.

        Args:
            player (AbstractPlayer): The player who is taking the turn.
            play_fall (Dict[Card, Card]): A dictionary mapping the cards to play to the cards to fall.

        Raises:
            AssertionError: If the input signature is incorrect, some of the played cards were not matched to a correct card to fall,
                the player does not have the turn, some of the played cards are not available, or some of the cards were played multiple times.
        """
    ...
    def check_player_has_turn(self):
            """
            Check if the current player has the turn.

            Returns:
                bool: True if the current player has the turn, False otherwise.
            """
...
class EndTurn(Turn):
    ...
    def clear_table(self):
            """
            Clears the table of cards after a player's turn has ended. If the player picked up cards from the table,
            only the cards that were not picked up are removed. If the player did not pick up any cards, all cards on
            the table are removed if the player is also the target player.
...
class Skip(Turn):
    """
    A class representing a turn where the player skips their turn.

    Args:
    moskaGame (MoskaGame): The MoskaGame instance for which the turn is being played.

    Attributes:
    moskaGame (MoskaGame): The MoskaGame instance for which the turn is being played.
    player (AbstractPlayer): The player who is playing the turn.
    """
    ...
    def __call__(self, player: AbstractPlayer):
        ...
        Args:
            player (AbstractPlayer): The player whose turn it is.

        Raises:
            AssertionError: If the input signature is incorrect, the game is not initiated, or there are no plays left.
