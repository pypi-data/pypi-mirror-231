...
from utils.local import get_ip
...
class MessageParser(object):
    ...
    def parse(self, packet, callback = None):
        ...
        plen = len(packet)
        if plen < 10:
            print(f"MessageParser.parse. Not support message len: {len}")
            return
    ...
    def parse_by_cmd(self, cmd, data):
        ...
        result = (int)(data[8])
        if result != 0:
            print(f"cmd {hex(cmd)} result is {result}")
            return None
    ...
    def signals(self, data):
        ...
        packet["sample_length"] = int.from_bytes(data[57:61], 'little')
        packet["resolution"] = int.from_bytes(data[61:63], 'little')
        packet["filter"] = int(data[63])
        packet["date_len"] = int.from_bytes(data[64:68], 'little')
        packet["data"] = data[68:]
...
class X7Parser(object):
    ...
    def parse(self, packet):
        ...
        if cmd == 0x10:
            print(parse_net_param(packet[12:-2]))
