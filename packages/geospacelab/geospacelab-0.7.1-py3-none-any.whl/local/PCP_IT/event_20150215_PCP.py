...
def show_IMF_and_TEC():
    ...
    db_omni.set_layout(panel_layouts=layout, left=0.15, right=0.85, top=0.4, bottom=0.1)
    db_omni.draw()
    # db_omni.add_vertical_line(time_c, color='r', linewidth=2, linestyle='-')
    # db_omni.add_shading(dt_fr, time_c, alpha=0.15)
    db_omni.add_panel_labels(labels=['b', 'c', 'd'])
    ...
    db_tec.set_layout(2, 4, left=0.05, right=0.9, bottom=0.45, top=0.95, wspace=0.19, )
    ind = 0
    for row in range(2):
        for col in range(4):
            time_c = dts_tec[ind]
            ...
            ind += 1
    db_omni.save_figure(file_name="IMF_with_TECs_20150215_v3", file_dir=file_dir_root)
    pass


def show_IMF_TEC_2():
    fig = create_figure(figsize=(14, 8))

    dt_fr_omni = datetime.datetime(2015, 2, 15, 12)
    dt_to_omni = datetime.datetime(2015, 2, 16, 3)
    # dt_fr_omni = datetime.datetime(2015, 2, 27, 0)
    # dt_to_omni = datetime.datetime(2015, 2, 28, 0)
    db_omni = fig.add_dashboard(dashboard_class=TSDashboard, dt_fr=dt_fr_omni, dt_to=dt_to_omni)
    omni_config = {
    ...
    ds_4 = db_omni.dock(datasource_contents=['gfz', 'kpap'])
    ds_5 = db_omni.dock(datasource_contents=['supermag', 'indices'])
    # fp_ie = ['/home/lei/extra/AFEISCAT3/Data/Indices/IL/IL20150215.dat', '/home/lei/extra/AFEISCAT3/Data/Indices/IL/IL20150216.dat']
    ...
    sme = ds_5['SML']
    sme.visual.axis[1].lim = [None, None]
    sme.visual.axis[1].label = "Indices"
    ie = ds_6['IL']
    layout = [[Bx, By, Bz], [v_sw], [n_p], [ncf], [sme, ie], [sym_h], [kp]]

    ######################3
    dt_fr_tec = datetime.datetime(2015, 2, 15, 15)
    dt_to_tec = datetime.datetime(2015, 2, 15, 23)
    db_tec = fig.add_dashboard(dashboard_class=GeoDashboard, dt_fr=dt_fr_tec, dt_to=dt_to_tec)
    ds_tec = db_tec.dock(datasource_contents=['madrigal', 'gnss', 'tecmap'])
    tec = db_tec.assign_variable('TEC_MAP', dataset_index=0)
    dts = db_tec.assign_variable('DATETIME', dataset_index=0).value.flatten()
    ...
    for dt_c in times:
        db_omni.set_layout(panel_layouts=layout, left=0.1, right=0.5, top=0.9, bottom=0.1)
        db_omni.draw()
        # db_omni.add_vertical_line(time_c, color='r', linewidth=2, linestyle='-')
        # db_omni.add_shading(dt_fr, time_c, alpha=0.15)
        db_omni.add_panel_labels()
        db_omni.add_vertical_line(dt_c, color='r', linewidth=2, linestyle='-')
        db_omni.add_shading(dt_fr, dt_c, alpha=0.15)
        db_omni.add_panel_labels()

        #######
        db_tec.set_layout(1, 1, left=0.6, right=0.9, bottom=0.1, top=0.9, )

        ind_tec = ds_tec.get_time_ind(dt_c, time_res=300)
        panel = db_tec.add_polar_map(
            row_ind=0, col_ind=0,
            style='mlt-fixed', cs='AACGM', mlt_c=0.,
            pole='N', ut=dt_c, boundary_lat=55,
        )
        # panel1 = db.add_polar_map(
        #     row_ind=0, col_ind=0,
        #     style='lst-fixed', cs='GEO', lst_c=0.,
        ...
        ind_t = ds_sd.get_time_ind(ut=dt_c)
        # initialize the polar map
        ...
        panel.add_label(x=0.0, y=0.95, label="(h)", fontsize=12)
        panel.add_title(0.5, 1.1, title="GNSS TEC, {:s}".format(dt_c.strftime("%Y-%m-%d %H:%M UT")), fontsize=12)

        fp_res_rec = file_dir_root / "IMF_TECs/jpg/"
        fp_res_rec.mkdir(exist_ok=True)
        fp_res_rec = fp_res_rec / ("TEC_MAP_OMNI_" + dt_c.strftime("%Y-%m-%d_%H%M") + ".jpg")
        print(dt_c)
