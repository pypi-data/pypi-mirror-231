...
class ChangGuangMicroscopeFile(MicroscopeSmallImage):
    ...
    def _from_info_json(self, ):
        ...
        with open(self.info_json, 'r') as fd:
            ...
            self.scan.fov_rows, self.scan.fov_cols = [dct['Info']['ScanRows'], dct['Info']['ScanCols']]
            self.scan.fov_height = dct['Info']['FOVHeight']
            self.scan.fov_width = dct['Info']['FOVWidth']

            self.fov_location = np.array(dct['fovs_pos'])[:, :, ::-1]
            ...
            self.scan.objective = dct['Info']['ScanObjective']
            self.scan.scale_x = self.scan.scale_y = dct['Info']['Scale']
            self.scan.exposure_time = dct['Info']['ExposureTime']
