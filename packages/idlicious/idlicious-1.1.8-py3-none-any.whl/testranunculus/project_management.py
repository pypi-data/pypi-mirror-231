    ...
    if uploaded_file is not None:
        ...
        if file_extension == 'py':
            ...
            if verify_functions(tmp_file_path, function_dict):
                with st.status('Saving your custom LLM...', expanded=True):
                    success_message = st.empty()
                    success_message.write('Functions verified successfully, you can now save the file by clicking the button below')
                    time.sleep(2)
                    ...
                    if os.path.exists(file_path):
                        ...
                        if overwrite == 'Yes':
                            ...
                            st.info('File not saved, if you want to overwrite the file, select Yes')
                    else:
                        # Save the file to the data folder
                        with open(file_path, 'wb') as f:
                            f.write(uploaded_file.getbuffer())
...
def config_project():
    ...
    upload_custom_llm = st.checkbox(
        "Upload custom LLM", 
        value=False,
        help="Do you want to upload a custom LLM file?",
        )
    if upload_custom_llm:
        upload_custom_llm_file()
