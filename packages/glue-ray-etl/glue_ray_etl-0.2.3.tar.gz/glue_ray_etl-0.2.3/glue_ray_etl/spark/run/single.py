import warnings
from typing import Any, Optional, Iterable, Callable, Type
...
def in_pandas_map_partition(spdf: PySparkDataFrame, f: Callable[[PandasDataFrame, ...], PandasDataFrame],
                            ...
                            warning_modes: tuple[tuple[str, Type[Warning]], ...] = (('ignore', Warning), ),
                            **kwargs):
    if schema is None:
    ...
    def f_iter(pdfs):
        with warnings.catch_warnings():
            for warning_mode, warning_type in warning_modes:
                warnings.simplefilter(warning_mode, category=warning_type)
