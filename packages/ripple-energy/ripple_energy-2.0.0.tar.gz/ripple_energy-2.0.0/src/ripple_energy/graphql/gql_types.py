...
class Permission(GQLObject):
   """
   userSet - Specific permissions for this user.
...
class Group(GQLObject):
   """
   userSet - The groups this user belongs to. A user will get all permissions granted to each of their groups.
...
class User(GQLObject):
   ...
   isGuest - Designates whether this user should be treated as Guest. Guest accounts are created for users who don't want to or can't create an account so it is created for them

   isEmailVerified - Designates whether email has been verified 
