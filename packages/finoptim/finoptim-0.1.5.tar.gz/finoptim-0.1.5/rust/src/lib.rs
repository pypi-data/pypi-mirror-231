...
use numpy::{
    IntoPyArray, PyArray1, PyReadonlyArray1, PyReadonlyArray2, PyReadonlyArray3, PyReadonlyArrayDyn,
};
use pyo3::prelude::*;
...
// impl AsMut for Convergence {
//     fn as_mut(&mut self) -> &mut  {
//         &mut self.0
//     }
// }

// impl AsMut<Option<Vec<f64>>> for Convergence
// // where
// //     Option<Vec<f64>>: AsMut<T>,
// {
//     fn as_mut(&mut self) -> &mut Option<Vec<f64>> {
//         // self.coverages.expect().as_mut()
//         self
//     }
// }
...
fn rust_as_backend(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    fn py_optimise_predictions(
        ...
        let (_, timespan, n) = predictions.dim();
        ...
        for (i, p) in pricing_models.into_iter().enumerate() {
            ...
            match model {
                PricingModel::Reservations(Term::OneYear, _, _)
                | PricingModel::Reservations(Term::ThreeYears, _, _) => {
                    space.reserve(n);
        ...
        let fres = finalise_results(res, &models, cov, underutil);
    ...
    fn py_optim_final(
        ...
        //     .into_par_iter()
        //     .map_init(
        //         || CostFunction::new(usage.clone(), &models, days, record),
        //         |local_cost_function, start| inertial_optimiser(local_cost_function, start),
        //     )
        //     .collect_into_vec(&mut results);
        ...
        println!("{:?}", levels);
        println!("days : {days}");
        let mut c = CostFunction::new(usage.clone(), &models, days, record);
        // let steps = c.steps.clone();
        // let res = match c.record {
        //     false => default_optimiser(&mut c, steps.view(), levels),
        //     true => default_optimiser_details(&mut c, steps.view(), levels)
        // };
