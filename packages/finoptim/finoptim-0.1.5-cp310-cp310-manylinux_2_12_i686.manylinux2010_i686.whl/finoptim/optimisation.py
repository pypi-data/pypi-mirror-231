...
from finoptim.validations import __standardize_prices__, __find_period__, __standardize_usage_data__
...
def optimise(data: Union[list[pd.DataFrame], pd.DataFrame],
    ...
    ### TO DO
    # check if prices have the same dimension as usae
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if len(horizon) == 1:
            ...
            print(f"convex optimisation with period {period}")
            ...
            res = rs.simple_optimisation(
                X, prices.values, period, horizon, convergence_detail, step=10)
        else:
            print("inertial optimisation", end='...')
