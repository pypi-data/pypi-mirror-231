# Python Client For Nutanix Aiops Versioned APIs
...
```python
# Initialize the API
stats_api_instance = StatsApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Returns a list of various attributes and metrics (time series data) that are available for a given entity type.
try:
    api_response = stats_api_instance.get_entity_metrics_v4(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=aiops&version=v4.0.a2&language=python). This documentation is auto-generated, and the location may change.
