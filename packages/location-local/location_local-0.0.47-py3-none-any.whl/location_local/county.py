...
class County(GenericCRUD):
    ...
    def insert(
            ...
            county: str, lang_code: LangCode, title_approved: bool = False) -> int:
        logger.start(object={'coordinate': coordinate, 'county': county,
                     'lang_code': lang_code, 'title_approved': title_approved})
        insert_sql = "INSERT IGNORE INTO county_table (coordinate) VALUES (POINT(%s, %s))"
        self.cursor.execute(
            insert_sql, (coordinate["latitude"], coordinate["longitude"]))
        county_id = self.cursor.lastrowid()
        insert_ml_sql = "INSERT IGNORE INTO county_ml_table (county_id, lang_code," \
            " title, title_approved) VALUES (%s, %s, %s, %s)"
        self.cursor.execute(insert_ml_sql, (county_id,
    ...
    @staticmethod
    def get_county_id_by_county_name(county_name: str) -> int:
        logger.start(object={'county_name': county_name})

        connector = Connector.connect("location")
        cursor = connector.cursor()

        select_sql = "SELECT county_id FROM county_ml_view WHERE title = %s LIMIT 1"
        cursor.execute(select_sql, (county_name,))
