...
class Country(GenericCRUD):
    ...
    def insert(self, coordinate: Dict[str, float],
        ...
        try:
            insert_sql = "INSERT IGNORE INTO country_table (coordinate, iso, `name`, nicename, iso3, numcode, phonecode) VALUES (POINT(%s, %s), %s, %s, %s, %s, %s, %s)"
            self.cursor.execute(
                insert_sql,
                (coordinate["latitude"],
        ...
        try:
            ...
            insert_ml_sql = "INSERT IGNORE INTO country_ml_table (country_id, lang_code, title," \
                " title_approved) VALUES (%s, %s, %s, %s)"
            self.cursor.execute(insert_ml_sql, (country_id,
    ...
    def get_country_id_by_country_name(country_name: str) -> int:
        ...
        select_sql = "SELECT country_id FROM country_ml_view WHERE `name` = %s LIMIT 1"
        cursor.execute(select_sql, (country_name,))
