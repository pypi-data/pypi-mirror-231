...
class Neighborhood(GenericCRUD):
    ...
    def insert(
            ...
            neighborhood: str, lang_code: LangCode, title_approved: bool = False) -> int:
        logger.start(object={'coordinate': coordinate, 'neighborhood': neighborhood,
                     'lang_code': lang_code})
        insert_sql = "INSERT IGNORE INTO neighborhood_table (coordinate) VALUES (POINT(%s, %s))"
        self.cursor.execute(
            insert_sql, (coordinate["latitude"], coordinate["longitude"]))
        neighborhood_id = self.cursor.lastrowid()
        insert_ml_sql = "INSERT IGNORE INTO neighborhood_ml_table (neighborhood_id, lang_code," \
            " title, title_approved) VALUES (%s, %s, %s, %s)"
        self.cursor.execute(insert_ml_sql, (neighborhood_id,
    ...
    @staticmethod
    def get_neighborhood_ids_by_neighborhood_name(neighborhood_name: str) -> list[int]:
        logger.start(object={'neighborhood_name': neighborhood_name})

        connector = Connector.connect("location")
        cursor = connector.cursor()

        select_sql = "SELECT neighborhood_id FROM neighborhood_ml_view WHERE title = %s"
        cursor.execute(select_sql, (neighborhood_name,))
