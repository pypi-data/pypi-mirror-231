...
class MessageSender:
    ...
    def send_mouse_move_to(self, x, y, button='', duration=0.01):
        ...
        # 根据距离计算点的数量
        num_points = max(int(distance / 18), 2)
        number = random.randrange(num_points, num_points + 10)

        print('start', start, 'end', end, 'distance', distance, 'num_points', num_points, 'number', number)
