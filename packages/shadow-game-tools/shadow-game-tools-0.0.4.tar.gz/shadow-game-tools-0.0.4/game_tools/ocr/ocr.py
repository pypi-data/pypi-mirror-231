...
class OCRDetector:
    ...
    def paddle_ocr(self, image, hwnd, x1, y1, x2, y2, color_ranges=None):
        img = self.image_color_regions(image, x1, y1, x2, y2, color_ranges)
        path = save_image(img, f'paddle-ocr-{hwnd}.bmp')
        return self._rpc_list[0].ocr(path)

    def easy_ocr(self, image, hwnd, x1, y1, x2, y2, color_ranges=None, allowlist=None):
        img = self.image_color_regions(image, x1, y1, x2, y2, color_ranges)
        path = save_image(img, f'easy-ocr-{hwnd}.bmp')
