...
def get_historic_values(  # pylint: disable=R0914
    ...
    for tag in set(tags):
        ...
        response_value = requests.get(f"https://{host}/{tag.controller_id}/value/{tag.get_identifier()}?ts={start_s}&time_format=epoch", headers=header, timeout=(5, None))
        response_series = requests.get(
            f"https://{host}/{tag.controller_id}/timeseries/{tag.get_identifier()}?from={start_s}&to={end_s}&time_format=epoch", headers=header, timeout=(5, None)
        )
...
def write_value(
    ...
    url = f"https://{host}/v1/controllers/{tag.controller_id}/tags/{tag.get_identifier()}"
