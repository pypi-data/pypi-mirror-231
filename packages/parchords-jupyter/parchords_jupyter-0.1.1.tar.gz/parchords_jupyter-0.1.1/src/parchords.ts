...
d3s.select.prototype.transition = d3Transition;
...
export class ParChords {
  ...
  private axisScale: d3sc.ScalePoint<string>;
  private colorScale: d3sc.ScaleOrdinal<string, string, never>;

  constructor(view: DOMWidgetView) {
    this.view = view;
    this.axisScale = d3sc.scalePoint().padding(0.1);
    this.colorScale = d3sc.scaleOrdinal(schemeSet3);
  ...
  private updateData() {
    ...
    const selection = this.view.model.get('selection') as number[];
    ...
    const pathGenerator = d3line()
      .x((_v, vi) => this.axisScale(fields[vi]) as number)
    ...
    gSubstrate
      ...
      .style('opacity', (_d, i) =>
        selection.find((v) => v === i) ? 0.9 : defOpacity
      );

    let dragOrigXPos = 0;
    const dragBehavior = d3drag<SVGTextElement, string>();
    dragBehavior
      ...
      .on('end', (event, datum) => {
        ...
        const oldFields = this.view.model.get('axis_fields') as string[];
        ...
        if (oldIndex !== -1) {
          ...
          const newIndex = Math.max(
            0,
            Math.min(oldIndex + steps, newFields.length)
          );
    ...
    gLegend
      ...
      .attr('width', 7)
      .attr('height', 2)
      .style('fill', (d: string) => this.colorScale(d));
  }
