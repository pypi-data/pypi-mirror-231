...
const d3_drag_1 = __webpack_require__(/*! d3-drag */ "webpack/sharing/consume/default/d3-drag/d3-drag");
const d3s = __importStar(__webpack_require__(/*! d3-selection */ "webpack/sharing/consume/default/d3-selection/d3-selection?5fc9"));
...
const d3_transition_1 = __webpack_require__(/*! d3-transition */ "webpack/sharing/consume/default/d3-transition/d3-transition");
d3s.select.prototype.transition = d3_transition_1.transition;
const MARGIN = { top: 20, right: 15, bottom: 5, left: 30 };
const LEGEND_WIDTH = 50;
class ParChords {
    constructor(view) {
        this.view = view;
        this.axisScale = d3sc.scalePoint().padding(0.1);
        this.colorScale = d3sc.scaleOrdinal(d3_scale_chromatic_1.schemeSet3);
        const g = d3s
            .select(view.el)
            .append('svg')
            .attr('width', 100)
        ...
        this.view.model.on('change:selection', this.updateSelection, this);
        ...
        view.model.on('msg:custom', (payload) => this.highlightItem(payload.idx));
    }
    updateSubstrateSize() {
    ...
    updateData() {
        ...
        const selection = this.view.model.get('selection');
        const colorValues = d3a
            .rollups(cValues, (v) => v.length, (d) => d)
            .sort((a, b) => (a[1] < b[1] ? 1 : -1))
            .map((v) => v[0]);
        this.colorScale.domain(colorValues);
        ...
        // update the scale for x positions of axes
        this.axisScale.domain(fields).range([0, substWidth]);
        // init array of scales for y-position of points on parallel axes
        ...
        const pathGenerator = d3_shape_1.line()
            .x((_v, vi) => this.axisScale(fields[vi]))
            .y((v, vi) => valueScales[vi](v));
        const defOpacity = selection.length > 0 ? 0.03 : 0.5;
        // encode each item into a path
        gSubstrate
            .selectAll('path')
            .data(values)
            .join('path')
            .attr('class', (_d, i) => 'idx' + i)
            .attr('d', pathGenerator)
            .style('fill', 'none')
            .style('stroke', (_d, i) => this.colorScale(cValues[i]))
            .style('opacity', (_d, i) => selection.find((v) => v === i) ? 0.9 : defOpacity);
        let dragOrigXPos = 0;
        const dragBehavior = d3_drag_1.drag();
        dragBehavior
            .on('start', (event, datum) => {
            console.log('dragBehavior start: ' + datum);
            // n.b. d3.pointer is misleading here because DOM context changes from start to end
            dragOrigXPos = event.x;
        })
            // .on("drag", (event, d) => (d.x = event.x, d.y = event.y))
            .on('end', (event, datum) => {
            console.log('dragBehavior end: ' + datum);
            // console.log(event);
            const deltaX = event.x - dragOrigXPos;
            const steps = (deltaX / this.axisScale.step()) >> 0;
            // console.log('origX: ' + dragOrigXPos);
            // console.log('new X: ' + event.x);
            // console.log('delta: ' + deltaX);
            console.log('steps: ' + steps);
            const oldFields = this.view.model.get('axis_fields');
            const oldIndex = oldFields.indexOf(datum);
            if (oldIndex !== -1) {
                // n.b. we need to create a new array object
                const newFields = oldFields.filter((item) => item !== datum);
                // newFields.splice(oldIndex, 1);
                const newIndex = Math.max(0, Math.min(oldIndex + steps, newFields.length));
                newFields.splice(newIndex, 0, datum);
                console.log(newFields);
                this.view.model.set('axis_fields', newFields);
        ...
        gSubstrate
            ...
            .attr('transform', (d) => 'translate(' + this.axisScale(d) + ')')
            .each(function (d, i) {
    ...
    highlightItem(idx) {
        ...
        const path = d3s.select(this.view.el).selectAll('path.idx' + idx);
        const origColor = path.style('stroke');
        path.style('stroke', 'red');
        path.raise();
        path.transition().delay(100).duration(400).style('stroke', origColor);
    }
}
...
class ParChordsModel extends base_1.DOMWidgetModel {
    defaults() {
        return Object.assign(Object.assign({}, super.defaults()), { _model_name: ParChordsModel.model_name, _model_module: ParChordsModel.model_module, _model_module_version: ParChordsModel.model_module_version, _view_name: ParChordsModel.view_name, _view_module: ParChordsModel.view_module, _view_module_version: ParChordsModel.view_module_version, axis_fields: [], color_field: '', _marks_val: [], _marks_color: [], selection: [], width: 700, height: 400 });
    }
}
