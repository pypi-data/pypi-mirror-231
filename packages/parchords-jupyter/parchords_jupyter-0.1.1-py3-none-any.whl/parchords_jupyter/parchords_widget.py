...
class ParChordsWidget(DOMWidget):
    ...
    selection = List(Int()).tag(sync=True)
    """ Row numbers of selected items """
    ...
    def _observe_data(self, change):
        ...
        if not len(self.axis_fields) == 0:
            self._marks_val = change.new[self.axis_fields].dropna().values.tolist()
    ...
    def _observe_fields(self, change):
        # print('## PCP ## update field ' + change.name + ' to: ' + ' '.join(change.new))
        if len(change.new) != 0:
            self._marks_val = self.data[change.new].dropna().values.tolist()
        else:
            self._marks_val = []
