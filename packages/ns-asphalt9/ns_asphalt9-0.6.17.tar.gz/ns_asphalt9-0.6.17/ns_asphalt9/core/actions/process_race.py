...
def race_worker(progress_manager, navi_data, pro, race_navi_event):
    ...
    while race_navi_event.is_set():
        ...
        action = get_action(progress, navi_data, offset_progress)
        if action:
            if action in ["DPAD_LEFT", "DPAD_RIGHT"]:
                pro.press_group(action, 0)
            if action == "DPAD_LEFT_DPAD_LEFT":
                pro.press_group(["DPAD_LEFT"] * 2, 0)
            if action == "DPAD_RIGHT_DPAD_RIGHT":
                pro.press_group(["DPAD_RIGHT"] * 2, 0)
        if count % 5 == 0:
...
def process_race():
    ...
    if race_navi_event.is_set():
        ...
        for i in range(500):
            ...
            if progress < 0 and ocr.OCR.has_next() or none_progress_count > 10:
                break
