...
@fixture
def expected_cancel_operation_request(execute_op_id) -> CancelOperationRequest:
    return CancelOperationRequest(operation_id=execute_op_id)
...
class RequestsData:
    ...
    expected_cancel_operation_request: CancelOperationRequest
    oauth_token: str
...
def requests_data(
        ...
        expected_cancel_operation_request,
        oauth_token,
        expected_metadata,
) -> RequestsData:
    return RequestsData(
...
@fixture
def running_op(execute_op_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, expected_download_files, OperationStatus.RUNNING)


@fixture
def successful_op(execute_op_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, expected_download_files, OperationStatus.SUCCESS)


@fixture
def program_error_op(execute_op_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, expected_download_files, OperationStatus.SUCCESS, program_error=True)


@fixture
def system_error_op(execute_op_id, expected_download_files) -> Operation:
    return get_op(execute_op_id, expected_download_files, OperationStatus.FAILURE)


@fixture
def args(cfg, project_id):
    return Mock(config=cfg.absolute(), project_id=project_id)
...
def common_logs(execute_op_id) -> List[tuple]:
    return [
        ...
        ('datasphere.main', 10, 'operation `exec-op-id` performs the job'),
...
def get_execute_mocks(mocker, code_data, files_data, requests_data, get_op_side_effects: list):
    ...
    stub.Execute.return_value = Operation(id=requests_data.execute_op_id)
...
def assert_common_mocks_calls(
    ...
    if execution_status == ExecutionStatus.CANCEL:
        op_stub.Cancel.assert_called_once_with(requests_data.expected_cancel_operation_request,
...
def test_successful_run(
    ...
    get_op_side_effects = [running_op, running_op, successful_op]  # wait 2 times then get result

    mocks = get_execute_mocks(mocker, code_data, files_data, requests_data, get_op_side_effects)

    caplog.set_level(logging.DEBUG)

    execute(oauth_token=requests_data.oauth_token, args=args)
...
def test_program_failed_run(
    ...
    get_op_side_effects = [running_op, program_error_op]
...
def test_canceled_run(
    ...
    get_op_side_effects = [running_op, KeyboardInterrupt]
...
def test_list(mocker, capsys, requests_data, project_id, args):
    ...
    ls(oauth_token=requests_data.oauth_token, args=args)
