...
from easyass.common.assattr import AssAttr
from easyass.common.asstypes import AssColor
...
class Styles(list):
    ...
    def parse(self, ass_str: str) -> Errors:
        ...
        if title == FORMAT_LINE_TITLE:
            ...
            format_err = self.style_format.parse(ass_str)
            ...
            style_err = style_item.parse(ass_str, self.style_format)
            style_err.pos('item')
            err += style_err
...
class StyleFormat(list):
    def __init__(self, *args):
        super().__init__()
        if not len(args):
            self.extend(STYLE_ATTR_DEF.keys())
...
class StyleItem:
    ...
    def parse(self, ass_str: str, style_format: StyleFormat) -> Errors:
        ...
        if len(style_values) != len(style_format):
            return err.error(f'`Styles` line does not match `Format` line. ')
        for index, style_attr in enumerate(style_format):
            try:
