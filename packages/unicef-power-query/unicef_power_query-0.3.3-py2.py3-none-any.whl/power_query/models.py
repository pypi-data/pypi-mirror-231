...
class CeleryEnabled(models.Model):
    SCHEDULED = frozenset(
        {states.PENDING, states.RECEIVED, states.STARTED, states.RETRY}
    )
...
class Parametrizer(NaturalKeyModel, models.Model):
    ...
    source = models.ForeignKey(
        "Query", blank=True, null=True, on_delete=models.CASCADE, related_name="+"
    )
...
class Query(NaturalKeyModel, CeleryEnabled, models.Model):
    ...
    owner = models.ForeignKey(
        get_user_model(), on_delete=models.CASCADE, related_name="power_queries"
    )
    ...
    parametrizer = models.ForeignKey(
        Parametrizer, on_delete=models.CASCADE, blank=True, null=True
    )
    ...
    def execute_matrix(self, persist: bool = True, **kwargs: Any) -> QueryMatrixResult:
        ...
        results: QueryMatrixResult = {
            "timestamp": strftime(timezone.now(), "%Y-%m-%d %H:%M")
        }
        with configure_scope() as scope:
            ...
            with transaction.atomic():
                ...
                self.datasets.exclude(
                    pk__in=[dpk for dpk in results.values() if isinstance(dpk, int)]
                ).delete()
        return results
    ...
    def run(
        ...
        if settings.POWER_QUERY_EXTRA_CONNECTIONS:
            connections_model.extend(
                [
                    django_apps.get_model(label2model)
                    for label2model in settings.POWER_QUERY_EXTRA_CONNECTIONS
                ]
            )
        connections = {
            f"{model._meta.object_name}Manager": model._default_manager.using(
        ...
        try:
            ...
            signature = dict_hash(
                {"query": self.pk, **(arguments if arguments else {})}
            )
            if use_existing and (
                ds := Dataset.objects.filter(query=self, hash=signature).first()
            ):
...
class Dataset(NaturalKeyModel, models.Model):
    ...
    extra = models.BinaryField(
        null=True, blank=True, help_text="Any other attribute to pass to the formatter"
    )
...
class ReportDocument(models.Model):
    ...
    report = models.ForeignKey(
        Report, on_delete=models.CASCADE, related_name="documents"
    )
