...
def get_amendments(soup, per_edit=False):
    ...
    for amend in soup.find_all("amend"):
        ...
        edits = list(get_edits(amend))
        assert len(edits) >= 1, "No edits found"
        if per_edit:
            for i, (text_original, text_amended, edit_type, edit_indices) in enumerate(edits):
            ...
            md['text_original'] = edits[0]['text_original']
            md['text_amended'] = edits[0]['text_amended']
            md['edits'] = [{'edit_id': e['edit_id'], 'edit_type': e['edit_type'], 'edit_indices': e['edit_indices']}
                           for e in edits]
