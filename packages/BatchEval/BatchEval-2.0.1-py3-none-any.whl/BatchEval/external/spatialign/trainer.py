...
from BatchEval.external.module import contrast_loss, trivial_entropy, cross_instance_loss
from BatchEval.external.module.losses import CrossInstLoss, ContrastLoss, ScaleMseLoss
from BatchEval.external.spatialign import DGIAlignment
from BatchEval.external.utils import Dataset, get_format_time, get_running_time, EarlyStopping
...
class Spatialign:
    ...
    def init_bank(self):
        ...
        for idx, data in enumerate(self.dataset.loader_list):
            ...
            header_bank[idx] = latent_x.detach().cpu().numpy()
            del data
            del recon_x
        return header_bank
    ...
    def train(self,
        ...
        dev_ids = [0, 1, 2, 3, 4]
        cross_loss = CrossInstLoss()
        cross_loss = nn.DataParallel(cross_loss, device_ids=dev_ids)
        contr_loss = ContrastLoss()
        contr_loss = nn.DataParallel(contr_loss, device_ids=dev_ids)
        scale_loss = ScaleMseLoss()
        # scale_loss = nn.DataParallel(scale_loss, device_ids=dev_ids)
            ...
            for idx, data in enumerate(self.dataset.loader_list):
                ...
                with torch.cuda.amp.autocast():
                    graph_loss, dgi_loss, recon_loss, latent_x, recon_x = self.model(
                        x=data.x, edge_index=data.edge_index, edge_weight=data.edge_weight, domain_idx=data.domain_idx,
                        neigh_mask=data.neigh_graph)
                    recon_loss = scale_loss(data.x, recon_x).to(self.device)
                    # update memory bank
                    self.update_bank(idx, latent_x.detach().cpu().numpy(), alpha=alpha)

                    t_device = torch.device(f"cuda:{dev_ids[idx % len(dev_ids)]}")

                    header = torch.tensor(self.header_bank[idx], device=t_device)
                    intra_inst = contr_loss(feat1=latent_x, feat2=header, tau=0.05, weight=1.).mean().to(self.device)

                    intra_clst = contr_loss(feat1=latent_x.T, feat2=header.T, tau=0.05, weight=1.).mean().to(self.device)
                    # 0.2
                    entropy_clst = trivial_entropy(feat=latent_x, tau=0.01, weight=1.)
                    ...
                    for i in np.delete(range(len(self.dataset.data_list)), idx):
                        t_device = torch.device(f"cuda:{dev_ids[idx % len(dev_ids)]}")
                        header = torch.tensor(self.header_bank[i], device=torch.device(t_device))
                        inter_loss = cross_loss(feat1=latent_x, feat2=header, tau=0.1, weight=1.).to(self.device)
