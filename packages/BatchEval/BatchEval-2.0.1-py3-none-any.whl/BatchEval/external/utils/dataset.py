...
from BatchEval.external.utils import get_format_time


class Dataset:
    """
    Multiple dataset loading, preprocessing and convert to dataloader
    :param merge_data: AnnData
    :param batch_key: str
    ...
    def __init__(self,
        ...
        self.data_list = [merge_data[merge_data.obs[batch_key] == c] for c in merge_data.obs[batch_key].cat.categories]

        self.batch_key = batch_key
        self.loader_list = self.get_loader(merge_data,
                                           # min_genes=min_genes, min_cells=min_cells,
                                           is_reduce=is_reduce, is_hvg=is_hvg, n_pcs=n_pcs, n_hvg=n_hvg,
                                           n_neigh=n_neigh, is_undirected=is_undirected)
        self.n_domain = merge_data.obs[batch_key].cat.categories.size
        self.n_node = merge_data.shape[0]

        self.inner_dims = n_pcs if is_reduce else merge_data.shape[1]
        self.inner_genes = merge_data.var_names.tolist()
