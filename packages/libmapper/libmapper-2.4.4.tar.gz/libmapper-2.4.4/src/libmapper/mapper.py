__all__ = [
    ...
    '__version__', 'has_numpy', 'Device', 'Location', 'Direction', 'Operator', 'Property',
...
__version__ = '2.4.3.15+gc4f090ce'
    ...
    def set_value(self, value):
        ...
        if isinstance(value, list):
            if any(not (isinstance(x, int) or isinstance(x, float)) for x in value):
                print("mpr.Signal.set_value() accepts only scalars or lists of type float and int")
                return self
            _len = len(value)
...
class Device(Object):
    ...
    def add_signal(self, dir, name, length=1, datatype=Type.FLOAT, unit=None, min=None, max=None,
        ...
        if num_inst is not None and isinstance(num_inst, int):
            signal.reserve_instances(num_inst - 1)
