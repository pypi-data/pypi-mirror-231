...
class AppOfApps(AbstractApplication):
  ...
  def generate_resources(self) -> str:
    ...
    for (app_name, env_name, project, destination_namespace) in self._find_deploying_apps(self.app_name, self.env_name):
      template_vars = merge({}, self.config.get_vars(), self.config.get_env_vars(env_name), self.config.get_app_vars(env_name, app_name), {
        '__application': {
  ...
  def generate_resources(self) -> str:
    ...
    for yml_child in yml_children:
      ...
      if yml_child.element_rel_path.endswith('.j2'):
        template_vars = merge({}, self.config.get_vars(), self.config.get_env_vars(self.env_name), self.config.get_app_vars(self.env_name, self.app_name))
