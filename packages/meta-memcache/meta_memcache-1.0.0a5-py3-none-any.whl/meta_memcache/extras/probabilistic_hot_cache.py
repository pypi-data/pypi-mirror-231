...
from meta_memcache.metrics.base_metrics_collector import BaseMetricsCollector
...
class ProbabilisticHotCache(ClientWrapper):
    ...
    def _lookup_hot_cache(
        ...
        if found := self._store.get(key.key):
            ...
            value = None
        if self._metrics:
            next(self._metrics.counters["hits" if is_found else "misses"])
    ...
    def _store_in_hot_cache_if_necessary(
        ...
        if self._metrics:
            self._metrics.gauges["item_count"] = len(self._store)
    ...
    def get(
        ...
        if self._allowed_prefixes and not self._allowed_prefixes.prefixes(key.key):
            if self._metrics:
                next(self._metrics.counters["skips"])
            return super().get(
