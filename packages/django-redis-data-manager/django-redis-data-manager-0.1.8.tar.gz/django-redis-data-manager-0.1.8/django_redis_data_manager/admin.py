...
class RedisInstanceAdmin(DjangoObjectToolbarAdmin, admin.ModelAdmin):
    ...
    def data_manager_link(self, obj):
        ...
        return reverse(
            "admin:{}_{}_data_manager".format(app_label, model_name),
            kwargs={"instance_id": obj.pk},
        )
    ...
    def get_urls(self):
        ...
        return [
            path(
                "data-manager-api/<int:instance_id>/delete",
                self.admin_site.admin_view(self.data_manager_delete),
                name="{0}_{1}_delete".format(app_label, model_name),
            ),
            path(
                "data-manager-api/<int:instance_id>/flushdb",
                self.admin_site.admin_view(self.data_manager_flushdb),
                name="{0}_{1}_flushdb".format(app_label, model_name),
            ),
            path(
                "data-manager-api/<int:instance_id>/getDetail",
                self.admin_site.admin_view(self.data_manager_getDetail),
                name="{0}_{1}_getDetail".format(app_label, model_name),
            ),
            path(
                "data-manager-api/<int:instance_id>/getKeys",
                self.admin_site.admin_view(self.data_manager_getKeys),
                name="{0}_{1}_getKeys".format(app_label, model_name),
            ),
            path(
                "data-manager/<int:instance_id>/",
                self.admin_site.admin_view(self.data_manager_view),
                name="{0}_{1}_data_manager".format(app_label, model_name),
            ),
        ] + super().get_urls()
    ...
    def data_manager_view(self, request, instance_id):
        ...
        instance = self.model.objects.get(pk=instance_id)
        config = {
            "data-api-url": reverse(
                "admin:{}_{}_getKeys".format(app_label, model_name),
                kwargs={"instance_id": instance_id},
            ),
            "detail-api-url": reverse(
                "admin:{}_{}_getDetail".format(app_label, model_name),
                kwargs={"instance_id": instance_id},
            ),
            "flushdb-api-url": reverse(
                "admin:{}_{}_flushdb".format(app_label, model_name),
                kwargs={"instance_id": instance_id},
            ),
            "delete-api-url": reverse(
        ...
        return render(
            request,
            "django-redis-data-manager/data-manager.html",
            {
    ...
    def data_manager_get_key_detail_table(self, request, db, key):
        ...
        if value_type == "string":
            ...
            try:
                value = json.dumps(
                    json.loads(value), ensure_ascii=False, indent=4, sort_keys=True
                )
            except:
                pass
        ...
        html = (
            render_to_string(
                "django-redis-data-manager/key-detail-table-{0}.html".format(
                    value_type
                ),
                {
                    "key": key,
    ...
    def data_manager_get_namespace_detail_table(self, request, db, key):
        ...
        html = (
            render_to_string(
                "django-redis-data-manager/namespace-detail-table.html",
                {"key": key, "count": len(keys)},
            )
            .strip()
            .replace("\n", "")
        )
