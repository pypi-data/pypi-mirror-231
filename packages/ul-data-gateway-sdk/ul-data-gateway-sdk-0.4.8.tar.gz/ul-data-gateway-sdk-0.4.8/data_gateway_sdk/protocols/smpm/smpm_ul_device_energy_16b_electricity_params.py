...
from data_aggregator_sdk.constants.enums import SensorType, ResourceKind
...
class SmpmUlDeviceEnergy16BElectricityParamsData(Packet):
    ...
    def to_integration_data(self, received_at: datetime, device_tz: tzinfo, **kwargs: Any) -> List[IntegrationV0MessageData]:
        sensors = []
        meter_resource_kind = ResourceKind.PHASE_A_ACTIVE_CONSUMED if self.current_ch_2 and self.current_ch_3 else ResourceKind.COMMON_ACTIVE_CONSUMED

        sensors.append(
            IntegrationV0MessageSensor(
                channel_id=1,
                meter_resource_kind=meter_resource_kind,
        ...
        if self.current_ch_2 and self.current_ch_3:
            sensors.extend(
                [
                    IntegrationV0MessageSensor(
                        channel_id=1,
                        meter_resource_kind=ResourceKind.PHASE_B_ACTIVE_CONSUMED,
                        sensor_type=SensorType.ENERGY_METER_CURRENT,
                        value=Decimal(str(self.current_ch_2)),
                    ),
                    IntegrationV0MessageSensor(
                        channel_id=1,
                        meter_resource_kind=ResourceKind.PHASE_C_ACTIVE_CONSUMED,
                        sensor_type=SensorType.ENERGY_METER_CURRENT,
