...
def run_anchor_heatmap(args):
    anchor_heatmap(PKResults(args.index), args.anchors, args.features,
...
def run_dryrun(args):
    metadata_dict = dryrun(genomes_input=args.genomes,
                           genomes_input_paired=args.genomes_paired,
                           split_memory=args.rounds, threads=args.threads)
...
def main():
    ...
    index_parser.add_argument(
        '--rounds', metavar='<int>', type=int, default=1,
    ...
    anchor_hm_parser.add_argument(
        '-a', '--anchors', metavar='<anchors.fa.gz>', type=str, action='store',
        required=True, nargs='+', help='anchors')
    anchor_hm_parser.add_argument(
    ...
    download_parser.add_argument(
        '-c', '--clade', choices=('Spolyrhiza', 'Solanum', 'Zea', 'Hsapiens', 'Bsubtilis', 'Athaliana'),
        help=('download publicly available genomes. Clade: max_samples. '
