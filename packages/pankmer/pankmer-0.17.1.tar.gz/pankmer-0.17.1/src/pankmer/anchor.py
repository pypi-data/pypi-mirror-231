...
def anchor_region(
    *pk_results,
    ...
    anchor_dicts = (
        result.get_collapsed_regional_scores(ref, {contig: [[start, end]]})
        for result in pk_results
    )
    if output_file:
        with (
...
def get_chromosome_sizes_from_ref(reference: str):
    ...
    """
    return pd.DataFrame(
        ((k, len(v)) for k, v in Fasta(reference).items()), columns=("name", "size")
    )
...
def anchor_dicts_tuple(*pk_results, ref, chromosomes):
    return tuple(
        result.get_collapsed_regional_scores(ref, {c: [] for c in chromosomes})
        for result in pk_results
    )
...
def anchor_genome(
    *pk_results,
    ...
    for result in pk_results:
        result.threads = processes
    anchor_dicts = anchor_dicts_tuple(*pk_results, ref=ref, chromosomes=chromosomes)
...
def anchor_heatmap(
    ...
    results = tuple(
        pk_results.get_collapsed_regional_scores(
