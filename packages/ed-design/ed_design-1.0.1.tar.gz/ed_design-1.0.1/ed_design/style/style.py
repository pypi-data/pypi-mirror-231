...
def style(style='envidan', prop_cycle_palette='normal'):
    """
    ...
    if style == 'envidan':
        try:
            file_path = pkg_resources.resource_filename(__name__, '/envidan.mplstyle')
            plt.style.use(file_path)
            prop_cycle(prop_cycle_palette)
        except FileNotFoundError:
            print("The 'envidan.mplstyle' file was not found. Check your package structure.")
...
def prop_cycle(palette='normal'):
    ...
    """
    colors = edd.Colors()
    prop_cycle_palette = colors.get_palette(palette)
...
def font_check(font_name=None):
    ...
    if font_name is None:
        ...
        if len(satoshi_lst) == 0:
            ...
            print(f'The fonts {satoshi_lst}" are not present on your system. Please install them from the path ed_design/style/_fonts/_satoshi')
