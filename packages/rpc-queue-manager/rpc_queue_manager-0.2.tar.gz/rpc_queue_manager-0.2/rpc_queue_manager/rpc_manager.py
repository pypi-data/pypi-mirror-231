...
class RPCQueueConsumer:
    def __init__(self, host='localhost', callback=None, **kwargs):
        ...
        self.queue_name =   kwargs.get('queue', 'rpc_queue')
    ...
    def start_consuming(self):
        ...
        try:
            ...
            print(f"RPC Consumer Start {self.queue_name}........")
...
class RPCQueueProducer:
    ...
    def set_queue(self, data):
        ...
        """
        try:
            # Ensure that 'queue_name' is present in the data dictionary
            if 'queue' not in data:
                raise ValueError("The 'queue_name' key is missing in the data dictionary.")
            ...
            # Get the target queue name from the data dictionary
            target_queue_name = data['queue']
            print(target_queue_name)

            # Publish the request message to the specified queue with reply_to and correlation_id properties
            self.channel.basic_publish(
                exchange='',
                routing_key=target_queue_name,  # Use the specified queue name
                properties=pika.BasicProperties(
                    reply_to=self.callback_queue,
                    correlation_id=self.corr_id,
                ),
                body=str(data_to_send)
            )
