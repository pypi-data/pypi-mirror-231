...
def mergeSimulationsSet(simulationsSet,
    ...
    if combineNeighborLists:
        ...
        if len(neighborLists) > 1:
            #Iterate over pairs of neighbor lists
            pass

        if len(neighborLists) > 1:
            remaining = ""
            for i in neighborLists:
                remaining += f"\"{i}\" ({aggregatedSimulation['topology']['forceField'][i]['type'][1]}), "
            remaining = remaining[:-2]+"."
            logger.warning(f"[pyUAMMD] Resulsting simulation will have multiple neighbor lists !!!. Remaining neighbor lists: {remaining}")
