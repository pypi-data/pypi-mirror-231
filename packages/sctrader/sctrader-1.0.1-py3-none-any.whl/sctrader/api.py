...
from sctrader.joinquant_follower import JoinQuantFollower
from sctrader.log import logger
from sctrader.ricequant_follower import RiceQuantFollower
from sctrader.xq_follower import XueQiuFollower
from sctrader.xqtrader import XueQiuTrader
...
def use(broker, debug=False, **kwargs):
    ...
    if broker.lower() in ["wk_client", "五矿客户端"]:
        from sctrader.wk_clienttrader import WKClientTrader

        return WKClientTrader()

    if broker.lower() in ["htzq_client", "海通证券客户端"]:
        from sctrader.htzq_clienttrader import HTZQClientTrader
    ...
    if broker.lower() in ["universal_client", "通用同花顺客户端"]:
        from sctrader.universal_clienttrader import UniversalClientTrader
