...
class AMPL {
  ...
  */
   ...
   */
  std::string snapshot(bool model = true, bool data = true, bool options = true,
                       fmt::CStringRef fileName = "") const {
    return internal::getStringFromDLL(internal::AMPL_Impl_snapshot(
        ampl_, model, data, options, fileName.c_str(), internal::ErrorInfo()));
  }
  /**
