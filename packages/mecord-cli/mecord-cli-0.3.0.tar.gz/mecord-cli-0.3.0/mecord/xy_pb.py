...
def _try_network():
    try:
        for url in ["https://www.baidu.com/"]:
            s = requests.session()
            s.keep_alive = False
    ...
    try:
        if os.path.exists(TASK_NOTIFY_DATA):
            ...
            for it in data:
                ...
                if "country" in it:
                    counrty = it["country"]
                if TaskNotify(counrty, it["taskUUID"], it["status"], f"{msg} *", it["dataStr"], True) == False:
                    newData.append(it)
            with open(TASK_NOTIFY_DATA, 'w') as f:
                json.dump(newData, f)
    finally:
...
def TaskNotify(country, taskUUID, status, msg, dataStr, isRetry=False):
    ...
    if r1 != 0:
        ...
        return False
    rsp.ParseFromString(r3)
    print(f"rece server ok={rsp.ok} notify_id = {rsp.notify_id}")
    return rsp.ok == True
