...
def cli(ctx: click.Context, credentials_filepath: str):
    ...
    if (credentials_filepath is None):
        credentials_filepath = env_with_fallback("HASHBOARD_CREDENTIALS_FILEPATH", "GLEAN_CREDENTIALS_FILEPATH", DEFAULT_CREDENTIALS_FILEPATH)
...
@click.argument(
    "filepaths", nargs=-1, required=True, type=click.Path(exists=True, dir_okay=False, allow_dash=True)
)
@click.pass_context
...
def pull(
    ...
    click.echo(f"🔎 Found {len(local_by_path)} Hashboard resources in working directory.")
