...
gfs = gfs.filter(ee.Filter.eq('creation_time',most_recent_forecast))\
         .filter(ee.Filter.inList('forecast_hours',which_hours))
...
def getSpeedDirection(gfs):
   ...
   return gfs.addBands(ee.Image.cat([speed,direction]).rename(['Speed','Direction']))\
         .copyProperties(gfs).set('system:time_start',gfs.get('forecast_time'))
...
Map.addTimeLapse(gfs.select(['temperature_2m_above_ground']),{'min':-20,'max':40,'palette':cmOceanThermal,'dateFormat':'YYMMddHH','advanceInterval':'hour','legendLabelLeftAfter':'C','legendLabelRightAfter':'C'},'Temperature')
Map.addTimeLapse(gfs.select(['precipitable_water_entire_atmosphere']),{'min':0,'max':30,'palette':cmOceanTempo,'dateFormat':'YYMMddHH','advanceInterval':'hour','legendLabelLeftAfter':'kg/m^2','legendLabelRightAfter':'kg/m^2'},'Precipitable Water')


Map.addTimeLapse(gfs.select(['Speed']),{'min':0,'max':60,'palette':cmOceanSpeed,'dateFormat':'YYMMddHH','advanceInterval':'hour','legendLabelLeftAfter':'km/h','legendLabelRightAfter':'km/h'},'Wind Speed')
Map.addTimeLapse(gfs.select(['Direction']),{'palette':windDirectionPalette,'min':0,'max':360,'dateFormat':'YYMMddHH','advanceInterval':'hour','legendLabelLeftAfter':'degrees','legendLabelRightAfter':'degrees'},'Wind Direction')
