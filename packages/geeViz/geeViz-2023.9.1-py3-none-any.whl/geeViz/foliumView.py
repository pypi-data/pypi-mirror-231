...
def add_ee_layer(self, ee_object, vis_params, name,visible):
    try:    
        ...
        elif  isinstance(ee_object, ee.featurecollection.FeatureCollection) or \
        isinstance(ee_object,ee.feature.Feature) or \
        isinstance(ee_object,ee.geometry.Geometry):
            strokeWidth = 2
            strokeColor = '000'
            if 'strokeWidth' in vis_params.keys():strokeWidth =  vis_params['strokeWidth']
            if 'strokeColor' in vis_params.keys():strokeColor =  vis_params['strokeColor']
...
class foliumMapper:
    ...
    def setMapArg(self,key,value):self.mapArgs[key]=value
    ...
    def centerObject(self,ee_object):
        if isinstance(ee_object, ee.featurecollection.FeatureCollection) or \
        ...
        self.mapBounds = [[numpy.min(ys),numpy.min(xs)],[numpy.max(ys),numpy.max(xs)]]
