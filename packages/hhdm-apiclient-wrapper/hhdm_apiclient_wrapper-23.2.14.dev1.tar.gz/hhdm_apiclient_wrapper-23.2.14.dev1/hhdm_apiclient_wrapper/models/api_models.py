...
class ApiGetResult(Generic[T]):
    def __init__(self, return_value: T, success: bool, message: str, status_code: HTTPStatus | None = None) -> None:
...
class UpdateModel(IApiModel):
    def __init__(self, last_modified_time_max_allowed_value: str, parameter_updates: List[ParameterUpdateModel], collection_update_models: Dict[str, List] = {}) -> None:
    ...
    def serialize(self) -> Dict:
        return {
            ...
            'ParameterUpdates': list([param.serialize() for param in self.parameter_updates]),
            'CollectionUpdateModels': {k: v.serialize() for k, v in self.collection_update_models.items()}
        }
