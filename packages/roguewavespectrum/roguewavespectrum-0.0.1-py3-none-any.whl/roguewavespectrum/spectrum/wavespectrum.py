...
from typing import TypeVar, List, Mapping, Hashable, Iterator
from xarray import (
    Dataset,
    DataArray,
    concat,
...
from roguewavespectrum.time import nat
...
class WaveSpectrum(ABC):
    ...
    ** Implementation Considerations **
    ...
    def __init__(self, dataset: Dataset, physics_options: PhysicsOptions = None, **kwargs):
        ...
        for required_variable in [NAME_F, NAME_E]:
            if required_variable not in dataset and required_variable not in dataset.coords:
                raise ValueError(
                    f"Required variable/coordinate {NAME_E} is"
    ...
    def __deepcopy__(self: _T, memodict) -> _T:
        cls = self.__class__
        return cls(self.copy(deep=True))
    ...
    def frequency_moment(self, power: int, fmin: float = 0.0, fmax: float = np.inf) -> DataArray:
        ...
        return _integrate(self.e * self.frequency ** power, NAME_F, fmin, fmax)
    ...
    def angular_frequency(self) -> DataArray:
        ...
        data_array.name = "angular_frequency"
        return data_array

    @property
    def latitude(self) -> DataArray:
        """
        Latitudinal locations of the variance density spectra, The latitudes are given as decimal degrees north.

        :return: latitudes decimal degrees north.
        """
    ...
    def longitude(self) -> DataArray:
        """
        Longitudinal locations of the variance density spectra, The longitudes are given as decimal degrees east.

        :return: longitudes decimal degrees east.
        """
    ...
    def wave_speed(self) -> DataArray:
        ...
        return DataArray(wavespeed, dims=wavenumber.dims, coords=wavenumber.coords)
    ...
    def wavenumber_spectral_density(self) -> DataArray:
        """
        Spectral density as a function of wave number. Conversion through multiplication with the Jacobian of the
        ...
        :return: Energy spectral density as function of wavenumber.
        """
        return self.variance_density * self.groupspeed / (np.pi * 2)

    @property
    def saturation_spectral_values(self) -> DataArray:
        """
        Saturation spectrum as introduced by Phillips.
        :return:
        """
        return self.wavenumber_spectral_density * self.wavenumber ** 3

    @property
    def slope_spectral_values(self) -> DataArray:
    ...
    def mean_squared_slope(self, fmin: float = 0.0, fmax: float = np.inf) -> DataArray:
        ...
        :return:
        """
        return (_integrate(self.slope_spectral_values, NAME_F, fmin, fmax))
    ...
    def peak_direction(
            ...
            fmin: float = 0.0,
            fmax: float = np.inf,
            directional_unit: DirectionalUnit = 'degrees',
            directional_convention: DirectionalConvention = 'mathematical'
    ) -> DataArray:
        """
        Direction of the peak of the spectrum. The direction is expressed in degrees anti-clockwise from east and
        ...
        :param fmin: minimum frequency, inclusive
        :param fmax: maximum frequency, inclusive
        :param directional_unit: units of the output angle, one of 'degrees' or 'radians'
    ...
    def peak_directional_spread(
        ...
        """
        Directional spread of the peak of the spectrum. The spread is expressed in degrees and calculated in terms of
        ...
        return wave_directional_spread(a1, b1, unit=directional_unit)
    ...
    def mean_direction_per_frequency(
        ...
        """
        Mean direction of each frequency bin. The direction is expressed in degrees anti-clockwise from east and
        represents the direction the waves travel *to*.
        ...
        :return: direction
        """
        return wave_mean_direction(self.a1, self.b1, directional_unit, directional_convention)


    def mean_spread_per_frequency(self, directional_unit: DirectionalUnit = 'degrees') -> DataArray:
        """
        Directional spread of each frequency bin. The spread is expressed in degrees and calculated in terms of
        the definition by Kuik et al. (1988), their equation 34.
        ...
        :return: directional spread
        """
        return wave_directional_spread(self.a1, self.b1, directional_unit)

    def _spectral_weighted(self, property: DataArray, fmin: float = 0.0, fmax: float = np.inf):
        """
        Calculate the spectral weighted mean property.
        :param property: spectral property to calculate weighted mean for
        :param fmin: minimum frequency, inclusive
        :param fmax: maximum frequency, inclusive
        :return: spectral weighted mean property
        """
        return _integrate(property * self.e, NAME_F, fmin, fmax) / self.m0(fmin, fmax)
    ...
    def mean_direction(
        ...
        """
        Mean direction the spectrum. Per default the direction is expressed in degrees anti-clockwise from east and
        represents the direction the waves travel *to*.
        ...
        return wave_mean_direction(
            self.mean_a1(fmin, fmax), self.mean_b1(fmin, fmax), directional_unit, directional_convention)
    ...
    def mean_directional_spread(
        ...
        return wave_directional_spread(
            self.mean_a1(fmin, fmax), self.mean_b1(fmin, fmax), directional_unit)
    ...
    def depth(self) -> DataArray:
        ...
        if NAME_DEPTH in self.dataset:
            ...
            return where(depth.isnull(), np.inf, depth)

        else:
            if len(self.dims_space_time) == 0:
                return DataArray(np.inf)
    ...
    def bandpass(self: _T, fmin: float = 0., fmax: float = np.inf, interpolate_to_limits: bool = False) -> _T:
        """
