...
def met(event):
    if not event.args:
        ...
        for obj in find('user'):
            ...
            reply(event, f'{nmr} {obj.user} {obj.perms} {lap}s')
            nmr += 1
        if not nmr:
            reply(event, 'no user')
        return
...
def mre(event):
    if not event.channel:
        reply('channel is not set.')
        return
    bot = byorig(event.orig)
    if 'cache' not in dir(bot):
        reply('bot is missing cache')
        return
    if event.channel not in bot.cache:
        reply(f'no output in {event.channel} cache.')
        return
    for _x in range(3):
    ...
    reply(f'{size} more in cache')


def pwd(event):
    if len(event.args) != 2:
        reply('pwd <nick> <password>')
        return
    arg1 = event.args[0]
    arg2 = event.args[1]
