...
class TestSWMLAIUpdated(unittest.TestCase):
    ...
    def test_ai_method(self):
        ...
        data_map_expression = AI.SWAIGFunction.DataMap.Expressions(
            string="It's going to rain",
            pattern="rain",
            output_response="Bring an umbrella.",
            output_action=["notify_user"]
        )

        data_map_webhook = AI.SWAIGFunction.DataMap.Webhooks(
            url="https://api.weather.com/rain_check",
            headers={"Authorization": "Bearer XYZ"},
            method="GET",
            output_response="It's going to rain in {location}.",
            output_action=["notify_user"]
        )

        data_map = AI.SWAIGFunction.DataMap(expressions=[data_map_expression], webhooks=[data_map_webhook])
