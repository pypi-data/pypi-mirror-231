...
class temporal_slot(dictslot):  # temporal identifier
	...
	def __init__(self, t=STR_TIME_PRESENT):
		ldprint('--> ion.temporal_slot({0} ({1}))'.format(t, type(t)))
		super().__init__()
		if (t is None):
			self.time = None
		elif (isinstance(t, dict)):
			super().__init__(copy.deepcopy(t))
...
#-- ion semantics agents

class semantics_agent(core):
	_classname = 'ion.semantics_agent'

	def __init__(self, sa=None):
		if (sa is None):
			self._semantics = semantics()
			self._datastorage = data_storage()
		elif (isinstance(sa, ion.semantics_agent)):
	...
	@classmethod
	def getClassName(cls):
		return semantics_agent._classname
...
class semanticsagent(semantics_agent):
	pass

class sem_agent(semantics_agent):
	pass

class semagent(semantics_agent):
	pass

class s_agent(semantics_agent):
	pass

class sagent(semantics_agent):
	pass
