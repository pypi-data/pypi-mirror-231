...
def _remove_preposition(message):
	...
	if (prestr == _PACKET_PREPOSITION):
		...
		raise ValueError("ERROR: Illegal iON packet.")  # 例外を発生
	return message
		...
		try:
			...
			while True:
				try:
					...
					ldprint('[SVR] Received message: \'{0}\', Client address: {1}, Client port: {2}'.format(cli_message, cli_address, cli_port))
...
class database_agent(server):
	...
	def query_processing(self, cli_sock, cli_query):  # Function for Override
		...
		# (2-1) Found -> RESPONSE_SUCCEEDED、続けて検索したデータを返す
