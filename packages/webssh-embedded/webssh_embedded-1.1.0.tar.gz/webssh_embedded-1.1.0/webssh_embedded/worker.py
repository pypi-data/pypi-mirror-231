...
class Worker(object):
    ...
    def insert_command(self,command_execution_status,execution_time):
           try:
                data={"Command_ID":self.command_ids[0],"Command":self.input_command[0],"Command_Execution_Status":command_execution_status
                      ,"Execution_Time":execution_time,"Timestamp":self.entry_timestamp[0]}
                self.conn.post("http://localhost:5000/command",data=data)
        ...
        try:
            ...
            try:
                ...
                if(command_execution_status and execution_time and len(self.input_command)!=0):
                    ...
                    if(self.conn_status==1 and len(self.input_command)!=0):
                        self.insert_command(command_execution_status,execution_time)
