"""Defines :class:`CeDataWrangler`.

The class :class:`CeDataWrangler` is modified from
class :mod:`smol.cofe.wrangling.StructureWrangler`
to store the iteration information required by :mod:`WFacer`.
"""
...
class CeDataWrangler(StructureWrangler):
    ...
    Merely a data storage and validation structure, will not perform any operation
    to the data.

    .. note:: Not compatible with :mod:`smol` < 0.5.0.
    """
    ...
    def add_entry(
        ...
        Same as :class:`StructureWrangler` but refuses to insert symmetrically
        equivalent entries. It also records the iteration number when then entry
        was added.
