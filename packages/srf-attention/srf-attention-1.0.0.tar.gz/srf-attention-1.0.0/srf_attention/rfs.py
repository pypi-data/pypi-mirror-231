...
def orthogonal_matrix_chunk_hadamard(d, device = None):
    (d1,d2,d3)=(torch.diag(torch.sign(torch.randn(d)-0.5)),
                torch.diag(torch.sign(torch.randn(d)-0.5)),
                torch.diag(torch.sign(torch.randn(d)-0.5)))
...
def compute_simplex_dir(d, device = None):
    simp_dir = torch.diag(torch.ones(d))/math.sqrt(2) - torch.ones((d,d))*(1/((d-1) * math.sqrt(2))) *(1 + 1/math.sqrt(d)) #begin by getting the proj directions
    simp_dir[d-1,:] = 1/math.sqrt(2 * d) * torch.ones((d))
    simp_dir[:,d-1] = 0
    simp_dir = simp_dir / math.sqrt(torch.sum(simp_dir[1,:]**2))
    chi2 = torch.distributions.chi2.Chi2(torch.ones(d))
    rand_sim = torch.matmul(torch.diag(torch.sqrt(chi2.sample())),simp_dir)
