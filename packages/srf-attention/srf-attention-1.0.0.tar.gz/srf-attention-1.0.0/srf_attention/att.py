...
class Attention(torch.nn.Module):
    def __init__(self, d, causal, n_features = None, scale = False, normalize = False, device = device):
        super().__init__()
        self.causal = causal
    ...
    def redraw_(self, device = None):
        projection = self.draw_features(device = device)
    ...
    def forward(self, q=None, k=None, v=None, mode = None, attn_fn = None):
        ...
        if mode == 'train':
            if attn_fn == 'torch':
                ...
                q = q.transpose(1, 2)
                k = k.transpose(1, 2)
                o = attn_no_softmax.flash_attn_func(q, k, v, softmax_scale=1.0, causal=self.causal)
                return o