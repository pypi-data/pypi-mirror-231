...
def _get_run_logs(run: Run, follow: bool, rank: Optional[int], failed: bool, resumption: Optional[int],
                  prev: bool) -> Tuple[Optional[str], int]:
    if prev:
    ...
    **kwargs,
    ...
    try:
        ...
        if isinstance(submissions[0], Run):
            last_line, err = _get_run_logs(submissions[0], follow, rank, failed, resumption, prev)
            if err == 1:
                return 1
