...
class TestUtils(unittest.TestCase):
    def test_to_specs(self):
        ...
        expected = [egx.XSpec(egx.XType(egx.XType.FLOAT), [0.0, 25.0])]
        self.assertEqual(len(expected), len(actual))
        self.assertEqual(expected[0].xtype.id(), actual[0].xtype.id())
    ...
    def test_mixint_lhs(self):
        xspecs = [
            egx.XSpec(egx.XType(egx.XType.FLOAT), [0.0, 25.0]),
            egx.XSpec(egx.XType(egx.XType.INT), [0, 25]),
        ]
        doe = egx.lhs(xspecs, 10)
