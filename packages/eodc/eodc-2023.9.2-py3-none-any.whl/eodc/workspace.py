...
class MinIOAdapter(WorkspaceAdapter):
    ...
    def __init__(
        self, url: str, access_key: SecretStr, secret_key: SecretStr, mc_bin_path: Any
    ):
        self.minio_client = Minio(
            url, access_key=access_key, secret_key=secret_key, secure=True
        )

        self.minio_admin_client = MinioAdmin(self.ALIAS, binary_path=mc_bin_path)
        sp.run(
            f"mc alias set {self.ALIAS} {self.url} \
