...
from procustodibus_broker.cnf import load_cnf

API_TIMEOUT = 16  # seconds


def setup_api(cnf):
    """Generates and saves new credential for configured broker.
    ...
    url = f"{cnf.api}/users/{cnf.broker}/credentials/signature"
    headers = {
        "authorization": f"X-Custos user=^{cnf.broker} broker_setup=^{code}",
...
def poll_api(cnf, event_type, max_events=10):
    """Polls API for next N events.

    Arguments:
        cnf (Config): Config object.
        event_type (str): Event type (eg 'alerts').
        max_events (int): Max events to pull (default 10).
    ...
    request = Request(
        "POST",
        f"{cnf.api}/queues/{event_type}/next",
...
def get_health_info(cnf):
    ...
    response = requests.get(f"{cnf.api}/health", timeout=API_TIMEOUT)
    response.raise_for_status()
    return response.json()
...
def hello_api(cnf):
    ...
    request = Request("GET", f"{cnf.api}/queues/{version}/hello")
    response = send_with_session(cnf, request)
    return response.json()
...
def load_signing_key(cnf):
    """Loads the broker's signing key.

    Arguments:
        cnf (Config): Config object.

    Returns:
        SingingKey: Broker's signing key.
    ...
    try:
        if type(credentials) is not dict:
            credentials = load_cnf(credentials)["broker"]["credentials"]
...
def load_setup(cnf):
    ...
    try:
        if type(setup) is not dict:
            setup = load_cnf(setup)["broker"]["setup"]
        setup["code"]
    except KeyError:
...
def save_signing_key(cnf, signing_key):
    ...
    credentials = f"""
        # broker-credentials.conf generated {format_datetime()}
        # for broker {cnf.broker} on {getfqdn()}
        [Broker.Credentials]
...
def _raise_setup_issue(problem):
    raise ValueError(
        f"{problem}; see {DOCS_URL}/guide/integrations/brokers/#troubleshoot to fix"
    )
