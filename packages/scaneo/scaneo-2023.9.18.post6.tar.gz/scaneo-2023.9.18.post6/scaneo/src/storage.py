    ...
    def __init__(self):
        ...
        if url and access_key and secret_key and bucket:
            self.storage = CloudStorage(
                url, access_key, secret_key, bucket, os.getenv("REGION", "us-east-1")
            )
...
class CloudStorage:
    def __init__(self, url, access_key, secret_key, bucket, region):
        self.url = url
    ...
    def save(self, name, data):
        ...
        self.client.put_object(self.bucket, name, data_file, len(data_bytes))
        return self.get_url(name)
