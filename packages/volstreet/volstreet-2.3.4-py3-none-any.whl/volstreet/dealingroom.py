...
from volstreet import constants
...
def set_large_order_threshold(threshold: int):
    constants.LARGE_ORDER_THRESHOLD = threshold


def set_error_notification_settings(key, value):
    constants.ERROR_NOTIFICATION_SETTINGS[key] = value


def set_notifier_level(level: str):
    constants.NOTIFIER_LEVEL = level
...
def log_errors(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            user_prefix = constants.ERROR_NOTIFICATION_SETTINGS.get("user", "")
            logger.error(
                f"{user_prefix}Error in function {func.__name__}: {e}\nTraceback:{traceback.format_exc()}"
            )
            notifier(
                f"{user_prefix}Error in function {func.__name__}: {e}\nTraceback:{traceback.format_exc()}",
                constants.ERROR_NOTIFICATION_SETTINGS["url"],
                "ERROR",
            )
            raise e
...
def retry_angel_api(data_type=None, max_attempts=25, wait_increase_factor=1.2):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            for attempt in range(1, max_attempts + 1):
                ...
                try:
                    ...
                    if data_type == "ltp":
                        ...
                        return data["data"]
                except (DataException, Exception) as e:
...
class Index:
    ...
    def fetch_ltp(self, future=None):
        ...
        if self.name in ["FINNIFTY", "MIDCPNIFTY"]:  # Finnifty & Midcpnifty temp fix
            ltp = fetch_ltp("NFO", self.symbol, self.token)
    ...
    def splice_orders(self, quantity_in_lots):
        if quantity_in_lots > self.freeze_qty:
            ...
            if loops > constants.LARGE_ORDER_THRESHOLD:
                raise Exception(
    ...
    def overnight_straddle(
        ...
        def check_eligibility_for_overnight_straddle(effective_tte: float) -> bool:
            ...
            square_off_tte = effective_tte - 1
            ...
            if square_off_tte < 1:
                if is_next_day_expiry(effective_tte):
                    logger.info(
                        f"{self.name} current expiry is next day so the trade is eligible"
                    )
                    return True
        ...
        if not eligible_for_short:
            notifier(
                f"{self.name} No short straddle eligible for overnight straddle.",
                notification_url,
                "INFO",
            )
        ...
        if eligible_for_short:
            ...
            notifier(
                f"{self.name} Deploying overnight straddle with straddle {sell_straddle}. IV: {iv}, VIX: {vix}",
                notification_url,
                "INFO",
            )
        else:
            ...
            sell_straddle = None
            notifier(
                f"{self.name} No straddle eligible for overnight straddle. VIX: {vix}",
                notification_url,
                "INFO",
            )
        ...
        if buy_straddle is None and sell_straddle is None:
            ...
            notifier(
                f"{self.name} Exiting current position on straddle {buy_straddle}.",
                notification_url,
                "INFO",
            )
            ...
            notifier(
                f"{self.name} Entering new position on strike {sell_straddle}.",
                notification_url,
                "INFO",
            )
        ...
        def justify_stop_loss(info_dict, side):
            ...
            try:
                simulated_option_price = bs.simulate_price(
                    ...
                    flag=side,
                    original_price=info_dict.get(f"{side}_avg_price"),
...
def notifier(message, webhook_url=None, level="INFO"):
    ...
    if levels.index(level) < levels.index(constants.NOTIFIER_LEVEL):
        return
