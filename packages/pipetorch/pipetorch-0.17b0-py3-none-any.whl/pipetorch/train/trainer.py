    ...
    def device(self):
        try:
            ...
            if self._gpu is False:
                ...
                try:
                    device = self._get_first_gpu() or self._get_first_tpu() or torch.device('cpu')
                    if device.type == 'cpu':
...
class BatchManager:
    ...
    def assigned_gpus(self):
        return { t.device.index for t in self.all if t.device.type == 'cuda' }
    ...
    def assigned_devices(self):
        try:
            ...
            self._assigned_devices = { t.device for t in self.all if t.device.type == 'cuda' }
            return self._assigned_devices
