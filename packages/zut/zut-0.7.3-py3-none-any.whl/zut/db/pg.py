...
class PgBaseAdapter(DbAdapter[T_Connection, T_Cursor, T_Composable, T_Composed]):
    ...
    def copy_from_csv(self, file: IOBase|Path|str, table: str|tuple, columns: list[str] = None, delimiter: str = ';', quotechar: str = '"', nullchar: str = '', noheader: bool = False):
    ...
    BEGIN
        ...
        FOR sql_query IN
            SELECT
                format('ALTER %s %I.%I SET SCHEMA %I', routine_type, routine_schema, routine_name, new_schema)
    ...
    BEGIN
        ...
        FOR sql_query IN
            SELECT
                format('DROP %s IF EXISTS %I.%I CASCADE', routine_type, routine_schema, routine_name)
            FROM information_schema.routines
            WHERE routine_schema = old_schema
            -- postgis:
            AND routine_name NOT LIKE 'box%%'
...
class PgNoticeManager:
    ...
    def handler(self, diag: Diagnostic):
        return pg_notice_handler(self, diag, logger=self.logger)
