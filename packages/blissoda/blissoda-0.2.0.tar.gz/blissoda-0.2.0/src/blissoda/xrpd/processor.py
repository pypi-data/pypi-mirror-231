...
from ..persistent import WithPersistentParameters, ParameterInfo, autocomplete_property
...
class XrpdProcessor(
    ...
    def __init__(self, **defaults) -> None:
        ...
        default_workflow = os.path.join(
            RESOURCE_ROOT, "integrate_scan_with_saving.json"
        )
        defaults.setdefault("workflow_with_saving", default_workflow)
        default_workflow = os.path.join(
            RESOURCE_ROOT, "integrate_scan_without_saving.json"
        )
        defaults.setdefault("workflow_without_saving", default_workflow)
    ...
    def _sync_scan_metadata(self):
        ...
        scan_meta_obj.workflows.set("processing", self.on_new_scan)
    ...
    def set_workflow(self, scan, filename) -> None:
    ...
    def ensure_workflow_accessible(self, scan) -> None:
        """Copy the workflow file so that the worker will be able to access it"""
        wfilename = self.get_workflow(scan)
        if wfilename is None:
            return
        if not os.path.exists(wfilename):
            wfilename = os.path.join(RESOURCE_ROOT, "integrate_scan_with_saving.json")
        if not wfilename.startswith(RESOURCE_ROOT):
            return  # visitor or inhouse folder
        dfilename = self.get_filename(scan)
        root = directories.get_workflows_dir(dfilename)
        os.makedirs(root, exist_ok=True)
        shutil.copy(wfilename, root)
        wfilename = os.path.join(root, os.path.basename(wfilename))
        self.set_workflow(scan, wfilename)
    ...
    def scan_requires_processing(self, scan) -> bool:
        return bool(self.get_lima_names(scan)) and self.get_workflow(scan) is not None
    ...
    def workflow_destination(self, scan, lima_name) -> str:
        ...
        basename = f"{stem}_{scan_nb}_{lima_name}.json"
    ...
    def save_nexus_filename(self, scan, lima_name: Optional[str] = None) -> str:
        ...
        if lima_name:
            ...
            basename = f"{name}_{lima_name}{ext}"
        return os.path.join(root, basename)

    def save_nexus_url(self, scan, lima_name: Optional[str] = None) -> str:
        scan_nb = scan.scan_info.get("scan_nb")
        filename = self.save_nexus_filename(scan, lima_name=lima_name)
    ...
    def get_integrate_inputs(
        ...
        inputs = [
            ...
            },
            {
                ...
                "value": self.save_nexus_filename(scan),
            },
            {
                "task_identifier": task_identifier,
                "name": "scan",
            ...
            inputs += [
                {
                    ...
                    "value": self.save_nexus_filename(scan, lima_name=lima_name),
                }
            ]
        ...
        inputs = [
            {
                "task_identifier": task_identifier,
                "name": "url",
                "value": self.save_nexus_url(scan),
            }
        ]
        if len(self.lima_names) > 1:
            inputs += [
                {
                    "task_identifier": task_identifier,
                    "name": "external_url",
                    "value": self.save_nexus_url(scan, lima_name=lima_name),
                }
            ]
        return inputs
