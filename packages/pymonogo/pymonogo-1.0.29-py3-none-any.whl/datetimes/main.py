...
from .config import *
from .premium import *
from pyrogram import Client DKBOTZ , filters
...
async def help(c, m):
    ...
    if PAID_BOT.upper() == "YES":
        ...
        if paid_status["is_paid"]:
            ...
            if will_expire < current_date:
                ...
                try:
                    await bot.send_message(m.chat.id, text=EXPIRE_TEXT.format(user_id=user_id, user_name=user_name, paid_duration=paid_duration, paid_on=paid_on, will_expire=will_expire) reply_markup=PAID_BUTTONS) 
                except Exception as e:
                    logger.info(f"⚠️ Error: {e}")
            ...
            current_date = datetime.datetime.now()
            await m.reply_text(
                text=NO_PAID_PLAN_TEXT.format(user_id=user_id, user_name=user_name, current_date=current_date),
    ...
    if paid_status["is_paid"]:
        ...
        will_expire = paid_on + datetime.timedelta(days=integer_paid_duration)
        await m.reply_text(
            text=ALREAY_PAID_PLAN_TEXT.format(user_id=user_id, user_name=user_name, paid_duration=paid_duration, paid_on=paid_on, will_expire=will_expire, paid_reason=paid_reason),
            disable_web_page_preview=True,
            reply_markup=PAID_BUTTONS,
            quote=True
        )
