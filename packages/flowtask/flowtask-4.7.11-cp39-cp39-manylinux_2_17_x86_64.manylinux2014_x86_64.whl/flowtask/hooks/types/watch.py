...
class BaseWatcher:
    def __init__(self, *args, **kwargs):
        ...
        self.args = args
        self.kwargs = kwargs
        self._logger = logging.getLogger(name='Hook.Watchdog')
...
class BaseWatchdog(BaseHook):
    ...
    async def on_startup(self, app: WebApp) -> None:
        print(
            'CALLING SETUP for Watcher Trigger'
        )
        self.watcher.start()

    async def on_shutdown(self, app: WebApp) -> None:
        print(
            'CALLING STOP Watcher Trigger'
        )
        self.watcher.stop()
