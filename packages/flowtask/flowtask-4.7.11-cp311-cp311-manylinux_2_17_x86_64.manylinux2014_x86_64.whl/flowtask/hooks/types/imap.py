...
from ..interfaces import MaskInterface, CacheSupport
...
class ImapWatcher(BaseWatcher):
    ...
    def close_watcher(self):
        try:
            ...
            self._logger.error(
                f"Error closing IMAP Connection: {exc}"
            )

    def connect(self, max_retries=3, delay=5):
    ...
    def process_email(self, email_id, **kwargs):
        self._logger.debug(
            f"Email {email_id} has processed."
        )
    ...
    def run(self, **kwargs):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                self._logger.debug(
                    f"IMAP: Running Search: {search_criteria}"
                )
                    ...
                    for email_id in emails:
                        ...
                        if status == 'OK':
                            with CacheSupport(every=self._expiration) as cache:
                                ...
                                if 'has_attachments' in self.args and self.args['has_attachments'] is True:
                ...
                print(
                    f"An error occurred while checking the mailbox: {e}"
                )
...
class IMAPWatchdog(BaseWatchdog, MaskInterface):
