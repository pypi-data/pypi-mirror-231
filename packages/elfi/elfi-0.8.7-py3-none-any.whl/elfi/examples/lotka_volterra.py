...
def get_model(n_obs=50, true_params=None, observation_noise=False, seed_obs=None, **kwargs):
    ...
    priors = [
        ...
        elfi.Prior(ExpUniform, -6., 0., model=m, name='r2'),  # easily kills populations
    ...
    elfi.Constant(100, model=m, name='prey0')
    elfi.Constant(50, model=m, name='predator0')

    if observation_noise:
        priors.append(elfi.Prior(ExpUniform, np.log(0.5), np.log(50), model=m, name='sigma'))

    elfi.Simulator(sim_fn, *priors, m['prey0'], m['predator0'], observed=y_obs, name='LV')
