...
class scu():
    ...
    HTTP rest API.
    """
    def __init__(self, ip='localhost', port='8080', debug=False, lims_az=(-270.0, 270, 3.0), lims_el=(15, 90, 1.35), dish_type='mke', post_put_delay=0.0):
    ...
    def wait_settle(self, axis='all', timeout=600, query_delay=.25, tolerance=0.01, wait_by_pos=False, initial_delay=1.0):
        ...
        if axis == 'all':
            ...
            self.wait_settle('az', initial_delay=0.0, timeout=None)
            self.wait_settle('el', initial_delay=0.0, timeout=None)
            self.wait_settle('fi', initial_delay=0.0)
            self.wait_settle('az', initial_delay=0.0, wait_by_pos=True, tolerance=0.1)
            self.wait_settle('el', initial_delay=0.0, wait_by_pos=True, tolerance=0.1)
            return
