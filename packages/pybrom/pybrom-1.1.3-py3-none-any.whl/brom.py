...
from pydantic_core import Url as _Url
...
Url = Annotated[_Url, UrlConstraints(
    max_length=2083,
...
class ОбъектСсылка(Ссылка):
    ...
    def УникальныйИдентификатор(self) -> УникальныйИдентификатор:
...
class Сериализатор:
    ...
    def ВЗначениеБром(self, значение):
        ...
        # String
        elif issubclass(тип, str):
            return self.__factory.ValueString(Value=str(значение))
        # Number
        elif issubclass(тип, (int, float, Decimal)):
            return self.__factory.ValueNumber(Value=float(значение))
        # Boolean
        elif тип == bool:
    ...
    def ИзЗначенияБром(self, значение):
        ...
        if имяКласса == r'ValueKeyValue':
            return КлючИЗначение(ключ=self.ИзЗначенияБром(значение.Key),
                                 значение=self.ИзЗначенияБром(значение.Value))
    ...
    def __ИзЗначенияБром_ОбъектСсылка(self, значение):
        reference = self.__bromClient.Контекст().ПолучитьОбъектСсылку(значение.Type,
                                                                      УникальныйИдентификатор(значение.Value))
...
class ServerSideException(ConnectionError):
    class Model(pydantic.BaseModel):
        descr: StrictStr = None
        exception: "ServerSideException.Model" = pydantic.Field(
            default=None,
            validation_alias=pydantic.AliasChoices('exception', 'inner'))

        def decsr_list(self) -> list:
            txt = list()

            def descr(exc: "ServerSideException.Model", txt: list):
                if exc.descr:
                    txt.append(exc.descr)
                if isinstance(exc.exception, type(exc)):
                    descr(exc.exception, txt)

            descr(exc=self, txt=txt)
            return txt

        def decsr_str(self) -> str:
            return f'\n'.join(self.decsr_list())

    def __init__(self, descr:Model):
        self.add_note(descr.decsr_str())


class БромКлиент:
    def __init__(self, публикация="", пользователь="", пароль="", строкаПодключения="", jwt: str | None = None):
        ...
        self.__connectionSettings = НастройкиПодключения(публикация, пользователь, пароль, jwt)
        self.__metadata = МетаданныеКонфигурация(self)
        self.__dataContext = КонтекстДанных(self)

        session = Session()
        session.params[r'l'] = r'ru'
        session.params[r'vl'] = r'ru_RU'
        if jwt:
            session.params[r'AccessToken'] = jwt
        else:
            session.auth = HTTPBasicAuth(username=пользователь, password=пароль)
        ...
        try:
            ...
            try:
                exc.response.encoding = exc.response.apparent_encoding
                ss_exc = ServerSideException(descr=ServerSideException.Model.model_validate_json(exc.response.text))
                raise ss_exc from exc
            except:
                raise exc
    ...
    def Выполнить(self, код: str, параметр=None, /):
        paramArray = self.__serializer.ВЗначениеБром(параметр)
        value = self.__soapClient.service.ExecuteCode(код, paramArray)
        return self.__serializer.ИзЗначенияБром(value)
...
class Запрос:
    ...
    def УстановитьПараметр(self, имя: StrictStr, значение):
...
class НастройкиПодключения:
    def __init__(self, адресПубликации, имяПользователя, пароль, jwt: str | None = None):
        self.__publicationAddress = адресПубликации
        self.__userName = имяПользователя
        self.__password = пароль
        self.__jwt: str | None = jwt
...
class КлючИЗначение:
    __slots__ = r'__Ключ', r'__Значение'

    def __init__(self, ключ: StrictInt | StrictStr, значение: StrictStr | Структура | None = None):
        self.__Ключ = ключ
        self.__Значение = значение

    @property
    def Ключ(self) -> int | str:
        return self.__Ключ

    @property
    def Значение(self) -> StrictStr | Структура:
        return self.__Значение

    @Значение.setter
    def Значение(self, val: StrictInt | StrictStr | Структура) -> None:
        self.__Значение = val

    def __repr__(self) -> str:
        return str(self.__Ключ)

    def __eq__(self, other) -> bool:
        return self.__Ключ == other
