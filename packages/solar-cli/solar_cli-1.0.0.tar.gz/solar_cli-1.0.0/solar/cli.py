...
@click.option(
    ...
    show_default=True,
    default=False,
    help="Не спрашивать подтверждение операций",
)
@click.pass_context
def cli(ctx, query: str, collection: str, url: str, force: bool):
    """Solr CLI"""
    ...
    url_str = f"{url_parsed.scheme}://{url_parsed.hostname}"
    if url_parsed.port is not None:
        url_str += f":{url_parsed.port}"
