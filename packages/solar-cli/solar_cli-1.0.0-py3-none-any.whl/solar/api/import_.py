...
class Importer(ApiEngine):
    ...
    async def import_configs(
        ...
        params = dict(
            action="UPLOAD", name=name, overwrite=overwrite_str
        )
        ...
        if overwrite:
            collections = await self.config_collections(config_name=name)
            for c in collections:
                print(f"Reloading collection {c}")
                await self.reload_collection(c)
