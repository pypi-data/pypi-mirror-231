...
class ApiEngine:
        ...
        async with self.client.request(
            ...
            **kwargs,
            ...
            try:
                ...
                logger.error("Can't decode answer :( {e}")
                return None
    ...
    async def config_collections(self, config_name: str) -> List[str]:
        url = f"/api/schema-designer/info?configSet={config_name}&wt=json"
        resp = await self.api_request(path=url)
        if resp is None:
            raise ValueError(f"Error fetching {config_name=} collections :(")
