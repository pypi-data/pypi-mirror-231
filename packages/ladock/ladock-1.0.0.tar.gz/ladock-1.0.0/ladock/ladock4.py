...
def process_docking_ligand_native(counter, spacing, npts, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, parameter):
...
def process_and_optimize_smi(smi_file, ligand_tmp_dir):
    try:
...
def process_and_optimize_sdf(sdf_file, ligand_tmp_dir):
    try:
...
def process_in_ligand_tmp_dir(ligand_tmp_file, ligand_tmp_dir, max_workers):
    ...
    if smi_files:  # Periksa apakah ada file .smi yang tersedia
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for smi_file in smi_files:
                    smi_futures.append(executor.submit(process_and_optimize_smi, smi_file, ligand_tmp_dir))
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                for sdf_file in sdf_files:
                    sdf_futures.append(executor.submit(process_and_optimize_sdf, sdf_file, ligand_tmp_dir))
...
def process_ligand_link(ligand_url, ligand_dir, ligand_tmp_dir, lig_name, num_iterations, max_workers, spacing, npts, csv_path, prepare_receptor, receptor_name, prepare_ligand, directory, ensamble, output_model_dir, counter, parameter):
    retry_count = 0
    max_retries = 10
    ...
    for ligand_tmp_file in ligand_tmp_files:
        process_in_ligand_tmp_dir(ligand_tmp_file, ligand_tmp_dir, max_workers)        
        process_docking_ligand_test(counter, prepare_receptor, prepare_ligand, prepare_gpf, prepare_dpf,  lig_name, output_model_dir, receptor_name, ligand_tmp_dir, csv_path, parameter, ensamble, directory, max_workers)
...
def process_docking_ligand(directory, pdbqt_file, counter, prepare_receptor, receptor_name, prepare_gpf, lig_name, prepare_dpf, parameter, ensamble, output_model_dir, csv_path):
    try:
...
def process_docking_ligand_test(counter, prepare_receptor, prepare_ligand, prepare_gpf, prepare_dpf, lig_name, output_model_dir, receptor_name, ligand_tmp_dir, csv_path, parameter, ensamble, directory, max_workers):
    os.chdir(directory)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        for pdbqt_file in ligand_files:
            futures.append(executor.submit(
                process_docking_ligand, directory, pdbqt_file, counter, prepare_receptor, receptor_name, prepare_gpf, lig_name, prepare_dpf, parameter, ensamble, output_model_dir, csv_path))

        for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="  Docking & save energy"):
            pass  # Tidak perlu melakukan apa-apa, hanya menunggu tugas selesai
...
def main(): 
    ...
    config_file = 'config_ladock4.txt'
    ...
    for directory in glob.glob(os.path.join(current_directory, 'model*')):
        if os.path.isdir(directory):
            ...
            if os.path.exists(f"{receptor_name}.pdb") and os.path.exists(f"{lig_name}.pdb"):
                ...
                process_docking_ligand_native(counter, spacing, npts, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, parameter)
                        ...
                        for line in lines:
                            if line.strip():
                                if not line.startswith("#"):
                                    ...
                                    process_ligand_link(ligand_url, ligand_dir, ligand_tmp_dir, lig_name, num_iterations, max_workers, spacing, npts, csv_path, prepare_receptor, receptor_name, prepare_ligand, directory, ensamble, output_model_dir, counter, parameter)
