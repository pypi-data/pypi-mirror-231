        ...
        for ligand_new_pdb in ligand_new_pdbs:
            try:
                ...
                command = [
                    'gmx', 'genrestr',
                    '-f', ligand_new_pdb,
                    '-n', 'ligand_new-index.ndx',
                    '-o', 'ligand_new-posre.itp',
                    '-fc', '1000', '1000'
                ]
                
                # Jalankan perintah dengan subprocess
                subprocess.run(command, check=True)

                print(f"File {ligand_new_pdb}-posre.itp berhasil dibuat untuk {ligand_new_pdb}")
            except subprocess.CalledProcessError as e:
                print(f"Terjadi kesalahan saat menjalankan gmx genrestr: {str(e)}")
...
def main():
    ...
    spc216_gro = config_variables['spc216_gro']
