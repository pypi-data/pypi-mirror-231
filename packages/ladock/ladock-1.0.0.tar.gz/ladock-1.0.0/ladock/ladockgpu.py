...
def process_docking_ligand_native(counter, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, nrun):
...
def process_ligand_link(ligand_url, ligand_dir, ligand_tmp_dir, lig_name, num_iterations, max_workers, csv_path, prepare_receptor, receptor_name, prepare_ligand, directory, ensamble, output_model_dir, counter, nrun):
    retry_count = 0
    max_retries = 10
    ...
    for directory in glob.glob(os.path.join(current_directory, 'model*')):
        if os.path.isdir(directory):
            ...
            if os.path.exists(f"{receptor_name}.pdb") and os.path.exists(f"{lig_name}.pdb"):
                ...
                process_docking_ligand_native(counter, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, nrun)
                        ...
                        for line in lines:
                            if line.strip():
                                if not line.startswith("#"):
                                    ...
                                    process_ligand_link(ligand_url, ligand_dir, ligand_tmp_dir, lig_name, num_iterations, max_workers, csv_path, prepare_receptor, receptor_name, prepare_ligand, directory, ensamble, output_model_dir, counter, nrun)
