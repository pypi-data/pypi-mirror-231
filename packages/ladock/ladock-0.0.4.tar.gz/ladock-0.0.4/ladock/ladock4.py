...
def process_docking_ligand_native(prepare_gpf, prepare_dpf, counter, spacing, npts, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, parameter):
...
def process_ligand_link(prepare_gpf, prepare_dpf, ligand_url, ligand_dir, ligand_tmp_dir, lig_name, num_iterations, max_workers, spacing, npts, csv_path, prepare_receptor, receptor_name, prepare_ligand, directory, ensamble, output_model_dir, counter, parameter):
    retry_count = 0
    max_retries = 10
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        for pdbqt_file in ligand_files:
            futures.append(executor.submit(
                process_docking_ligand, directory, prepare_gpf, prepare_dpf, pdbqt_file, counter, prepare_receptor, receptor_name, prepare_gpf, lig_name, prepare_dpf, parameter, ensamble, output_model_dir, csv_path))
    ...
    for directory in glob.glob(os.path.join(current_directory, 'model*')):
        if os.path.isdir(directory):
            ...
            if os.path.exists(f"{receptor_name}.pdb") and os.path.exists(f"{lig_name}.pdb"):
                ...
                process_docking_ligand_native(prepare_gpf, prepare_dpf, counter, spacing, npts, max_workers, num_iterations, csv_path, prepare_receptor, receptor_name, prepare_ligand, lig_name, directory, ensamble, output_model_dir, parameter)
                        ...
                        for line in lines:
                            if line.strip():
                                if not line.startswith("#"):
                                    ...
                                    process_ligand_link(prepare_gpf, prepare_dpf, ligand_url, ligand_dir, ligand_tmp_dir, lig_name, num_iterations, max_workers, spacing, npts, csv_path, prepare_receptor, receptor_name, prepare_ligand, directory, ensamble, output_model_dir, counter, parameter)
