...
cdef class Serializer(object):
    ...
    def __cinit__(
        ...
        _dataclass_params: typing.Optional[_DataClassParams] = None,
    ):
        """
        ...
        cdef SerializerField serializer_field
        self._dataclass_params = _dataclass_params or _DataClassParams(dataclass)
        ...
        fields = []
        excluded_fields = []
        for dataclass_field in dataclasses.fields(self._dataclass):
            hints = dataclass_field.metadata.get(
                _metadata_name,
                FieldHints(dict_key=dataclass_field.name)
            )
            if hints.dict_key is None:
                hints.dict_key = dataclass_field.name
            ...
            if (
                hints.ignore
                or (only and dataclass_field.name not in only)
                or (exclude and dataclass_field.name in exclude)
            ):
                excluded_fields.append(serializer_field)
            else:
                fields.append((serializer_field, hints))
        ...
        for parent in self._parent_serializers:
            self._field_encoders.update(parent._field_encoders)
        for serializer_field, hints in fields:
            field_type = self._dataclass_params.resolve_type(
                type_hints[serializer_field.field_name]
            )
            encoder = self._get_encoder(field_type, hints)
            if encoder:
                self._field_encoders[field_type] = encoder
                serializer_field.encoder = encoder
            if hints.cast_on_load:
                field_casters.append(Caster(hints.dict_key, field_type))
        self._fields = tuple(serializer_field for serializer_field, _ in fields)
        ...
        builder = SchemaBuilder(
            ...
            strict=strict,
            _dataclass_params=self._dataclass_params,
        )
        ...
        self._frozen_dataclass = self._dataclass.__dataclass_params__.frozen
    ...
    cpdef inline dump(
        ...
        if many:
            for post_dump in self._post_dumpers:
                data = iter([post_dump(instance, d) for instance, d in zip(obj, data)])
        else:
            for post_dump in self._post_dumpers:
                data = post_dump(obj, data)
    ...
    def _get_encoder(self, field_type, hints):
        ...
        param_args = {
            ...
            for arg, param in zip(
                self._dataclass_params.arguments,
                self._dataclass_params.parameters
            )
        }
        try:
            params = _DataClassParams(field_type, param_args)
