...
class Types:
    ...
    new_type: StrType
    nested_new_type: SimpleWithNewType
...
def types_object() -> Types:
    return Types(
        ...
        new_type=StrType("foo"),
        nested_new_type=SimpleWithNewType(name="bar"),
    )
...
class Circular:
    name: str
    foo: str
    sub_circular: typing.Optional["Circular"] = serpyco.nested_field(
        default=None, exclude=["foo"]
    )
...
def test_unit__json_schema__ok__nominal_case() -> None:
    ...
    assert {
        ...
        "definitions": {
            ...
            "test_unit.SimpleWithNewType": {
                "comment": "test_unit.SimpleWithNewType",
                "description": "Basic class.",
                "properties": {"name": {"type": "string"}},
                "required": ["name"],
                "additionalProperties": True,
        ...
        "properties": {
            ...
            "nested_new_type": {"$ref": "#/definitions/test_unit.SimpleWithNewType"},
        },
        "required": [
            "integer",
            "string",
    ...
    assert {
        ...
        "items": {
            ...
            "properties": {
                ...
                "new_type": {"type": "string"},
                "nested_new_type": {
                    "$ref": "#/definitions/test_unit.SimpleWithNewType"
                },
            },
            "required": [
                "integer",
...
def test_unit__global_type_encoders_metaclass__ok__no_exception() -> None:
    class WithMeta(metaclass=abc.ABCMeta):
        pass

    class Encoder(serpyco.FieldEncoder):
        def dump(self, my: WithMeta):
            return None

        def json_schema(self) -> dict:
            return {}

    serpyco.Serializer.register_global_type(WithMeta, Encoder())
    serpyco.Serializer.unregister_global_type(WithMeta)
...
def test_unit__decorators__instance_methods__ok__nominal_case():
    @dataclasses.dataclass
    class Decorated:
        foo: typing.Optional[str]
        bar: int
        ...
        @serpyco.pre_dump
        def add_two_to_bar(self, obj: "Decorated") -> "Decorated":
            obj.bar += 2
            return obj

        @serpyco.post_dump
        def del_foo_key(self, data: dict) -> dict:
            del data["foo"]
            return data

        @serpyco.pre_load
        def add_foo_if_missing(self, data: dict) -> dict:
            if "foo" not in data:
                data["foo"] = "default"
            return data

        @serpyco.post_load
        def substract_two_from_bar(self, obj: "Decorated") -> "Decorated":
            obj.bar -= 2
            return obj
...
def test_unit__decorators__class_methods__ok__nominal_case():
    @dataclasses.dataclass
    class Decorated:
        foo: typing.Optional[str]
        bar: int
        ...
        @classmethod
        @serpyco.pre_dump
        def add_two_to_bar(cls, obj: "Decorated") -> "Decorated":
            obj.bar += 2
            return obj

        @classmethod
        @serpyco.post_dump
        def del_foo_key(cls, data: dict) -> dict:
            del data["foo"]
            return data

        @classmethod
        @serpyco.pre_load
        def add_foo_if_missing(cls, data: dict) -> dict:
            if "foo" not in data:
                data["foo"] = "default"
            return data
        ...
        @classmethod
        @serpyco.post_load
        def substract_two_from_bar(cls, obj: "Decorated") -> "Decorated":
            obj.bar -= 2
            return obj
...
def test_unit__decorators__instance_methods_with_self__ok__nominal_case():
    @dataclasses.dataclass
    class Decorated:
        foo: typing.Optional[str]
        bar: int
        ...
        @serpyco.pre_dump
        def add_two_to_bar(self) -> "Decorated":
            self.bar += 2
            return self
        ...
        @serpyco.post_load
        def substract_two_from_bar(self) -> "Decorated":
            self.bar -= 2
            return self
...
def test_unit_nested_field__ok__circular_reference_with_exclude():
    serializer = serpyco.Serializer(Circular)
    serializer.dump(Circular("root", "foo1", Circular("child", "foo2")))


def test_unit_nested_generic_dataclass__ok__nominal_case():
    T = typing.TypeVar("T")
