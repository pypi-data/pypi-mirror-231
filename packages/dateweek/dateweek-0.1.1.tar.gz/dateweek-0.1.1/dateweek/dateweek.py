...
class DateWeek:
    ...
    def time_steps(self, timestep: int = 10) -> List[datetime]:
        """Gera timestamps de N in N minutos para a quantidade de dias especificados

        Args:
            timestep (int): Tamanho do intervalo entre os timestamps em minutos. Defaults to 10
            n_days (int): Quantidade de dias. Defaults to 2
            end_date (datetime, optional):
                Dia de fim dos timestamps, se não for especificado o inicio será 'hoje'.

        Yields:
            Generator[datetime, datetime, datetime]
        """
        for ts_step in range(int(24 * 7 * 60 / timestep) + 1):
            yield self.first_day + timedelta(minutes=timestep * ts_step)
