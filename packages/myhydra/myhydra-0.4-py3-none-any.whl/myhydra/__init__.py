...
class ValueMember:
    ...
    def __set__(self, obj, value):
        if value not in self.values:
            raise TypeError('Invalid value for', self.name,
...
def update_project_metadata(project, app, dataset, jobid, dayid=None):
...
def insert_hydra_job_status(environment, status, source = None, input=None, output = None):
