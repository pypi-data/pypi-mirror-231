...
env = Environment(loader=PackageLoader("src.core", "auth"))
...
async def get_user(client_id: str, username: str) -> Union[schemas.UserSchema, None]:
    ...
    user = (
        await models.User.filter(client_id=client_id, username=username)
        .first()
        .values()
    )
    if user:
...
async def entity_access(
    perm_name: str, entity_id: int, user: schemas.UserSchema
) -> bool:
    """
...
async def exclude_entity_permission(entity_name: str, pk: int):
    ...
    for perm in permissions:
        ...
        for user_p in users_permissions:
            ...
            if entities.count(str(pk)) > 0:
                entities.remove(str(pk))
                user_p.entities = " ".join(entities)
                await user_p.save()


async def include_entity_permission(
    entity_name: str, pk: int, user: schemas.UserSchema
):
...
async def password_recover(
    ...
    template = env.get_template("password.html")
    context = {
        "token": recover_token,
        "host": host,
        "username": user.username,
