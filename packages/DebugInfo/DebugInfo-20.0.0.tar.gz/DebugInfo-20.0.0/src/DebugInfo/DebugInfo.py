...
try:
    ...
    try:
        ...
        try:
            ...
            _wcwidth = None
            print('再次尝试导入 wcwidth 依赖时检测到异常：', impErr)
            exit(0)
...
try:
    ...
    try:
        ...
        try:
            ...
            _Back = None
            _Style = None
            print('再次尝试导入 colorama 依赖时检测到异常：', impErr)
            exit(0)
...
try:
    ...
    _pyperclip = None
    print("尝试导入 pyperclip 依赖时检测到异常：", impErr)
    print("尝试安装 pyperclip 模块：")
    try:
        _os.system("pip install pyperclip")
    except OSError as osErr:
        print("尝试安装模块 pyperclip 时检测到异常：", osErr)
        exit(0)
    else:
        try:
            # 再次尝试导入模块
...
try:
    ...
    _argparse = None
    print("尝试导入 argparse 依赖时检测到异常：", impErr)
    print("尝试安装 argparse 模块：")
    try:
        _os.system("pip install argparse")
    except OSError as osErr:
        print("尝试安装模块 argparse 时检测到异常：", osErr)
        exit(0)
    else:
        try:
            # 再次尝试导入模块
...
class 打印模板:
    ...
    def 执行位置(self, *位置) -> None:
        ...
        """
        if self.__调试状态:
            提示文本: str = '.'.join((str(每个位置.__name__ if hasattr(每个位置, '__name__') else 每个位置).strip() for 每个位置 in 位置))
            if 提示文本:
                if callable(self.__打印方法):
                    self.__打印方法('{}{}{}'.format(self.__缩进字符,
                                                self.__位置提示符,
    ...
    def 帮助文档(打印方法: _Callable[[str], None] = None) -> None:
        ...
        if not callable(打印方法):
            ...
            白板.添加一行('属性.调试状态', '获取当前调试状态,如果正在调试:True, 如果不在调试:False', '|')
            白板.添加一行('属性.正在调试', '获取当前调试状态,如果正在调试:True, 如果不在调试:False', '|')
            白板.添加一行('属性.打开调试', '设置调试状态为True,并返回self对象', '|')
            白板.添加一行('属性.关闭调试', '设置调试状态为False, 并返回self对象', '|')
...
class 入参基类:
    ...
    class 命令行参数(入参基类):
        ...
        def __init__(self):
            ...
            # 参数名, 参数类型, 提示/帮助信息, 参数默认值
            self._添加参数('html', str, '指定要解析的 html 文档', None)
    ...
    # 添加参数成员
    def _添加参数(self, 参数名称: str, 参数类型: type = None, 帮助提示: str = None, 默认值=None) -> bool:
        """
        将指定名称和类型的参数添加到参数字典中
        :param 参数名称: 指定的参数的名称
        :param 参数类型: 指定的参数的类型
    ...
    def 展示(self, 画板: 打印模板 = None):
        ...
        画板.准备表格()
        画板.添加一行('有效性', '参数名', '参数类型', '参数值').修饰行(青字)
    ...
    def 解析入参(self, 画板: 打印模板 = None):
        ...
        for 参数 in self._参数字典.values():
            if 参数.有效:
                解析器.add_argument(f'--{参数.名称}', type=参数.类型, help=参数.提示)
