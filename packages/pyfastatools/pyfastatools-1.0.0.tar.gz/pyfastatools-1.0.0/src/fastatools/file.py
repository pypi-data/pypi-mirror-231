...
from fastatools.iterator import FastaIterator
from fastatools.record import FastaFormat, FastaHeader, FastaRecord
from fastatools.types import FilePath
from fastatools.utils import detect_compression
...
class FastaFile:
    ...
    def parse(self, verbose: bool = False) -> FastaIterator:
        it = self._parse()
        if verbose:
            it = tqdm(it, total=self.count, desc="Parsing FASTA", unit=" records")
