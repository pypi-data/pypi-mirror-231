...
from ..util import remove_prefix, unwrap, file_md5
from .app import App
...
def create_endpoints_output_subject(app_id: str) -> str:
    ...
    # TODO: losing-array - see entry_point.py for more details - we should be able to treat this like an array
    joined_batch_hierarchy = '${" meta("_seaplane_batch_hierarchy").parse_json().join(".") }'
...
def print_endpoints(schema: Dict[str, Any]) -> None:
    ...
    if len(apps) > 0:
        log.info("\nDeployed Endpoints:\n")
        # TODO: Joshua: Replace this with the Endpoints API
    for app_id in apps:
        entry_point_type = schema["apps"][app_id]["entry_point"]["type"]
        version = "latest"  # right now we only support latest version
        if entry_point_type == "API":
...
def deploy_task(
    ...
    # TODO: Josh - Actually only one task saves output now, this do this already?
