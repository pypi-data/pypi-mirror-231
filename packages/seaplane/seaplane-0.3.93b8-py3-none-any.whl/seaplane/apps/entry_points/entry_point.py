...
def start_task(task_id: str, save_result: bool) -> None:
    ...
    while True:
        ...
        if "_seaplane_request_id" not in message.meta:
            ...
            # TODO: losing-array: Somewhere along the path to the next task, arrays in metadata
            # are being turned to strings. For now I'm just making it a string initially
            # and we'll treat it as a string in the task emit() call
            # The path where something is going wrong is:
            # TaskContext.emit() - manipulates the metadata batch heirarchy and appends to it
            # processor.write() - encodes the metadata with msgpack.packb
            # Benthos decoding step
            # processor.read() - unpacks msgpack
        ...
        # The task will have written messages (by virtue of TaskContext.emit), so we now
