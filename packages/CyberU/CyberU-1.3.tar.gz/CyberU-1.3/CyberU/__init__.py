...
def DebugConsume(func):
    def wrapper(*a, **b):
        def inner1(f, *a, **b):
            ...
            filename1 = rmtail(filename1, '.py', strict=False)  # 不加strict 控制台调试时会异常
...
def consume(func):
    ...
    def wrapper(*a, **b):
        def inner1(f, *a, **b):
            stole = nowstr()
            ret = f(*a, **b)
            funcname1 = inspect.getframeinfo(inspect.currentframe().f_back)[2]
            try:
                funcname2 = inspect.getframeinfo(inspect.currentframe().f_back.f_back)[2]
                # funcname2 = inspect.getframeinfo(inspect.currentframe().f_back.f_back)[3]
                # funcname2 = (funcname2[0])
                # funcname2 = funcname2[funcname2.find('.') + 1:funcname2.find('(')]
                funcname3 = inspect.getframeinfo(inspect.currentframe().f_back.f_back.f_back)[2]
            except Exception as e:
                pass
            if counttime(stole) > 1:
                log(f'函数{funcname1}/{funcname2}/{funcname3} 所消耗的时间：'
                f'{int(counttime(stole))} s')
            return ret
...
def set_env_var(d,origin=False):
    if origin:
        for i in d:
            os.environ[i]=d[i]
        return
...
def get_env_var(name,origin=False):
    ...
    return jsontodict(os.environ['globals'])[name]
# 获取用户个性化设置
...
class delta_time(datetime.timedelta):
    def __new__(cls, seconds=0, *args, **kwargs):
        if isinstance(seconds, datetime.timedelta):
            return super().__new__(cls, seconds=seconds.seconds)
        return super().__new__(cls, seconds=seconds, *args, **kwargs)
...
class Cmd:
    def __init__(self,init_root=None,silent=False):
        ...
        self.process = subprocess.Popen('cmd.exe', stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                                        stderr=subprocess.PIPE, text=True, bufsize=10)
        import queue
        self.output=''
        self.silent=silent
    ...
    def refresh_output(self,t=0.5, silent=None,max_retries=20):
        """
        将队列的输出放到成员字符串 output 中
        @param t: 等待输出全部进入队列完全的时间
        @param silent:
        @return:
        """
        sleep(t)
        if silent==None:
            silent=self.silent
        while not self.output_queue.empty() and max_retries>0:
...
def save_var(variable, filename):
    """保存变量到指定的文件中"""
    import pickle
    f = file_operate(add_extension(varpath(filename), '.pkl'), 'wb')
...
def execute_module(path, root=None, pool=None):
    """
    ...
    if pool:
        ...
        try:
            ...
            warn('需要主程序在if __name__ == "__main__":中运行。')
...
def _run_module(file_path):
    ...
    if not isfile(file_path):
        file_path = projectpath(file_path)
    Run()
...
def get_lock(name=None, interval=1,max_wait=10):
    ...
    while isfile(lockpath(f'{name}'),exist=True,notnull=False):
        sleep(interval)
        max_wait-=1
        if max_wait<0:
            Exit('锁超时了。')
    createfile(lockpath(f'{name}'))
    return True
getlock=get_lock
...
def monitor_file_size(path: str,t=None, interval: int = 15, max_time: int = 999,silent=True) -> bool:
    """
...
# 文件系统读写
# region
def get_full_filename(root, filename,recursive=True,loop=20):
    """
    从文件夹中获取文件名的完整文件名
    @param root:
    @param filename:
    @param recursive:  文件名（不带路径
    @return:
    """
    while True:
        loop-=1
        ...
        sleep(2)
        if loop<0:
            warn(f'get_full_filename({root},{filename})超时')
            return False
...
def extensionandname(fname, silent=True, exist=True, parentpath=False, withdot=True, full=None):
    """
    分割带路径的文件名和.+扩展名
    @param fname:
    @param silent:
...
def lockpath(s=''):
    return get_base_path(projectpath('lock'), s)
...
class pcsv():
    ...
    def add_piece(self, piece_uid, author='', title=''):
        ...
        @return:
        """
        self.add({'num': piece_uid, 'author': author, 'title': title, 'disk': diskname})
    ...
    def backup(self, force=False):
        ...
        backupname = extensionandname(self.path)[0] + '_backup' + extensionandname(self.path)[1]
    ...
    def add(self, l, duplicate=None, save=True, isnew=True):
        ...
        if type(l) in [dict]:
            ...
            self.data = pandas.concat(
                [self.data, pandas.Series(l, index=self.data.columns).to_frame().T],
                ignore_index=True)
        ...
        if len1 == len2:
            ...
            get_lock(self.path)
            pandas.Series(l, index=self.data.columns).to_frame().T.to_csv \
                (self.path, index=False, encoding=self.encoding, mode='a', header=False)
            release_lock(self.path)
        if not self.silent:
...
def file_operate(mode, path, IOList=None, encoding=None):
    """
...
class txt():
    ...
    @DebugConsume
    def __init__(self, path, encoding='utf-8', silent=None, ext=None):
        self.silent = silent
...
class RefreshTXT(txt):
    ...
    def get(self, silent=None):
        ...
        if len(self.l) < 1:
            return None
        self.l = Set(self.l + rtxt(self.path, encoding=self.encoding, silent=self.silent).l)
...
class cache(txt):
    ...
    def __init__(self, path, silent=None, json=True):
        super().__init__(path, silent=silent)
        self.silent = silent
        self.path = path
        self.json = json
        self.encoding='utf-8'
...
def Log(s, front=242, font=1, background=238, withtime=True, traceback=False, deleted=False):
    """
    打印单行输出（不处理换行）
    @param s:
    @param front:
    ...
    try:
        ...
        print(sss,
              ...
              ANSIFormatter.front(front), ANSIFormatter.font(font), s, ANSIFormatter.reset_all(),
...
def setRootPath(dname=None, d=None, strict=strict()):
    """
    动态更改操作盘
    @param d:盘符
...
class VideoDownloader():
    ...
    def output(self):
        self._terminal.refresh_output()
        delog(self._terminal.output[-30:])
...
def download_video(url, vpn=True, cookie='opera', targetroot=None,silent=True,
    ...
    if targetname:
        ...
        targetname=''
    move(target, targetroot+'/'+filename(target)+'/'+targetname,overwrite=True)
    _terminal.close()
...
def Element(l, s, method=By.XPATH, depth=5, silent=debug, strict=True):
    res = Elements(l, s, depth=depth, silent=silent, method=method, strict=strict,comlete=False)
    if res == []:
        return None
    else:
        return res[0]
...
def Elements(l, s, depth=7, silent=True,
    ...
    while True:
        ...
        if len(ret):
            ...
            if atr not in ['text']: # text  以外的属性
                ...
                try:
                    l= [i.text if i.text else i.get_attribute('text') for i in ret]
                    if strict:
            ...
            if depth >= 10:
                ...
                if strict:
                    root.save_screenshot(cachepath('current.png'))
                    look(cachepath('current.png'))
                    copyto(s)
                    Exit(message)
                if not silent:
                    warn(message)
                ...
                return Elements(l, original_s, depth=depth + 1, method=method, strict=strict)
...
def _browser(url='', mine=None, silent=None, t=100, mute=True, _type='edge')->selenium.webdriver:
    ...
    if silent:
        {
            'chrome':lambda:options.add_argument("--headless") if mine else options.add_argument('--headless=new')
    ...
    service = {
        'edge':lambda: None,
        'chrome':lambda:webdriver.ChromeService(executable_path=chromedriver_py.binary_path),
    ...
    if mine==True:
        ...
        _path=tail(command,'=')
        delete((f'{_path}/{i}'for i in ['/Default/Code_Cache','/Default/Cache','/Default/Service Worker/CacheStorage',
...
class Edge():
    def __init__(self, url=None, silent=False, driver=None, mine=False, mute=False):
        if not driver == None:
            self.driver = driver[0]
            return
        else:
            self.driver = _browser(_type='edge',url='', silent=silent, mine=mine, mute=mute)
        if not url == None:
            self.get(url)
        self.silent = silent
        self.mine = mine
    ...
    def forum(self, url=None, titletail=None, hostname=None,
        ...
        for url in urllist:
            ...
            if saveuid:
                ...
                page.save(shotspath(f'{hostname}/{title}{pastcount}/第{count}页/'),
    ...
    def click(self, *a, strict=True, depth=9,**b,) -> bool:
        ...
        if len(a) > 1:
            ...
            Exit(' 未实现')
        s = a[0] if a else b['xpath'] if b['xpath'] else b['s']
...
da      @return:
    ...
    def executejsfile(self, jsname):
        if not '.js' in jsname:
            jsname += '.js'
        return self.driver.execute_script(''.join(txt(jspath(jsname)).l))
...
if (element) {
    ...
    def fullscreen(self, path=None, scale=100, autodown=True, pause=1, clip=True, clipinterval=0.6,
                   cuttop=0, cutbottom=0, cutleft=0, cutright=0, maxheight=40000, _look=False,overwrite=None):
        """
    ...
    def elements(self, s=None,xpath=None, depth=9,
                 silent=True, strict=True, root=None,
                 refresh=False,complete=True) -> list:
        """
        ...
        @return:
        """
        # depth,silent,strict,root,refresh,complete=b['depth'] or 9,b['silent'] or True, b[silent] or True, b['root']
        if root == None:
            root = self.driver
        s=xpath if xpath else s

        # 获取元素列表
        try:
            if not type(s) == list:
                ret = elements([root], s, depth=depth,
                               silent=silent, strict=strict,complete=complete)
            else:
                for i in s:
                    ret = elements([root], i, depth=depth,
                                   silent=silent,strict=False,complete=complete)
                    if not ret == []:
                        break
            if ret in [None, []]:
                if refresh:
    ...
    def get(self, url, t=None,resethight=False,pre_wait=3,interval=3):
        """
        ...
        """
        if resethight:
            self.set_window_size(self.get_width(),200)
        try:
        ...
        # sleep(pre_wait)
        # self.set_window_size(self.get_width(),300)
        if resethight:
            self.down(t=interval)
            while 300>(self.getscrollheight()):
                delog(self.getscrollheight())
                sleep(1)
            self.set_window_size(self.get_width(), self.getscrollheight())
            self.up()
    ...
    def title(self):
        ...
        try:
            ...
            pass
        try:
            ret = self.executejs(r"return document.getElementsByTagName('title')[0].text")
            if not ret in ['', None]:
                return standarlizedFileName(ret)
        except Exception as e:
            pass
    ...
    def screenshot(self, path,overwrite=True,pre_wait=True):
        """
        截图并保存到缓存文件夹。不下滚。
        @return:
        """
        ...
        if path == None:
            path = projectpath('cache/browser_screenshot.png')
        if overwrite or not isfile(path):
            _data=self.driver.get_screenshot_as_png()
...
class Chrome(Edge):
    def __init__(self, url=None, mine=None, silent=False,
                 t=100, driver=None, mute=True,safelock=True,
                 register=True):
        """
        ...
        @param driver:
        @param mute:
        @param safelock: 是否注册当前使用chrome的context
        @param register: sagelock 的同名
        """
        self.mine = mine
        self.type = 'chrome'
        safelock=safelock and register
        if safelock:
            self.register()
        if not driver == None:
            self.driver = driver[0]
            ...
            self.driver = _browser(_type='chrome',url=url, mine=mine, silent=silent, t=t, mute=mute)
        if not url == None:
            self.get(url)
        self.silent = silent
...
def click(x=10, y=10, button='left', silent=True, interval=0.2, confidence=1, limit=0, gap=0.05,
          grayscale=True, xoffset=0, yoffset=0, strict=False, moveto=True, pos=None, pic=None,
          moveback=True):
    """
    点击屏幕或者识别屏幕内容
    @param x:x为坐标，或者图片路径
    ...
    if not pic == None:
        return locate_on_screen(image=pic, confidence=confidence, limit=limit, gap=gap,
                                grayscale=grayscale, xoffset=xoffset, yoffset=yoffset,
                                strict=strict)
    if not pos == None:
        (x, y) = pos

    # 点击图片
    if type(x) in [str] and isfile(path=x,exist=True):
        if not any(element in x for element in ['png', 'jpe', 'jpeg']):
            x += '.png'
        path = picpath(x)
        # 路径中文名问题
...
@consume
def pagedownload(url, path, t=999, silent=True, depth=0, auto=None, redownload=None,
                 overwrite=False, page=None, driver=None):
    """
    ...
    if t == 0:
        t = 15
        warn('t=0，请检查。')
    t = int(t / download_speed)
    path = standarlizedPath(path, strict=True)
    path = path.replace('~', '_')
    if page == [None] or driver == [None]:
        page = None
        driver = None
    if not driver == None:
        driver = driver[0]
    ...
    if not redownload:
        if exists(path) and not size(path) == 0:
            if not overwrite:
                ...
                move(path, cachepath(f'trashbin/{now()}'))  # 因为 overwirte, 所以先删除原来存在的文件
                warn(f'因为 overwrite， pagedownload 在下载前先移动删除原来存在的文件。 {path}')
    ...
    def recursive()->bool:
        ...
        """
        if auto:
            lis1 = listfile(downloadroot)
        download_success=monitor_file_size(path=f'{downloadroot}/{downloadname}.crdownload',max_time=t)
        if driver == None: # 如果是新开启浏览器实例，就关闭这个新实例
            page.quit()
        else:
            page.close()
            page.switchto(previouspage)
        sleep(1)  # ？
        ...
        def reload(s, d):
            ...
            return pagedownload(url, path, t=t + t, depth=depth + 1, silent=silent,
                                auto=auto,
                                redownload=redownload, overwrite=overwrite, page=[page],
                                driver=[driver])

        for ii in listfile(downloadroot):
            ii = filename(ii)
            if downloadname in ii and '.crdownload' in ii:
                return reload(f'{t}s后下载失败。没有缓存文件存留（自动删除） 请手动尝试 {url}', ii)
            if downloadname == ii or '.' in ii and downloadname == rmtail(ii, '.'):
                if '.htm' in ii:
                    return reload('下载了  .htm？？？', ii)
                _targetname = targetname
                if not '.' in targetname and '.' in ii:  # 传参没有传扩展名但是浏览器下载自动加了
                    _targetname = targetname + extension(ii, full=False, withdot=True)
                move(downloadroot + '/' + ii, targetroot + '/' + _targetname,
    ...
    if driver == None:
        downloadroot = standarlizedPath(cachepath('pagedownload'), strict=True)
        options = webdriver.ChromeOptions()
        prefs = {'download.default_directory': f'{downloadroot}'}
        ...
        downloadroot = userpath('Downloads/')
    targetroot, targetname = parentpath(path), filename(path)
    downloadname = str(Now().timestamp()).replace('.','-')

    try:
        if driver == None:
            page.get(url)  # 必须get一次以解决传入的url 会重定向的问题
        else:
            previouspage = driver.driver.current_window_handle
            previouscontent = listfile(downloadroot)
            page.open(url)
    ...
    while i < 10:
        ...
        try:
            ...
            a1.download='{downloadname}';\
            a1.click();")
            # root必须存在，否则会跳出为另存为
            # delog(f'pagedownload 正在下载 {url} 到 {downloadroot}{downloadname}')
            break
        except Exception as e:
            warn('下载重试中...')
            warn(e)
    ...
    downloadname=get_full_filename(downloadroot,downloadname,recursive=True)
    if not downloadname:
        warn(f'js执行下载失败，没有检测到下载文件 {url}')
        return False
    downloadname=rmtail(downloadname,'.crdownload',strict=False)
    return recursive()
