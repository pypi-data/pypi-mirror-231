...
def numero_derivacoes(n):
    """
    Calcula o número de derivações para n objetos.

    Args:
        n (int): Número de objetos.

    Returns:
        int: Número de derivações.
    """
    import scipy.special as sp
    return sum([(-1)**k * sp.comb(n, k) * (n - k)**n for k in range(n + 1)])
...
def numero_particoes(n, m):
    """
    Calcula o número de partições de n com no máximo m partes.

    Args:
        n (int): Número a ser particionado.
        m (int): Número máximo de partes.

    Returns:
        int: Número de partições.
    """
    partitions = [[0 for _ in range(m + 1)] for _ in range(n + 1)]
    for i in range(n + 1):
        partitions[i][0] = 1
...
def sequencia_farey(n):
    """
    Calcula a sequência de Farey de ordem n.

    Args:
        n (int): Ordem da sequência de Farey.

    Returns:
        list: Lista contendo os números na sequência de Farey.
    """
    farey = set()
    for denominator in range(1, n + 1):
        for numerator in range(1, denominator + 1):
