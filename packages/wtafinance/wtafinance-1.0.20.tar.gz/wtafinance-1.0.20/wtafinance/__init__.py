...
class ClassCodeConfig():
    ...
    def initConfig(cls):#cls 静态方法中的当前类
        url = "http://192.168.131.173:8080/app_finance/getcodeconfig"
...
def init_ClassCodeConfig(wtaUseTushare=True):

    if(not wtaUseTushare):    
        try:
            ClassCodeConfig.initConfig()
        except:
            print("Cannot find ClassCodeConfig,or can't connect to server")
