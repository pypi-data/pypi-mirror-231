...
def outputs(file):
    ...
    try:
        ...
        scale_variables = [
            key
            for key, measure in codebook.variable_measure.items()
            if measure == "scale"
        ]
        weights = ["Unweighted"] + [
        ...
        sample_comparisons = [
            ...
            for comb in list(
                combinations(
                    list(
                        product(
                            values["Group"].unique(), values["Time"].unique(), weights
                        )
                    ),
                    2,
                )
        ...
        for combination in tqdm(
            ...
            if (
                sample.paired
                and len(
                    set(sample.one.values["ID"]).intersection(
                        set(sample.two.values["ID"])
                    )
                )
                == 0
            ):
