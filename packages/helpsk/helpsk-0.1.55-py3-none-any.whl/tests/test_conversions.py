...
def check_retention_matrix(retention, current_cohort, time_delta):
    # iterate over dataframe and ensure that the cohort + interval (columns 0, 1, 2, etc.) is less
    # than or equal to the current cohort based on current_datetime
    for _, row in retention.iterrows():
        cohort = row['cohort']
        for interval in retention.columns[2:]:
            # if na, that should imply that the cohort + interval is >= the current cohort
            if pd.isna(row[interval]):
                assert cohort + time_delta(int(interval)) >= current_cohort
            else:
                assert cohort + time_delta(int(interval)) < current_cohort
...
def test_retention_matrix():  # noqa
    ...
    # convert current_datetime to week
    assert df.equals(copy_df)
    assert retention_week['cohort'].notna().all()
    # enture that the value associated with the current cohort is NA (since we not everyone in
    # the cohort has had a chance to convert)
    current_cohort = current_datetime.to_period('W').to_timestamp()
    assert retention_week.loc[retention_week['cohort'] == current_cohort, '0'].isna().all()
    check_retention_matrix(retention_week, current_cohort, time_delta=lambda x: timedelta(weeks=x))
    ...
    assert (retention_week['0'][retention_week['0'].notna()] == 1).all().all()
    ...
    retention_day = retention_matrix(
        ...
        unique_id='user_id',
        intervals='day',
        current_datetime=None,  # defaults to datetime.utcnow()
    )
    assert df.equals(copy_df)
    assert retention_day['cohort'].notna().all()
    current_datetime = pd.to_datetime(datetime.utcnow())
    current_cohort = current_datetime.to_period('D').to_timestamp()
    check_retention_matrix(retention_day, current_cohort, time_delta=lambda x: timedelta(days=x))
    ...
    current_cohort = df['datetime'].max().to_period('W').to_timestamp()
    ...
    check_retention_matrix(retention2, current_cohort, time_delta=lambda x: timedelta(weeks=x))
    ...
    assert (retention2['0'][retention2['0'].notna()] == 1).all().all()
...
def test_retention_matrix_by_month():  # noqa
    ...
    current_cohort = df['datetime'].max().to_period('M').to_timestamp()
    check_retention_matrix(
        retention_month,
        current_cohort,
        time_delta=lambda x: DateOffset(months=x),
    )

    assert (retention_month.drop(columns=['cohort', '# of unique ids']).fillna(0) <= 1).all().all()
    assert (retention_month['# of unique ids'] > 1).any()
    assert (retention_month['0'][retention_month['0'].notna()] == 1).all().all()
    ...
    assert (retention_month_28['0'][retention_month_28['0'].notna()] == 1).all().all()
    ...
    _retention_month = retention_month.drop(columns=['cohort', '# of unique ids', '0']).fillna(0)  # noqa
