...
def retention_matrix(
    ...
    # Step 4: Filter out any intervals that are on after the current datetime
    current_cohort = current_datetime.to_period(intervals).to_timestamp()
    for row in matrix.index:
        for col in matrix.columns:
            ...
            if row + DateOffset(**offset) >= current_cohort:
    ...
    # ensure all cohorts have a value of 1 for period 0 except those that are na (i.e. cohorts
    # that are on/after the current cohort)
    assert (matrix['0'][matrix['0'].notna()] == 1).all()
    return matrix
