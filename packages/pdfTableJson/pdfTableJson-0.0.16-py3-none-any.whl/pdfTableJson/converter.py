...
def main(input_path):
    ...
    all_text = getMoreData(input_path)
    ...
    # 전체 텍스트 JSON 형식 반환
    json_all_text = json.dumps(all_text, indent=4, ensure_ascii=False)

    # JSON 형식 변환
    json_data_combined = json.dumps(total_data_list, indent=4, ensure_ascii=False)

    # Json 파일 생성 처리
    f_make_json_file(json_all_text + json_data_combined, out_path, pdf_name) 
