...
class NavigationItem(object):
    ...
    # Indicates whether the item represents the currently active route
    active = False
...
class View(Link):
    ...
    The ``endpoint``, ``*args`` and ``**kwargs`` are passed on to: func:`~flask.url_for` to get the link.
    ...
    # Whether to consider query arguments (``?foo=bar&baz=1``) when
    # determining whether a ``View`` is active.
    ...
    def active(self):
        ...
        _, url = request.url_rule.build(self.url_for_kwargs, append_unknown=not self.ignore_query)

        return url == request.path if self.ignore_query else url == request.full_path
