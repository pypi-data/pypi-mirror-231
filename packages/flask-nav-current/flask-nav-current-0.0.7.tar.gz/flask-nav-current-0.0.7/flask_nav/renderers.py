...
class Renderer(Visitor):
    ...
    Visiting a node should return a string or an object that converts to
    ...
    def visit_object(self, node):
        ...
        Outside debug-mode, returns an empty string.
        """
        if current_app.debug:
            return tags.comment(f"no implementation in {self.__class__.__name__} to render {node.__class__.__name__}")
        return ""
