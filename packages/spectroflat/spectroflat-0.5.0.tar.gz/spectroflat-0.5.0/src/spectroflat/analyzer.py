...
class Analyser:
    ...
    def __init__(self, cube: np.array, config: Config, report_dir: str = None):
        self._coeffs = {MEAN: [], STD: [], MAX: []}
        self._dpx = {MEAN: [], STD: [], MAX: []}
        self._running = False
        self._report = None
    ...
    def _desmile_full(self) -> np.array:
    ...
    def _refine_hard_flat(self) -> float:
        if self._config.roi is None:
            ...
            hard_flat = af.remove(self._input)
            ...
            hard_flat = af.remove(self._input)
            temp = np.ones(self._input.shape)
    ...
    def _interpolate_full(self):
        for s in range(self.gain_table.shape[0]):
            ...
            self.gain_table[s] = GaussianBlur(self.gain_table[s],
                                              kernel=self._config.smile.line_distance).run().gain

    def _interpolate_roi(self):
        roi = self._config.roi
        for s in range(self.gain_table.shape[0]):
            self.gain_table[s][roi] = ResidualsRemover(self.gain_table[s][roi]).run().img
            self.gain_table[s][roi] = GaussianBlur(self.gain_table[s][roi],
                                                   kernel=self._config.smile.line_distance).run().gain
    ...
    def _plt_smoothing_result(self):
        ...
        plt_state_imgs(self.gain_table, title='Gain table after smoothing', pdf=self._report)
