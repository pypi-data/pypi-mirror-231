...
class SmoothModel:
    ...
    def create(self):
        ...
        self._remove_spectral_response()
        self._finalize()
        return self
    ...
    def _remove_spectral_response(self):
        if not self._config.gen_column_response_map and not self._config.average_column_response_map:
            return

        responses = [self.img[:, i].mean() / self.img[:, i + 1].mean() for i in range(0, self.img.shape[1] - 1, 2)]
        if self._config.average_column_response_map:
            responses = np.ones(len(responses)) * np.mean(responses)

        for i, response in enumerate(responses):
            self.response_map.append(0)
            self.response_map.append(response)
            self.img[:, i * 2 + 1] = self.img[:, i * 2 + 1] * response
        while len(self.response_map) < self.img.shape[1]:
            self.response_map.append(0)
