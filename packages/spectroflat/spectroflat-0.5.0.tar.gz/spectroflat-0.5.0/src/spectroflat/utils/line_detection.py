...
def find_line_cores(row: np.array, conf: SmileConfig, peaks: list = None) -> tuple:
    ...
    return peaks, _find_minima(peaks, row, conf.line_distance // 2, allow_shift)
...
def _find_minima(peaks: np.array, row: np.array, win: int, allow_shift: bool = False) -> list:
    return [_fit_peak(p, row, win, allow_shift) for p in peaks]


def _fit_peak(peak: int, row: np.array, win: int, allow_shift: bool) -> float:
    thresholds = [0.25, 0.5, 0.75, 1, 1.5, 2] if allow_shift else [0.3]
    if allow_shift:
