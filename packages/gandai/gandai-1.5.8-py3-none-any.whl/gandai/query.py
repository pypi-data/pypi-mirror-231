...
def mtd_validations() -> pd.DataFrame:
    statement = f"""
    WITH ranked_actors AS (
        ...
        WHERE
            a.type = 'research' 
            AND e.type = 'validate'
            AND date_trunc('month', to_timestamp(e.created)) = date_trunc('month', current_date)
        GROUP BY 
            a.key, analyst, s.uid
    )
    SELECT 
...
def enriched_searches_query():
    ...
    recent_event_count = mtd_validations()
