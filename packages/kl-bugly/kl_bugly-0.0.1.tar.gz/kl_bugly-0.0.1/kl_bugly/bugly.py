...
from bugly_sdk import bugly_defs
...
class BuglyHelper:
    _curpath: str
    _app_list: List[bugly_defs.BuglyAppInfo] = []
    _userId = ""
    _qq: int = 0
    ...
    # 获取问题类型列表
    def get_issue_list(self, app_id: str, platform_id: int, vername: str, start_date: str, end_data: str, max_num: int = 0) -> List[bugly_defs.IssueInfo]:
        ...
        while True:
            ...
            for item in res_data["issueList"]:
                issue_item = bugly_defs.IssueInfo.parse_obj(item)
    ...
    def get_issue_detail(self,
                         appid: str,
                         platform_id: int,
                         issue_item: bugly_defs.IssueInfo,
                         start_date: str,
                         end_date: str,
                         max_num: int = 0,
                         need_get_func: Callable[[str, str, str, datetime], bool] = None) -> List[bugly_defs.CrashInfo]:
        url = f'https://bugly.qq.com/v2/crash-reporting/crashes/{appid}/{issue_item.issueId}'
        get_param = {"fsn": get_fsn(), "pid": platform_id}
        res = self._get(url, params=get_param)
        start = 0
        crash_list: List[bugly_defs.CrashInfo] = []
        page_num = 10
        ...
        while True:
            ...
            for _, v in crashlist_resp_data["crashDatas"].items():
                crash_item_info = bugly_defs.CrashInfo.parse_obj(v)
    ...
    def getCrashDetail(self, crashitem: bugly_defs.CrashInfo, appid: str, pid: int, issuid: str, crash_hash: str, crashId: int) -> (bool):
        max_retrynum = 3
        retrynum = 0
        while (True):
            ...
            if "detailMap" not in res3_json["data"]:
                ...
                return True
                # raise Exception(f'{errtext}')
            crashitem.crashMap = bugly_defs.CrashMap.parse_obj(res3_json["data"]["crashMap"])
            crashitem.detailMap = bugly_defs.CrashDetailMap.parse_obj(res3_json["data"]["detailMap"])
    ...
    def _get_app_list(self):
        ...
        for item in res_json["data"]:
            appinfo = bugly_defs.BuglyAppInfo.parse_obj(item)
            applist_str += f'{appinfo.appName}-{appinfo.appId}\n'
            self._app_list.append(appinfo)
        print(f'获取用app列表成功:\n{applist_str}')

    # 获取首页崩溃汇总信息
    def get_crash_info(self, appinfo: bugly_defs.BuglyAppInfo, start_hour: str, end_hour: str) -> List[bugly_defs.CrashHourInfo]:
        ...
        ret: List[bugly_defs.CrashHourInfo] = []
        for item in res_data:
            iteminfo = bugly_defs.CrashHourInfo.parse_obj(item)
    ...
    def get_appinfo_by_id(self, appid: str) -> bugly_defs.BuglyAppInfo:
        for appinfo in self._app_list:
            if appinfo.appId == appid:
                return appinfo

    def get_appinfo_by_name(self, appname: str) -> bugly_defs.BuglyAppInfo:
        for appinfo in self._app_list:
