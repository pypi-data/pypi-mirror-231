...
class EmcServer(MongoClientTradeGateway):
    ...
    def req_buy_order_insert(self, obj):
        ...
        with open(temp_filename, 'w', newline='') as file:
            ...
            row = [fund_id, algo_type, stkcode, market, Direction, qty, '' ,start_time, end_time, oid, algo, 1]
    ...
    def req_sell_order_insert(self, obj):
        ...
        with open(temp_filename, 'w', newline='') as file:
            ...
            after_action = 0
            algo = f"limit_action={limit_action}:after_action={after_action}"
            row = [fund_id, algo_type, stkcode, market, Direction, qty, '' ,start_time, end_time, oid, algo, 2]
    ...
    def monitor_algo_task_update(self):
        while not self.is_stopped:
            header = ['算法单号', '自定义编号', '资金账号', '证券代码', '证券名称', '市场', '委托方向', '算法类型', '状态', '状态值', '目标数量', '执行进度', '成交数量', '成交金额', '成交均价', '开始时间', '结束时间', '备注', '算法风格']
            for acc in self.accounts_run:
                trade_acc = self.account_id[acc]
                algo_task_filename = self.recv_msg_dir + \
                    '\\' + str(trade_acc) + FILES.ALGO_TASKS + get_today_date()  + '.csv'
                data_list = []
                algo_types = {
                    '算法单号': str,
                    '自定义编号': str
                ...
                df = pd.DataFrame()
                try:
                    df = pd.read_csv(algo_task_filename, dtype=algo_types, encoding='gbk')
                except:
                    self.logger.warning("[monitor_algo_task_update] file not exist")
    ...
    def update_algo_status(self, record, acc):
        ...
        if  len(oid) == 0:
            self.logger.warning(f"[update_order] not use file to order record:{record}")
            return
    ...
    def monitor_order_update(self):
        while not self.is_stopped:
            header = ['算法单号', '资金账号', '证券代码','委托日期', '委托时间' , '委托价格', '委托编号', '合同序号', '委托数量', '成交数量', '成交均价', '撤单数量', '委托状态', '委托状态值', '业务类型', '信用交易类型', '来源']
            algo_order_types = {
                    '算法单号': str,
                    '自定义编号': str
                }
            for acc in self.accounts_run:
                ...
                order_filename = self.recv_msg_dir + \
                    '\\' + str(trade_acc) + FILES.ORDERS + get_today_date()  + '.csv'
            
                data_list = []

                df = pd.DataFrame()
                try:
                    df = pd.read_csv(order_filename, dtype=algo_order_types, encoding='gbk')
                except:
                    self.logger.warning("[monitor_order_update] file not exist")
        ...
        while not self.is_stopped:
            for acc in self.accounts_run:
                ...
                asset_filename = self.recv_msg_dir + \
                    '\\' + str(trade_acc) + FILES.ACCOUNTS + get_today_date()  + '.csv'
                df = pd.DataFrame()
                try:
                ...
                for _, row in df.iterrows():
                    ...
                    db_msg = {
                        ...
                        "balance": all_asset, # 净资产.  最好是由券商那边提供，通常普通账户是 可用资金(包含预扣费用) + 市值；信用账户是总资金 - 总负债
                        "holding": stkasset, # 市值,
        ...
        while not self.is_stopped:
            for acc in self.accounts_run:
                ...
                pos_filename = self.recv_msg_dir + \
                    '\\' + str(trade_acc) + FILES.POSITIONS + get_today_date()  + '.csv'
                dtypes ={
                    '证券代码' : str,
                    '市场': str,
                }
                df = pd.DataFrame()
                try:
                    df = pd.read_csv(pos_filename, dtype=dtypes, encoding='gbk')
                except:
                    self.logger.warning("[update_equity_position] file not exist")
                for acc in self.accounts_run:
    ...
    def monitor_trade_update(self):
        header = ['算法单号', '资金账号', '证券代码', '成交日期', '成交时间', '成交数量', '委托编号', '成交编号', '成交价格', '回报类型', '业务类型', '信用交易类型', '来源']
        algo_trade_types = {
            '算法单号': str
        }
        while not self.is_stopped:
            ...
            for acc in self.accounts_run:
                ...
                trade_filename = self.recv_msg_dir + \
                    '\\' + str(trade_acc) + FILES.TRADES + get_today_date()  + '.csv'
                data_list = []

                df = pd.DataFrame()
                try:
                    df = pd.read_csv(trade_filename, dtype=algo_trade_types, encoding='gbk')
                except:
                    self.logger.warning("[monitor_trade_update] file not exist")
