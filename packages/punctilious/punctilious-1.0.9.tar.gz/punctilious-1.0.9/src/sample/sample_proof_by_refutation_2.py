...
f_o1_eq_f_02 = t1.i.axiom_interpretation.infer_statement(theory_axiom, (f(o1) | u.r.eq | f(o2)))
with u.v('x') as x, u.v('y') as y:
    implication = t1.i.axiom_interpretation.infer_statement(axiom=theory_axiom,
...
substitution = h.child_theory.i.variable_substitution.infer_statement(p=implication, phi=(o1, o2))
inequality = h.child_theory.i.modus_ponens.infer_statement(p_implies_q=substitution, p=f_o1_eq_f_02)
...
h_inconsistency = t2.i.inconsistency_introduction_2.infer_statement(x_eq_y=h.child_statement,
    x_neq_y=inequality, inconsistent_theory=h.child_theory, subtitle='The proposition of interest')

# And finally, use the proof-by-contradiction-2 inference-rule:
proposition_of_interest = t1.i.proof_by_refutation_2.infer_statement(x_eq_y_hypothesis=h,
