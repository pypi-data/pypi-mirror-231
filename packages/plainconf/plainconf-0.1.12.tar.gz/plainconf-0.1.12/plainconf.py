...
def _get_vault_token_approle(
    vault_url: str, vault_approle_id: str, vault_approle_secret_id: str
):
...
def _get_vault_secrets(
    vault_client: hvac.Client, vault_mount_point: str, vault_path: str
) -> dict:
    try:
...
class Plainconf:
    def __init__(self, *args, **kwargs):
        ...
        self.environment: str = (
            kwargs.get("environment") or os.getenv("PLAINCONF_ENVIRONMENT") or "default"
        )

        # settings and secrets files

        settings_file: str = kwargs.get("settings_file") or os.getenv(
            "PLAINCONF_SETTINGS_FILE"
        )
        secrets_file: str = kwargs.get("secrets_file") or os.getenv(
            "PLAINCONF_SECRETS_FILE"
        )

        # vault configuration for hvac

        vault_url: str = kwargs.get("vault_url") or os.getenv("PLAINCONF_VAULT_URL")
        ...
        vault_token: str = kwargs.get("vault_token") or os.getenv(
            "PLAINCONF_VAULT_TOKEN"
        )
        vault_user: str = kwargs.get("vault_user") or os.getenv("PLAINCONF_VAULT_USER")
        vault_pass: str = kwargs.get("vault_pass") or os.getenv("PLAINCONF_VAULT_PASS")
        vault_approle_id: str = kwargs.get("vault_approle_id") or os.getenv(
            "PLAINCONF_APPROLE_ID"
        )
        vault_approle_secret_id: str = kwargs.get(
            "vault_approle_secret_id"
        ) or os.getenv("PLAINCONF_APPROLE_SECRET_ID")

        # fernet key

        fernet_key: str = kwargs.get("fernet_key") or os.getenv("PLAINCONF_FERNET_KEY")
        if fernet_key:
        ...
        if vault_url:
            if vault_user and vault_pass:
                vault_token = _get_vault_token_userpass(
                    vault_url, vault_user, vault_pass
                )
            ...
            vault_client = _get_vault_client(vault_url, vault_token)
