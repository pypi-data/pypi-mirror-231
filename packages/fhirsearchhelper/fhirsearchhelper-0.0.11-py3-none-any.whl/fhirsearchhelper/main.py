        ...
        if not q_search_params:
            ...
            if new_query_response.status_code == 403:
                ...
                if 'WWW-Authenticate' in new_query_response.headers:
                    ...
                    OO_body = {'issue': [{'severity': 'error', 'code': 'processing', 'diagnostics': f'WWW-Authenticate Error: {new_query_response.headers["WWW-Authenticate"]}'}]}
                else:
                    OO_body = {'issue': [{'severity': 'error', 'code': 'processing', 'diagnostics': f'The query responded with a status code of {new_query_response.status_code}'}]}
