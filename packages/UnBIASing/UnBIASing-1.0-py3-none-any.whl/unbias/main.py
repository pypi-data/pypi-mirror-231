...
class BiasPipeline:
    def __init__(self):
        ...
        self.debiaser_tokenizer.mask_token = "[MASK]"
    ...
    def mask_biased_words(self, text):
        # Detect biased words
        biased_entities = self.enhanced_ner([text])[0]

        for entity in biased_entities:
            word = entity['word']
            # Mask the biased word in a case-insensitive manner
            text = re.sub(r'\b' + re.escape(word) + r'\b', self.debiaser_tokenizer.mask_token, text, flags=re.IGNORECASE)

        return text



    def mask_and_replace(self, text):
      # Detect biased words
      biased_entities = self.enhanced_ner([text])[0]
      ...
      for entity in biased_entities:
          ...
          # Mask the biased word in a case-insensitive manner
          masked_text = re.sub(r'\b' + re.escape(word) + r'\b', self.bert_tokenizer.mask_token, text, flags=re.IGNORECASE)

          # Convert the text with the masked word to input_ids
          input_ids = self.bert_tokenizer.encode(masked_text, return_tensors="pt").to(self.device)
          mask_token_index = torch.where(input_ids == self.bert_tokenizer.mask_token_id)[1].tolist()  # Convert tensor to list

          # If there are no masked tokens, continue to the next entity
          if not mask_token_index:
              continue

          # Predict the replacement for the masked token using BERT
          with torch.no_grad():
              outputs = self.bert_model(input_ids)
          logits = outputs.logits
          mask_token_logits = logits[0, mask_token_index, :]
          top_token = torch.argmax(mask_token_logits, dim=1)
          predicted_token = self.bert_tokenizer.decode(top_token.tolist())

          # Match the capitalization of the original word
          if word.istitle():
              predicted_token = predicted_token.title()
          elif word.isupper():
              predicted_token = predicted_token.upper()

          # Replace the original word with the predicted replacement word in the original text
          text = re.sub(r'\b' + re.escape(word) + r'\b', predicted_token, text, flags=re.IGNORECASE)
    ...
    def process(self, texts):
        ...
        # Use mask_biased_words function for masking
        masked_results = [self.mask_biased_words(text) for text in texts]

        # Use mask_and_replace function for replacing
        replaced_results = [self.mask_and_replace(text) for text in texts]

        return {
            "classification": classification_results,
            "ner": ner_results,
            "masked_text": masked_results,
            "replaced_text": replaced_results
        }
# Usage
