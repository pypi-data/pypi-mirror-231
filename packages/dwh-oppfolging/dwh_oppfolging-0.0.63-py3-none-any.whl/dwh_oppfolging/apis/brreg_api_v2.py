...
class BrregUnitAPI:
    ...
    def make_fake_unit_update(
        ...
        orgnr: str,
        change: str = 'UKJENT',
        last_modified_date: datetime = datetime(1989, 12, 31, 23),
    ):
        """
    ...
    def get_specific_units_as_rows(
            self,
            orgnr_list: list[str],
            force_fake_update_if_none_found: bool = True
        ):
        """
        ...
        for orgnr in orgnr_list:
            ...
            if force_fake_update_if_none_found and update is None:
                update = self.make_fake_unit_update(orgnr)
            if update is not None:
                fact = self.get_unit(orgnr)
    ...
    def get_all_units_as_rows_updated_since(self, last_modified_date: datetime):
        ...
        rows = [
            self.make_row(updates[0], self.get_unit(orgnr))
        ...
        with requests.get(url, headers=headers, stream=True, timeout=100) as response:
            ...
            with gzip.open(response.raw, "rb") as file:
                ...
                for record in ijson.items(file, "item"):  # type: ignore
                    records.append(
                        self.make_row(
                            self.make_fake_unit_update(record["organisasjonsnummer"],"FLATFIL", last_modified_date),
                            record,
                        )
                    )
