...
class BrregUnitAPI:
    ...
    def get_specific_units_as_rows(
        ...
        If fake_if_not_found is True, then if the unit has no updates
        a fake update is made, and if the unit can't be found, the unit is faked.
        ...
        for orgnr in orgnr_list:
            update = self.get_unit_update_history(orgnr, True, fake_if_not_found)[0]
            fact = self.get_unit(orgnr, fake_if_not_found)
    ...
    def get_all_units_as_rows_updated_since(
        ...
        If fake_if_not_found is True then a faked unit is used in the row
        ...
        rows = [
            self.make_row(updates[0], self.get_unit(orgnr, fake_if_not_found))
