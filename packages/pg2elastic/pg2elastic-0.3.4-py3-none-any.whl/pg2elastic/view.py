...
from .constants import DEFAULT_SCHEMA, MATERIALIZED_VIEW, MATERIALIZED_VIEW_INDEX, TRIGGER_FUNC
from loguru import logger

from .settings import PG_SCHEMA
...
def _get_partitions(
    ...
    for table in tables:
        if not table.startswith(schema):
            continue
        regclasses.append(f"'{table}'::regclass")

    regclasses = ','.join(regclasses)

    if not regclasses:
        raise RuntimeError(
            f'Make sure you specified the right base tables for partitions query.'
        )

    statement = sa.text(
        f"SELECT inhrelid::regclass AS partition, inhparent::regclass AS table_name FROM pg_catalog.pg_inherits WHERE inhparent::regclass IN ({regclasses})")
...
def _get_publications(
    ...
    for table in tables:
        ...
        regclasses.append(table_name)

    regclasses = ','.join(regclasses)

    if not regclasses:
        raise RuntimeError(
            f'Make sure you specified the right base tables for publications query.'
        )

    statement = sa.text(
        f"SELECT * FROM pg_publication_tables WHERE pubname = '{TRIGGER_FUNC}' AND schemaname = '{schema}' AND tablename IN ({regclasses})")
...
def _get_parents(
    ...
    if regclasses:
        regclasses = f'WHERE inhrelid::regclass IN ({regclasses})'

    statement = sa.text(
        f"SELECT inhrelid::regclass AS partition, inhparent::regclass AS table_name FROM pg_catalog.pg_inherits{regclasses}")
...
def _get_indices(
    ...
    statement = sa.text(f'SELECT * FROM "{PG_SCHEMA}"."{MATERIALIZED_VIEW}"')
...
def _parents(
    schema: str = '', tables: Set[str] = ''
) -> sa.sql.Select:
    return _get_parents(schema, tables)


def _indices(
    schema: str = '', tables: Set[str] = ''
) -> sa.sql.Select:
    return _get_indices(schema, tables)


def _partitions(
    schema: str, tables: Set[str]
) -> sa.sql.Select:
    return _get_partitions(schema, tables)


def _publications(
    schema: str, tables: Set[str]
) -> sa.sql.Select:
    return _get_publications(schema, tables)
...
def create_view(
    ...
    if bool(partitions_mapping):
        for partition, table_name in partitions_mapping.items():
            partition = partition.replace(f'{schema}.', '')
            table_name = table_name.replace(f'{schema}.', '')
            reference = rows.get(table_name, {
                "primary_keys": set(),
                "foreign_keys": set(),
                "indices": set(),
            })
    ...
    engine.execute(DropIndex(MATERIALIZED_VIEW_INDEX))
    engine.execute(
        CreateIndex(
