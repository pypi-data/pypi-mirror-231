...
from pg2elastic.settings import ELASTICSEARCH_PORT
from pg2elastic.urls import (
    _get_auth,
    get_postgres_url,
    get_redis_url,
    get_search_url,
)
from pg2elastic.utils import get_config
...
class TestUrls(object):
    ...
    def test_get_postgres_url(self):
        ...
        # with patch("pg2elastic.urls.logger") as mock_logger:
        #     assert (
    ...
    def test_get_auth(self, mock_logger):
        assert __file__ == get_config(config=__file__)
        with patch("pg2elastic.urls.Plugins", return_value=MagicMock()):
            _get_auth("something")
            mock_logger.assert_not_called()

        with patch("pg2elastic.urls.Plugins", side_effect=ModuleNotFoundError):
