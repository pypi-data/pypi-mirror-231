...
from pg2elastic.constants import DEFAULT_SCHEMA
from pg2elastic.exc import (
    LogicalSlotParseError,
    ReplicationSlotError,
    TableNotFoundError,
)
from pg2elastic.view import CreateView, DropView
...
class TestBase(object):
    ...
    @patch("pg2elastic.base.logger")
    def test__can_create_replication_slot(self, mock_logger, connection):
        pg_base = Base(connection.engine.url.database)

        pg_base.create_replication_slot("foo")
        with patch("pg2elastic.base.Base.drop_replication_slot") as mock_slot:
            with patch("pg2elastic.base.Base.create_replication_slot"):
        ...
        with patch(
            "pg2elastic.base.Base.create_replication_slot", side_effect=Exception
        ) as mock_slot:
    ...
    @patch("pg2elastic.base.logger")
    @patch("pg2elastic.sync.Base.execute")
    @patch("pg2elastic.base.sa.DDL")
    ...
    @patch("pg2elastic.base.logger")
    @patch("pg2elastic.sync.Base.truncate_table")
    ...
    @patch("pg2elastic.base.logger")
    @patch("pg2elastic.sync.Base.truncate_tables")
    ...
    @patch("pg2elastic.sync.Base.truncate_schema")
    ...
    @patch("pg2elastic.base.logger")
    @patch("pg2elastic.base.Base.execute")
    ...
    @patch("pg2elastic.base.pg_execute")
    @patch("pg2elastic.base.pg_engine")
    @patch("pg2elastic.base.logger")
    ...
    @patch("pg2elastic.base.logger")
    @patch("pg2elastic.sync.Base.engine")
    ...
    def test_fetchone(self, connection):
        ...
        with patch("pg2elastic.base.compiled_query") as mock_compiled_query:
    ...
    def test_pg_execute(self, connection):
        with patch("pg2elastic.base.logger") as mock_logger:
            pg_execute(
                connection.engine,
                sa.select(1),
