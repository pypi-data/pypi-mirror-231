...
from pg2elastic.pgoutput.reader import LogicalReplicationReader
from pg2elastic.view import _parents, _indices
...
from .constants import (
    ...
    TG_OP,
    TRUNCATE,
    UPDATE, TRIGGER_FUNC, TEST_DECODING, PLUGIN,
)
from .exc import (
    ForeignKeyError,
    InvalidSchemaError,
...
from .settings import REDIS_CHECKPOINT, PG_HOST, PG_PORT, PG_USER, PG_PASSWORD
...
class Sync(Base, metaclass=Singleton):
        ...
        for schema in self.schemas:
            ...
            if tables:
                ...
                self.create_publications(
                    schema, tables=tables, join_queries=join_queries, index=self.index
                )
    ...
    def _child_node_resolver(
        self, node: Node, payload: Payload, foreign_keys: dict, filters: dict
    ) -> dict:
        """Handle where node is a through table with a direct references to
        root
        """

        if node.name not in foreign_keys:
            return filters
        ...
        if node.table in self.tree.tables:
            if node.is_root:
                ...
                for payload in payloads:
                    ...
                    # through table with a direct references to root 4276f1e3-9979-4990-a356-3af1a337352f
                    if not _filters:
                        _filters = self._through_node_resolver(
                            node, payload, _filters
                        )

                    # handle case where we insert into a child table with a direct reference to root
                    # set the parent as the new entity that has changed
                    if not _filters:
                        self._child_node_resolver(
                            node, payload, foreign_keys, filters
                        )
    ...
    def sync(
        ...
        for node in self.tree.traverse_post_order():
            ...
            try:
                ...
                logger.exception(f"Exception {e} in sync()...")
                raise
    ...
    def poll_db(self) -> None:
        ...
        logger.debug(
            f'Listening to publication on channel "{TRIGGER_FUNC}", {self.database}'
        )
        ...
        while True:
            ...
            while conn.notifies:
                ...
                if notification.channel == self.database:
                    ...
                    if all(key in payload for key in ('parent', 'table')) and payload['parent'] is not None:
                        payload['table'] = payload['parent']
    ...
    def _on_publish(self, payloads: List[Payload]) -> None:
        ...
        if txids != set([None]):
            ...
            self.checkpoint: int = max(txids)

    def pull(self) -> None:
        """Pull data from db."""
        txmin: int = self.checkpoint
        txmax: int = self.txid_current
        logger.warning(f"pull txmin: {txmin} - txmax: {txmax}")
        # forward pass sync
        self.search_client.bulk(
    ...
    def _truncate_slots(self) -> None:
        if self._truncate:
            ...
            # Not suitable with pgoutput
            # self.logical_slot_get_changes(self.__name, publication=self.index, upto_nchanges=None)
    ...
    def partitions(self):
        ...
        for partition_row in self.fetchall(_parents()):
            partition = dict(partition_row).get('partition', '')
            table_name = dict(partition_row).get('table_name', '')
            if partition and table_name:
                self.parent_mapping.update({partition: table_name})

    def indexes(self):
        self.view_mapping = {}

        for index_row in self.fetchall(_indices()):
            indices = dict(index_row).get('indices', [])
            table_name = dict(index_row).get('table_name', '')
            if table_name and table_name:
                self.view_mapping.update({table_name: indices})
    ...
    def receive(self, nthreads_polldb: int) -> None:
        ...
        if settings.USE_ASYNC:
            raise RuntimeError('USE_ASYNC deprecated due to "pgoutput" replication')
        ...
        if settings.USE_ASYNC:
            ...
            # Fetch partitions and indexes
            self.partitions()
            self.indexes()

            # start a background worker producer to capture the db publication changes
            # populate the Redis cache
            self.poll_db_publication()
    ...
    def match_parent_for(self, schema: str = '', partition: str = ''):
        parent = self.parent_mapping.get(f'{schema}.{partition}', '') or ''
        parent = parent.replace(f'{schema}.', '')
        return parent

    @threaded
    @exception
    def poll_db_publication(self) -> None:
        logger.warning(f'Listening to publication on index: {self.index}')

        LogicalReplicationReader(
            receiver=self.poll_db_change_consumer,
            publication_name=self.index,
    ...
    def map_payload_to_notification(self, payload, op):
        before = payload.get('before', None) or None
        after = payload.get('after', None) or None
        table = payload.get('meta', {}).get('table', '') or ''
        schema = payload.get('meta', {}).get('schema', '') or ''
        parent = self.match_parent_for(schema=schema, partition=table) or ''
        indices = self.view_mapping.get(table, []) or []

        event = {
            'xmin': payload.get('transaction', {}).get('tx_id', ''),
            'new': after,
            'old': before,
            'indices': indices,
            'tg_op': op,
    ...
    @exception
    def poll_db_change_consumer(self, payload=None) -> None:
        if payload is None:
            payload = {'op': '', 'meta': {}}
        ...
        if payload.get('op', '') == 'U':
            notification = self.map_payload_to_notification(payload=payload, op=UPDATE)

        """Received truncate event"""
        if payload.get('op', '') == 'T':
            notification = self.map_payload_to_notification(payload=payload, op=TRUNCATE)

        """Received delete event"""
        if payload.get('op', '') == 'D':
            notification = self.map_payload_to_notification(payload=payload, op=DELETE)

        """Received insert event"""
        if payload.get('op', '') == 'I':
            notification = self.map_payload_to_notification(payload=payload, op=INSERT)

        """Add message to redis"""
        if notification:
            if all(key in notification for key in ('parent', 'table')) and notification['parent'] not in ('', None):
                notification['table'] = notification['parent']

            if self.index in notification["indices"]:
                self.redis.push([notification])
                logger.debug(f"on_notify: {notification}")
                self.count["db"] += 1
    ...
    with Timer():
        if analyze:
            ...
            for document in config_loader(config):
                ...
                # First run, checkpoint is null, so it should be set
                # Otherwise, pgoutput publication should get all missed transactions
                if sync.checkpoint is None:
                    sync.pull()
