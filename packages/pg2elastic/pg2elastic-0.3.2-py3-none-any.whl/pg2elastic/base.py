...
from .view import create_view, DropView, is_view, RefreshView, _partitions, _publications
...
class Payload(object):
    __slots__ = ("tg_op", "table", 'parent', "schema", "old", "new", "xmin", "indices")
...
def parse_value(type_: str, value: str) -> Optional[str]:
    """
...
class Base(object):
    ...
    def logical_slot_get_changes(
        ...
        statement: sa.sql.Select = self._logical_slot_changes(
            slot_name,
            sa.func.PG_LOGICAL_SLOT_GET_BINARY_CHANGES,
    ...
    def parse_value(self, type_: str, value: str) -> Optional[str]:
        return parse_value(type_, value)
    ...
    def publication_slots(
        ...
        return self.fetchall(
            sa.select(["*"])
            .select_from(sa.text("PG_PUBLICATION"))
            .where(
                sa.and_(
                    *[
                        sa.column("pubname") == pubname,
                    ]
                )
            ),
            label="publication_slots",
        )
    ...
    def create_publications(
        ...
        """Create a database publications."""
        publication_tables: List[str] = []
        tables_whitelist = []
        publications = {TRIGGER_FUNC: []}
        queries: List[str] = []

        self.drop_publication(index)
        logger.debug(f"Creating publication on index: {index}")
        ...
        for table in self.tables(schema):
            ...
            logger.debug(f"Creating triggers on table: {schema_table}")

            self.drop_triggers(schema, [table])

            """Query base table partitions"""
            for partition_row in self.fetchall(_partitions(schema, {schema_table})):
                row = dict(partition_row)
                partition = row.get('partition', '')
                table_name = row.get('table_name', '')

                """Add partition table to publication"""
                if partition and table_name:
                    publication_tables.append(partition)
                    tables_whitelist.append(table_name)

            """Add base table to publication"""
            publication_tables.append(schema_table)

        if publication_tables:
            publication_tables = ', '.join(publication_tables)

        self.execute(sa.DDL(f'CREATE PUBLICATION "{index}" FOR TABLE {publication_tables};'))

    def drop_publication(
        self,
        index: str,
    ) -> None:
        """Drop all pg2elastic defined publication in database."""
        self.execute(sa.DDL(f'DROP PUBLICATION IF EXISTS "{index}"'))
    ...
    def fetchone(
        ...
        try:
            ...
            logger.exception(f"Exception {e} in fetchone()...")
            raise
        return row
    ...
    def fetchall(
        ...
        try:
            ...
            logger.exception(f"Exception {e} in fetchall()...")
            raise
        return rows
...
def pg_execute(
    ...
    try:
        ...
        logger.exception(f"Exception {e} in pg_execute()...")
        raise
...
def database_exists(database: str, echo: bool = False) -> bool:
    ...
    with pg_engine("postgres", echo=echo) as engine:
        ...
        try:
            ...
            logger.exception(f"Exception {e} in database_exists()....")
            raise
