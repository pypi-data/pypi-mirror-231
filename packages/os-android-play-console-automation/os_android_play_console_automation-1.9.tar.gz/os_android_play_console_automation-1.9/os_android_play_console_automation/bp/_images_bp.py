...
def clear_images(client_secrets_path, package_name, image_type: ImageType, language_initials, initials_as_google_translate=False):
    ...
    while True:
        try:
            ...
            for img_path in img_list:
                ...
                print(f'Image {str(counter)} for {image_type.value} added for package name "{package_name}" with the language "{language_initials}"')
                counter += 1
            ...
            print(e)
            exception_str = str(e)
            if 'Expected OAuth 2 access token' in exception_str:
                print(f'*************************************************\n'
                      f'Got weird \'no auth session token\' error.'
                      f'\nIt seems like a bug from Google\'s side. Running again'
                      f'\nNOTICE: This error code also mean that the language {language_initials}:'
                      f'\nisn\'t in the Play Console or '
                      f'\nmaybe you should remove the .dat file and refresh the login params'
                      f'\n*************************************************')
