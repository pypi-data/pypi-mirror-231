...
def build(image_file_paths, feature_names, project_name):
    ...
    ending_page_image_path = pkg_resources.resource_filename('video_images_creator', "closingframe1920.png") 
    ending_page_image = cv2.imread(ending_page_image_path) 
    os.mkdir(directory_name) 
    put_project_name(bg_img, project_name, directory_name)
...
def put_project_name(bg_image, project_name, directory_name): 

    # logo = cv2.imread("features/build_lgo.png")
    # logo_height, logo_width, _ = logo.shape
    # new_logo_width = int(logo_width * 0.60)
    # new_logo_height = int(logo_height * 0.60)
...
def create_left_to_right_movement(left_image_file_path, right_image_file_path, current_index_, directory_name, project_name, left_screen_name, right_screen_name, bg_img):
    ...
    start_point = np.array([455, 150])  
    end_point = np.array([1179, 150])   
...
def create_right_to_left_movement(left_image_file_path, right_image_file_path, current_index_, directory_name, project_name, right_screen_name, left_screen_name, bg_img):
    ...
    #these are for feature transitions 
    start_point = np.array([1179, 150])  
    end_point = np.array([455, 150])
