...
def parse_and_post_property(
    ...
    try:
        ...
        new_property = Properties(
            ...
            parentModule=parent_module if has_parent_module else "security-plans",
        )

        # Run api call to create property
        ret = new_property.create_new_properties(return_object=True)
...
def post_unmapped_properties(
    ...
    unmapped_properties = [
        prop for prop in properties if prop.attrib["name"] not in mapped_property_names
    ]

    # Post the unmapped properties.
    for prop in unmapped_properties:
        parse_and_post_property(
            trv, prop, parent_id=parent_id, parent_module=parent_module
        )
...
if __name__ == "__main__":
    ...
    def test_parse_and_post_property():
        ...
        assert (
            trv.infos[0]["event"]
            == "Created property 'marking' with value 'Controlled Unclassified Information'"
        )
    ...
    def test_bad_property():
        ...
        assert (
            trv.errors[0]["event"]
            == "Unknown issue: Failed to create property 'marking' with value '<UNKNOWN>'"
        )
    ...
    def test_all_unmapped():
        ...
        assert (
            trv.infos[0]["event"]
            == "Created property 'another' with value 'another-value'"
        )
        assert (
            trv.infos[1]["event"]
            == "Created property 'yet-another' with value 'yet-another-value'"
        )
