...
class MySQL(BaseDBPD):
    ...
    def __init__(
            ...
            description: str = 'MySQL database connection with credentials',
            show_description: bool = True,
            **sqlalchemy_create_engine_kwargs
    ):
        if port is None:
            connection_credentials = f'mysql://{username}:{password}@{host}/{database_name}'
        else:
            connection_credentials = f'mysql://{username}:{password}@{host}:{port}/{database_name}'
