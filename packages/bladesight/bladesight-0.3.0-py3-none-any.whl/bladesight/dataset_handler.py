...
def download_dataset_from_bladesight_data(dataset_path_on_s3 : str) -> None:
    ...
    with yaspin(text=f'Downloading {dataset_path_on_s3} from Bladesight Data...') as spinner:
...
class Dataset:
    ...
    def __getitem__(self, key : str):
        ...
        if table_name in self.tables:
            return self._read_sql(f"SELECT * FROM {table_name};", return_mode=self.dataframe_library)
        else:
            raise KeyError(
    ...
    def _read_sql(
        ...
        Args:
            ...
            return_mode (Literal["pd", 'pl'], optional): The return mode. Defaults to 'pd'.
...
class BladesightDatasetDirectory:
    def __init__(self):
        ...
        self.local_datasets = [self.replace_path_prefix(i) for i in get_local_datasets()]
