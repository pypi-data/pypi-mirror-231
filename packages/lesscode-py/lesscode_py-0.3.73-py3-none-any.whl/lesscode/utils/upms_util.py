...
def upms_register(item, full_url):
    if options.rms_register_enable:
        ...
        client_res = get(url=f"{options.rms_register_server}/upms/client/fetchall",
                         params={"client_name": client_name}).json()
        if not client_res.get("data"):
            post(url=f"{options.rms_register_server}/upms/client/insert",
        ...
        if not resource_res.get("data"):
            res = post(url=f"{options.rms_register_server}/upms/resource/insert",
