...
def update_rcparams(target: PlotTarget = PlotTarget.print,
    ...
    # font_family = font_family or Defaults.font_family_sans.value
...
def get_filename(filename: str,
    ...
    """Create & return Path to store figure. Create folder if doesn't exist.
    """
    fldr = tempfile.TemporaryDirectory().name if folder is None else folder
    fldr = Path(fldr)
    fldr.mkdir(exist_ok=True)

    return fldr.joinpath(filename).with_suffix(extension)
...
def utc_str(d, tz=pytz.timezone,
            format_spec: str = '%Y-%m-%d %H:%M') -> str:
    """Pretty-print a timezone-aware datetime to e.g. '2017-05-25 10:31 (UTC+1)'.
    """
    # return f'{d.tz_convert(tz):%Y-%m-%d %H:%M} (UTC{int(d.tz_convert(tz).utcoffset().total_seconds() / 3600):+0})'
    return f'{d.tz_convert(tz):{format_spec}} (UTC{int(d.tz_convert(tz).utcoffset().total_seconds() / 3600):+0})'
