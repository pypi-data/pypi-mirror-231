...
from pandasai.dfagent.base import create_pandas_df_agent 
from os.path import dirname
from .dfagent.result import Result
...
class PandasAI(Shortcuts):
    ...
    def dfagent_run(self,prompt:str):
        charts_root_dir = ""
        if self._save_charts_path is not None:
            charts_root_dir = self._save_charts_path
        else:
             # define chart save directory
            charts_root_dir = dirname(dirname(dirname(__file__)))
        
        self._prompt_id = str(uuid.uuid4())
        folder_name = self._prompt_id
        ...
        # chart_save_path is image save path
        tempt:str = f"""Complete the requirements of the question, and do not generate drawing code by default unless the question requires drawing.
                question:{prompt}. 
                If question requires drawing a picture, save the picture in this directory {chart_save_path}."""
        ...
        try:
            ...
            if os.path.exists(chart_save_path):
                result.picpath = chart_save_path
            else:
                result.picpath = None
        ...
        with redirect_stdout(io.StringIO()) as output:
            ...
            while count < self._max_retries:
                try:
                    ...
                    count += 1
                    code = self._retry_run_code(code, e, multiple)
                    # Add save chart code
                    if self._save_charts:
                        obj = add_save_chart(
                            code, self._prompt_id, self._save_charts_path, not self._verbose
