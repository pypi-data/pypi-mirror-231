...
from e6py_aio.http.endpoints import (
    PostRequests,
    FlagRequests,
    NoteRequests,
    TagRequests,
...
class HTTPClient(PostRequests, FlagRequests, NoteRequests, TagRequests, PoolRequests):
    ...
    def __del__(self):
        ...
        loop.run_in_executor(None, self.close)
    ...
    async def download(self, route: RawRoute, path: str) -> bool:
        ...
        async with self.__session.request(
            route.method, route.url, headers=headers, stream=True
        ) as response:
    ...
    async def request(
        self, route: Route, data: dict = MISSING, **kwargs: Dict[str, Any]
    ) -> Any:
        """
