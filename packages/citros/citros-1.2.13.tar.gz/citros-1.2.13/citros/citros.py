...
class Citros:
    ...
    def git_pull(self):
        try:
            ...
            self.try_merge_selectively(repo, repo.active_branch.name, ex)
        except Exception as ex:
            self.handle_exceptions(ex, exit=True)
    ...
    def verify_citros_branch(self, batch_id=None):
        ...
        if self.CITROS_ENVIRONMENT == "CLUSTER":
            assert batch_id is not None, "batch_id not given while on CLUSTER."
            user_branch = self.batch.get_batch(batch_id)['userBranch']
        else:
            _, user_branch = self.get_git_info(self.USER_PROJ_DIR)
        
        if citros_branch != user_branch:
            self.print(f"Your current citros branch `{citros_branch}` is different " + \
                        f"than the user branch `{user_branch}`.", color='red')
        ...
        if self.branch_exists(branch_name, check_remote):
            ...
            if create_if_not_exist:
                ...
                msg = f"Branch `{branch_name}` does not exist in the citros repository " + \
                      f"(locally or on the remote). Would you like to create it? (y/n)"
                answer = input(msg)
                if answer == "y":
                    self.create_and_checkout(branch_name)
