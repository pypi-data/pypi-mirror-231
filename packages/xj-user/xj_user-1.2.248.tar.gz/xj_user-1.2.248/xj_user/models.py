...
class BaseInfo(models.Model):
    ...
    user_name = models.CharField(verbose_name='用户名', max_length=32, unique=True, blank=True, null=True,db_index=True, help_text='登录的用户名')
    nickname = models.CharField(verbose_name='昵称', max_length=128, blank=True, null=True, help_text='')
    full_name = models.CharField(verbose_name='姓名', max_length=128, blank=True, null=True, db_index=True,help_text='用户的姓名（未实名验证）')
    phone = models.CharField(verbose_name='手机', max_length=128, unique=False, blank=True, null=True, db_index=True,help_text='登录用的手机号')
    email = models.EmailField(verbose_name='邮箱', unique=False, blank=True, null=True, db_index=True,help_text='登录用的邮箱')
    user_type = models.CharField(verbose_name='用户类型', max_length=255, blank=True, null=True,choices=[('PERSON', '个人用户'), ('COMPANY', '企业用户'), ('ADMIN', '管理员用户')],help_text='PERSON 个人用户, COMPANY 企业用户')
    ...
    platforms_to_users = models.ManyToManyField(to='Platform', through="PlatformsToUsers",through_fields=("user", "platform"))
...
class UserApplyRecord(models.Model):
    ...
    user = models.ForeignKey(BaseInfo, verbose_name='发起审批用户', blank=True, null=True, db_constraint=False, on_delete=models.DO_NOTHING)
    verify_user = models.ForeignKey(BaseInfo, verbose_name='审批用户', on_delete=models.DO_NOTHING, related_name="verify_user_info", db_constraint=False, blank=True, null=True)
    ...
    reject_reason = models.CharField(verbose_name='拒绝理由', max_length=500, blank=True, null=True, default="", help_text='')
