...
from pyzjr.show import StackedImages
from pyzjr.video import VideoCap
import pyzjr.Z as Z
import pyzjr as pz

def transColor(color,fmt=pz.BGR2RGB):
    """在pyzjr中仅仅提供了BGR格式的常见颜色,它的原理只是进行了通道顺序的改变"""
    if fmt == 0 or fmt == 1:
        cvtcolor = np.array(color)[::-1]
        return tuple(cvtcolor)
