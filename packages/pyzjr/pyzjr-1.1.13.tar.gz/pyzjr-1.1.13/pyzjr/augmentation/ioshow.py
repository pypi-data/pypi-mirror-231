...
from pyzjr.core._utils import call_plugins
...
def imread(filename,is_gray=False):
    ...
    with FileUrl_context(filename) as fname:
        img = call_plugins('imread', fname, plugin=None)
    ...
    if is_gray:
        img = transcolor(img, RGB2GRAY)
    return img

def transcolor(img,mode):
