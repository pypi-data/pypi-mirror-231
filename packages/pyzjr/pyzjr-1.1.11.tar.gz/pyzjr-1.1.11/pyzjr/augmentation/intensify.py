...
from pyzjr.augmentation.ioshow import StackedImages

def crop(img, x_min, y_min, x_max, y_max):
    height, width = img.shape[:2]
    assert x_max > x_min, "[pyzjr]:x_max 必须大于 x_min"
    assert y_max > y_min, "[pyzjr]:y_max 必须大于 y_min"
    assert x_min >= 0, "[pyzjr]:x_min 不能小于0"
    assert x_max <= width, "[pyzjr]:x_max 不能大于图像宽度"
    assert y_min >= 0, "[pyzjr]:y_min 不能小于0"
    assert y_max <= height, "[pyzjr]:y_max 不能大于图像高度"

    return img[y_min:y_max, x_min:x_max]

def base_random_crop_block(height, width, crop_height, crop_width, h_start, w_start):
    """
    :param height:图像原始高度
    :param width:图像原始宽度
...
def random_flip(image):
    """随机翻转,0-垂直,1-水平,2-水平和垂直翻转"""
    flip_option = random.randint(0, 2)
    image = flip(image, flip_option)
    return image

def random_Centerzoom(img, zoom_factor_range=(1,3)):
    zoom_factor = random.randint(zoom_factor_range[0], zoom_factor_range[1])  # Generate a random integer
    zoomed_img = Centerzoom(img, zoom_factor)
    return zoomed_img

def random_crop(img, crop_height, crop_width):
    """随机裁剪"""
    height, width = img.shape[:2]
    assert height >= crop_height, "[pyzjr]:The height of the image is less than the cropping height"
    assert width >= crop_width, "[pyzjr]:The width of the image is less than the cropping width"
    h_start = random.random()  # Generate a random float in [0, 1)
    w_start = random.random()
    x1, y1, x2, y2 = base_random_crop_block(height, width, crop_height, crop_width, h_start, w_start)
    img = crop(img,x1, y1, x2, y2)
    return img

def random_brightness(img,brightness_range=(0.5, 2.0)):
    """随机亮度变化"""
    brightness_factor = random.uniform(brightness_range[0], brightness_range[1])
    adjusted_image = brightness(img,brightness_factor)
