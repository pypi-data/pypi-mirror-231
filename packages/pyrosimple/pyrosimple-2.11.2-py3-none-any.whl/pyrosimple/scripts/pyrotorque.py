...
class TCPEventListener(EventListener):
    def __init__(self, event_queue: queue.Queue, host="127.0.0.1", port=8989):
    ...
    def listen(self):
        ...
        self.log.debug(f"Listening for events on TCP {self.host}:{self.port}")
        while True:
            (clientsocket, address) = self.serversocket.accept()
            try:
                infohash = clientsocket.recv(40)
                event = clientsocket.recv(1024)
                self.event_queue.put((event, infohash))
                clientsocket.send(b"OK")
            except Exception as exc:
                self.log.error("Encountered error %s, attempting to continue", exc)
            clientsocket.close()
...
class UDPListener(EventListener):
    pass
...
class RtorrentQueueManager(ScriptBaseWithConfig):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.event_listener: Optional[EventListener] = TCPEventListener(
            self.event_queue
        )

    def receive_event(self, infohash: str, event_name: str) -> None:
        print(event_name, infohash)
    ...
    def run_forever(self):
        ...
        self.event_listener_thread = self.event_listener.start()
        # Start job scheduler
        while True:
            try:
                event, infohash = self.event_queue.get(timeout=self.POLL_TIMEOUT)
                print(event, infohash)
            except queue.Empty:
                if reload_config:
                    self.reload_jobs()
    ...
    def enable_event_listener(self):
        self.event_listener_thread = self.event_listener.start()
