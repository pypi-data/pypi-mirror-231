...
def blastn(blastn_exe, query_fasta, blastn_database, project_folder, n_threads, task):
...
## midori2, pr2 database
def create_database_from_fasta(fasta_file, project_folder, databases_path):
    ' Create new FASTA database using makeblastdb '
...
## midori2, pr2 database
def create_database_silva(fasta_file, project_folder, databases_path):
    ' Create new FASTA database using makeblastdb '
    ...
    ## write temporary fasta file to remove white spaces
    tmp_fasta = Path(db_folder).joinpath('tmp.fasta')
    f = open(tmp_fasta, 'w')
    for record in tqdm(SeqIO.parse(fasta_file, "fasta"), desc='Converting fasta'):
        f.write('>' + str(record.description).replace(' ', ';') + '\n')
        f.write(str(record.seq) + '\n')
    f.close()

    db_name = Path(db_folder).joinpath('db')
    subprocess.call(['makeblastdb', '-in', str(tmp_fasta), '-dbtype', 'nucl', '-out', str(db_name)])
...
blast_csv = '/Users/tillmacher/Desktop/APSCALE_projects/test_apscale/10_local_BLAST/BLAST_LMouR_tele02_apscale_OTUs_(05_25_23_09_32)/LMouR_tele02_apscale_OTUs_blastn.csv'
read_table = '/Users/tillmacher/Desktop/APSCALE_projects/test_apscale/7_otu_clustering/LMouR_tele02_apscale_OTU_table.xlsx'
project_folder = '/Users/tillmacher/Desktop/APSCALE_projects/test_apscale/'
blastn_database = '/Users/tillmacher/Desktop/APSCALE_projects/APSCALE_databases/DB_MIDORI_UNIQ_NUC_GB249_srRNA_BLAST'
filter_tresholds = [97, 95, 90, 85]
database_type = 'Midori2'
threshold_e_value = 0.0000005
...
def filter_blast_results(blast_csv, read_table, project_folder, blastn_database, filter_tresholds, database_type, threshold_e_value):
    ...
    blast_df = pd.read_csv(blast_csv, header=None, sep=';;', names=col_names, engine='python').fillna('NAN')
    ...
    if database_type == 'Midori2' or database_type == 'Custom NCBI':
        ...
        if taxids_xlsx.is_file() == True:
            taxid_df = pd.read_excel(taxids_xlsx, index_col=0).fillna('nan')
    ...
    elif database_type == 'pr2':
        for query, hits in tqdm(sorted_hits_dict.items()):
            for hit in hits:
                taxonomy = []
                for i in range(2,8):
                    try:
                        taxonomy.append(hit[0].split(';')[i])
                    except IndexError:
                        taxonomy.append('')
                hit[0] = taxonomy
            ...
            for hit in hits:
                ...
                for i in range(3,8):
                    try:
                        ...
                        taxonomy.append('')
                try:
                    taxonomy.append('{} {}'.format(hit[0].split(';')[9], hit[0].split(';')[10]))
                except IndexError:
                    taxonomy.append('')
    ...
    for query, hits in tqdm(sorted_hits_dict.items()):
        ...
        ## filter taxon according to similarity thresholds
        filtered_list = []
        for taxonomy in taxonomy_list_set:
            # reduce taxonomy according to JAMP
        ...
        ## trim taxon to most recent common taxon
        taxon =[]
        ambigous_taxa = []
        for i in range(0,6):
            taxa = list(set([j[i] for j in filtered_list]))
            if len(taxa) != 1:
                taxon.append('')
                if i == 5:
                    ambigous_taxa.extend(taxa)
            else:
                taxon.append(taxa[0])

        ## cound the number of taxa and test if there is a dominant taxon present
        most_abundant_taxa = {}
        for t in ambigous_taxa:
            most_abundant_taxa[t] = [i[5] for i in taxonomy_list].count(t)

        # if so, identify the most abundant taxon
        dominant_taxon = ''
        if most_abundant_taxa != {} and len(set(most_abundant_taxa.values())) != 1:
            dominant_taxon = max(most_abundant_taxa, key=most_abundant_taxa.get)

        ## count number of ambigous taxa and number of genera
        n_ambigous_taxa = len(ambigous_taxa)
        n_genera = len(set([i.split(' ')[0] for i in ambigous_taxa]))
        ...
        ## SPECIES spA/spB
        ambigous_species = ['']
        if n_ambigous_taxa == 2 and n_genera == 1:
            flag = ['F2 - Two species, one genus']
            genus = list(set([i.split(' ')[0] for i in ambigous_taxa]))[0]
            ambigous_species = '{} {}'.format(genus, '/'.join([i.split(' ')[1] for i in ambigous_taxa]))
            taxon[5] = ambigous_species
            out = [query] + taxon + [similarity] + [database_type] + ['; '.join(ambigous_taxa)] + flag

        ## MULTIPLE AMBIGOUS SPECIES OF ONE GENUS
        elif n_ambigous_taxa >= 2 and n_genera == 1:
            flag = ['F3 - Multiple species of one genus']
            taxon[5] = list(set([i.split(' ')[0] for i in ambigous_taxa]))[0] + ' sp.'
            out = [query] + taxon + [similarity] + [database_type] + ['; '.join(ambigous_taxa)] + flag

        ## MULTIPLE AMBIGOUS SPECIES FROM MULTIPLE GENERA
        elif dominant_taxon != '':
            flag = ['F1 - Dominant taxon']
            t = [i for i in taxonomy_list_set if i[5] == dominant_taxon][0]
            out = [query] + t + [similarity] + [database_type] + ['; '.join(list(most_abundant_taxa.keys()))] + flag

        ## MULTIPLE GENERA OF AMBIGOUS TAXA
        elif ambigous_taxa != []:
            flag = ['F4 - Multiple genera']
            out = [query] + taxon + [similarity] + [database_type] + ['; '.join(ambigous_taxa)] + flag

        ## ALL REMAINING CASES
        else:
            flag = ['']
            out = [query] + taxon + [similarity] + [database_type] + [''] + flag

        ## add OTU to dict
        hit_dict_2[query] = out
    ...
    for OTU in tqdm(read_table_otus):
        if OTU in hit_dict_2.keys():
            blast_df_out_list.append(hit_dict_2[OTU])
        else:
            blast_df_out_list.append([OTU] + ['No Match']*6 + [0] + [database_type, '', 'F6'])
