...
def RGB2SegmentationID(RGB: np.ndarray) -> np.ndarray:
    ...
    for i in range(RGB.shape[0]):
        for j in range(RGB.shape[1]):
            seg_id = RGB2SegIDMapping().get(tuple(RGB[i, j]), 256)
...
@lru_cache(maxsize=1)
def RGB2SegIDMapping() -> Dict[Tuple[int, int, int], int]:
    """return mapping between rbg tuples to segmentation IDs

    Returns:
        Dict[Tuple[int, int, int], int]: mapps (r,g,b) -> seg_id
    """
    mapping = {}
    from airgen import __path__ as airgen_path

    with open(
        os.path.join(airgen_path[0], "asset", "rgb2segID.txt"), "r", encoding="utf-8"
    ) as f:
