...
def handle_columns(df, continuous_threshold=0.05, categorical_threshold=0.05):
    ...
    df * -- Dataframe name
    continuous_threshold = 0.05 (default), threshold to determine whether a column is continuous or not
    categorical_threshold = 0.05 (defaut), Threshold to determine whether a column is categorical or not
    
    Function takes input of a dataframe and computes the ratio of unique values to total values. Using this ratio and threshold, it classifies
    every column of the dataframe as numerical or categorical. May have bugs.
    This function is useful as when reading data from any flatfile/excel file, the column datatypes may be read wrong
    
    *** Ensure there is no overlap between the 2 given thresholds. 
    0 <categorical_threshold < continuous_threshold < 1 - Follow this rule always ***
    ...
    stoke = handle_columns(stroke_prediction, continuous_threshold = 0.06, categorical_threshold = 0.04)
    print(stroke.info())
    """
    continuous_columns = []
    categorical_columns = []
...
def predict_effect(df_in, outcome, model_in = LinearRegression(), type = 1,test_split = 0.2, random_st = 343, neural_net = 1, fname = 'feature_ranking', predict = 0):
    """ Predict effect function  -  This function will compute pre-requisite values for a dataset and predict the ranking of features against an outcome columm.
    ...
    *** Check Copyright details for usage permissions ***
    ...
    It will calculate pre-requisites using the input dataframe and input model. Here pre-requisites are referred to as column values which the predictive ML model will use to predict the ranking
    These are a set of performance based columns depending on type of classification.
    Based on computed values and type of ML(regression or classification), it will then predict the ranks of the features using pre-trained model and then produce feature ranking for the dataset.
    ...
    #Using predict effect function predict the ranking of the features against an output comlumn
    predict_effect(stroke,'stroke',rfclass,type = 2,neural_net=0,fname = 'feature_ranking_rf160_v1')

    #Using pre-defined neural network model to rank features against an output column for binary classification.
    ...
    if type == 1: #Regression dataset prediction
        ...
        for ind, row in columns_info.iterrows():
            ...
            reg_prereq.loc[len(cls_prereq)] = reg_prereq_row
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if type ==1:#Regression prediction
        ...
        reg_model ="Models/Model_note/neuralnetwork_Regress_model_39.999.h5"
        reg_trained_model = joblib.load(reg_model)
        ...
        reg_prereq.sort_values(['predicted'], ascending=False, inplace=True)
        prediction = reg_prereq['missing_feature','predicted'].values
        ...
        cls_model = "Models/Model_note/neuralnetwork_Classify_model_13.91.h5"
        cls_trained_model = tf.keras.models.load_model(cls_model)
...
def neural_network_build(df, X_train,y_train, outcome, predict = 0, epochs = 100):
    ...
    Function module for Neural network ML Model. User should adjust this model as per needs. They can add/remove/edit layers, neurons, activation function
    their datasets.
    This module will then be called inside the main module. The main module (regress_data) has a key to indicate if the model needs to take Neur
    function to be used or not. When using neural network model inside the main module, some input parameters of the main model will be redundan
