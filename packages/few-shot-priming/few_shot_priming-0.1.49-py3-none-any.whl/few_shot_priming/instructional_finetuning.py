...
def run_instructiona_fine_tuning_strategy(config, params, experiment, validate, logger, sampling_strategy):
    all_test_preds = []
    all_test_labels = []
    cutoff_len = 256
    ...
    if validate:
        test_data = val_data
    for test_index, _ in dataset[experiment_type].iterrows():
        if sampling_strategy == "similar":
            few_shots = sample_similar(test_index, similarities, dataset["training"], dataset[experiment_type])
        else:
            few_shots = sample_diverse(test_index, similarities, dataset["training"], dataset[experiment_type])
