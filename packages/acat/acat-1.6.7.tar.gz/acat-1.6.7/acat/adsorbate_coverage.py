    ...
    def populate_hetero_site_list(self):
        ...
        for adsid in self.ads_ids:            
            ...
            if self.subtract_heights is not None:
                ...
                dls = np.linalg.norm(np.asarray([s['position'] + s['normal'] * 1.
                                     - adspos for s in hsl]), axis=1)
            stid = np.argmin(dls)
            tmp_bl, st = dls[stid], hsl[stid]
...
class SlabAdsorbateCoverage(object):
    ...
    **'label'**: the updated label with the name of the occupying adsorbate
    ...
    .. code-block:: python
        ...
        [{'site': '3fold', 'surface': <acat.settings.CustomSurface object at 0x7f307239ccd0>, 
         ...
         {'site': 'bridge', 'surface': <acat.settings.CustomSurface object at 0x7f307239ccd0>, 
    ...
    def populate_hetero_site_list(self):
        ...
        for adsid in self.ads_ids:
            ...
            if self.subtract_heights is not None:
                ...
                _, dls = find_mic(np.asarray([s['position'] + s['normal'] * 1. 
                                  - adspos for s in hsl]), cell=self.cell, pbc=True)
            stid = np.argmin(dls)
            tmp_bl, st = dls[stid], hsl[stid]
