...
#        return
                ...
                for j in range(cols):
                    if vnames[j] != 'Vec':
                        ...
                        try:
                            ...
                            if silent == False:
                                print('Failed to create array. Ensure that column names are valid and that numpy has been imported.')
                            break
    ...
    def __init__ (self, name):
        ...
        global num_sheets, printer_inited
        self.mar = None
        oar = None

        if printer_inited == 0:
            printer_inited = 0 # it seems that if %reset -f is used in every run this might not work?
            # set up global function for displaying all sheets
            command = \
            "def display_sheets():\n"+\
            "    for i in globals():\n"+\
            "        if type(globals()[i]) == data_entry2.sheet:\n" +\
