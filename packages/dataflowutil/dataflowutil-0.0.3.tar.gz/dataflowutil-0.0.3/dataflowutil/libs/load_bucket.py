...
class LoadBucket:
    def __init__(self,connection,spreadsheets):
        ...
        self.data_config = spreadsheets.load_spreadsheets(self.cn.id_upload_data_to_bucket,extra_v.NAME_SHEET)
    ...
    def upload_files_bucket(self):
        try:
            for index,row in self.data_config.iterrows():
                ...
                self.spreadsheets.update_spreadsheets(self.cn.id_upload_data_to_bucket,extra_v.NAME_SHEET,f"D{index+1}",str(datetime.now()))
                self.spreadsheets.update_spreadsheets(self.cn.id_upload_data_to_bucket,extra_v.NAME_SHEET,f"C{index+1}",extra_v.STATUS_UPDATE["UPDATE"])
