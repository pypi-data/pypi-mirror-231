        ...
        try:
            ...
            for l in self.tags:  # list of list of polytags
                ...
                if os.path.isfile(l[0]['logo']):
                    self.image(l[0]['logo'], 10, nY-10, 5)
                # legalname
                self.set_font('Arial','B',5)
                self.text(15, nY-7, l[0]['dealer'])

                if len(l)>0:
                    if 'img' in l[0]:
                        if os.path.isfile(l[0]['img']):
                            self.image(l[0]['img'], x=90, y=nY-10, h=24, type='jpg')
                ...
                for m in range(len(l)):  # list of polytags
                    ...
                    self.text(60, nY-3, ' '.join(self.tag['provider']))
                    
                    # SKU
                    self.set_font('Arial','B',8)
                    self.text(10, nY, self.tag['sku'])
                    # Codebar
                    if len(self.tag['EAN13'])>0:
                        self.EAN13(20,nY-4,self.tag['EAN13'],h=4)

                    # Rate
                    self.set_font('Arial','B',16)
                    self.text(57, nY+2, ' '.join(self.tag['rate']))
                    self.set_font('Arial','',6)
                    self.text(60, nY+4, 'IVA INCLUIDO')
                    # Product name
                    self.set_font('Arial','B',10)
                    line = []
                    y_line = 7
                    for s in self.tag['name'].strip():
                        if len(s.strip()) == 0 and len(line)>30:
                            self.text(10, nY+y_line, ''.join(line))
                            line = []
                            y_line += 3
                        else:
                            line.append(s)
