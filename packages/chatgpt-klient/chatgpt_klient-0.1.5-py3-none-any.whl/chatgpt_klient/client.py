...
from chatgpt_klient.consts import MAX_TOKENS, LEGACY_ENGINES, CHAT_ENGINES, MAX_DELAY
...
class ChatGPTPrompt:
    ...
    def list_models(self):
        return LEGACY_ENGINES + CHAT_ENGINES
    ...
    def check_api_key_validity(self):
        try:
            ...
            r = self.check_response_validity(r, "davinci")
        except Exception:
            logger.error("Invalid API key.")
            raise InvalidAPIKeyError("Your API key does not seem to be valid. Please check it.")
        else:
    ...
    def check_model_validity(self):
        ...
        try:
            if self.engine in LEGACY_ENGINES:
                r = self.openai.Completion.create(
                    model=self.engine,
                    prompt="Hi",
                    max_tokens=5,
                ...
                if type(r) is not dict:
                    ...
                    logger.debug(f'Model validity test: {r["choices"][0]["text"]}')
            elif self.engine in CHAT_ENGINES:
                ...
                logger.debug(f'Model validity test: {r["choices"][0]["message"]["content"]}')
            else:
                raise InvalidModelError(f"Engine {self.engine} not supported")
        except Exception:
    ...
    def check_response_validity(self, r, engine=None) -> dict:
        if engine is None:
            engine = self.engine
        try:
    ...
    def send_prompt(self, text=None, max_tokens=None, no_history=False, delay=5):
        ...
        if self.engine in LEGACY_ENGINES:
            ...
            try:
                ...
                if delay > MAX_DELAY:
                    raise Exception("Recurring RateLimitError and delaying requests not working")
                time.sleep(delay)
                response = self.send_prompt(text, no_history=no_history, delay=delay * 2)
            except openai.InvalidRequestError:
                logger.exception("We shouldn't be getting here!")
            except requests.Timeout:
                logger.warning(f"Request failed with timeout, retrying")
                if delay > MAX_DELAY:
