...
class MakeSvgInstance:
        ...
        for i in range(xr):
            ...
            if self.chart_type == "Transit" or self.chart_type == "Composite":
                ...
                if i == 6:
                    ...
                    text_y_k = +10
                xtext = self.__sliceToX(0, (r-text_k), text_offset) + text_k + text_x_k
                ytext = self.__sliceToY(0, (r-text_k), text_offset) + text_k + text_y_k

                if self.chart_type == "Transit":
                    path = path + f'<text style="fill: {linecolor}; fill-opacity: 0; font-size: 1em"><tspan x="' + \
                    ...
                    path = path + f'<text style="fill: #000000; fill-opacity: .8; font-size: 1em; font-family: Cinzel; font-weight: bold"><tspan x="' + \
                        str(xtext)+'" y="'+str(ytext) + \
                        '">'+str(house_number)+'</tspan></text>'
                    path = path + '<line x1="'+str(t_x1)+'" y1="'+str(t_y1)+'" x2="'+str(t_x2)+'" y2="'+str(
                        t_y2)+'" style="stroke: '+t_linecolor+f'; stroke-width: 2px; stroke-opacity:.4;" class="house" id="{house_by_index[i+1]} House"/>'
            if self.chart_type == 'Transit' or self.chart_type == 'Composite':
                text_k = 90
            else:
                text_k = 53
    ...
    def __makePlanets(self, r):
        ...
        for e in range(len(keys)):
            ...
            if not too_close:
                ...
                output += f'<text transform="rotate({rotate})" text-anchor="{textanchor}" style="fill: white; font-size: 10px;">'
    ...
    def makeTemplate(self):
        ...
        if self.chart_type == "Transit" or self.chart_type == "Composite":
            ...
            td['transitRingAxes'] = self.__makeAxis(r, (r-self.c1+18.4), self.t_user.axis_list, 9.7, 'black', True)
            ...
            td['c4style'] = 'fill: %s; fill-opacity: 1; stroke: %s; stroke-width: 1px' % (
                self.colors_settings['paper_1'], self.colors_settings['zodiac_transit_ring_0'])
            td['makeAspects'] = self.__makeAspectsTransit(r, (self.c4))
