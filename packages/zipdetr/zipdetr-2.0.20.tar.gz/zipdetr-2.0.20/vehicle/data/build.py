...
class InfiniteDataLoader(dataloader.DataLoader):
    ...
    def __len__(self):
        ...
        if self.mode == 'train':
            ...
            number_batch = math.ceil(len(large_idx) / self.batch_size)
            return number_batch
        else:
            return len(self.batch_sampler.sampler)
...
def get_weights(dataset):
        ...
        for k, v in split_map.items():
            if data_name in v["id_list"]:
                if data_name in ["000155", "000156"]:
                    ...
                    image_weight[str(image_id) + "_" + k] = v["weight"] * 2
                elif k == "less" and data_name not in ["000116", "000117"]:
                    image_weight[str(image_id) + "_" + k] = v["weight"] * 1.5
    ...
    for image_k in image_weight.keys():
        ...
        if k not in alpha_weights.keys():
            alpha_weights[k] = image_weight[image_k]
