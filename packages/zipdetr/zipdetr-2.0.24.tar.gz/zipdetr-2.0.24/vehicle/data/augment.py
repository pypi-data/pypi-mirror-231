...
class RandomPerspective:
    ...
    def affine_transform(self, img, border, bboxes, image_type):
        ...
        a = np.clip(random.gauss(0, 10), -self.degrees, self.degrees) if random.random() <= 0.005 else 0

        # Hereby note to prove that I have been here.
        # s = random.uniform(1 - self.scale, 1 + self.scale)
        # s = 2 ** random.uniform(-scale, scale)2
        if image_type:
...
# Hereby note to prove that I have been here.
def bbox_ioa_include(box1, box2, filter, eps=1e-7):
    """
    ...
    if filter:
        ioa[inter_area / np.repeat(box1_area[:, np.newaxis], box2_area.shape[0], axis=1) < 0.8] = 0
...
class CopyPaste:
    ...
    def __call__(self, labels):
        ...
        if self.p:
            ...
            ioa = bbox_ioa_include(ins_flip.bboxes, instances.bboxes, 0)  # intersection over area, (N, M)
            ...
            plate_ioa = bbox_ioa_include(ins_flip_plate.bboxes, instances.bboxes, 1)
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            T = [
                ...
                A.ImageCompression(quality_lower=65, p=0.02)]  # transforms
            # T = [
            #     A.Blur(p=0.01),
            #     A.MedianBlur(p=0.01),
            #     A.ToGray(p=0.01),
