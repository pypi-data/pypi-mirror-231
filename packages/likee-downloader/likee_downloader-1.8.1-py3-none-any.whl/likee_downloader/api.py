...
class API:
    ...
    def make_post_request(payload, endpoint, content_type="json") -> dict:
        ...
        if response.status_code != 200:
            print(f"{glyph.no_entry} HTTP error: {response.status_code}")
            return {}
        # Convert response to json and check for error message
        response = response.json()
        if "message" in response and response.get("message") != "ok":
            print(f"{glyph.no_entry} API Error: {response.get('message')}")
            return {}
        elif "msg" in response and response.get("msg") != "success":
            print(f"{glyph.no_entry} API Error: {response.get('msg')}")
            return {}
        else:
            return response
    ...
    def capture_screenshot(self, username: str):
        ...
        print(
            f"{glyph.camera} Capturing profile screenshot ({username})... Please wait."
        )
        self.driver.get(self.user_profile_url % username)
        self.driver.get_screenshot_as_file(file_path)

        print(f"{glyph.camera_with_flash} Screenshot captured: {file_path}")
    ...
    def get_user_id(self, username: str) -> int:
        ...
        """
        print(f"{glyph.information} Obtaining userId ({username})... Please wait.")
        ...
        print(f"{glyph.check_mark_button} userId obtained: {user_id}")
        return user_id
    ...
    def get_user_videos(
        ...
        print(f"{glyph.information} Getting {limit} videos... Please wait.")
        response = self.make_post_request(payload, self.user_vids_endpoint)
        videos.extend(response.get("data").get("videoList"))
        if limit:
            self.pause()
            if len(videos) < limit and len(response) > 0:
                last_id = videos[-1].get("postId")
    ...
    def download_video(video_data: dict, username: str):
        ...
        response = requests.get(video_data.get("videoUrl").replace("_4", ""), stream=True)
        total_size_in_bytes = int(response.headers.get("content-length", 0))
        block_size = 1024 * 1024  # 1 Megabyte

        with Progress() as progress:
            task_id = progress.add_task(
                f"{glyph.information} Downloading: {video_id}.mp4",
        ...
        print(f"{glyph.check_mark_button} Downloaded: {file_path}\n")
