...
class TemplatingEngine:
    ...
    async def process_request(self, request: dict[str, Any]) -> str:
        ...
        # Convert param indexes to name.
        if method and method.param_structure is ParamStructure.BY_NAME:
            request["params"] = _get_converted_param_names(
                method.params, request["params"]
            )
...
def _get_converted_param_names(
    method_params: list[ContentDescriptor], params: dict[str, Any]
) -> dict[str, Any]:
    """Get params with names from index."""
    return {
        method_params[param].name: params[i] for i, param in enumerate(method_params)
    }
