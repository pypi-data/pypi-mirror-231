...
def sphereGaussianCorona(q, R, Rg, Ncoil, coilequR, nu=0.5, coilSLD=0.64e-4, sphereSLD=4.186e-4, solventSLD=6.335e-4, d=1):
    ...
    This is not immediatly visible as the :math:`Q^{-2}` at high Q results from the second dominating term.
    The gnerealized Gaussian F(Q) is always positive and does not cross zero.
...
def multiShellDisc(q, radialthickness, shellthickness, shellSLD, solventSLD=0, alpha=None, nalpha=60):
    ...
    shellthickness : list of float or float, all >=0
        Thickness of shells from inner to outer along disc axis, units nm, same length as radialthickness.
         - Innermost thickness is only taken once (core).
    ...
    **Contrast matched bicelle** in SANS (see [2]_ for details).
    ...
    ::
     ...
     hbicelle = js.ff.multiShellDisc(q, radialthickness=[R,head], shellthickness=[th-2*head,head],
                                                     shellSLD=hsld, solventSLD=6.335*1e-4)
     dsld = np.r_[7.39,5.05]*1e-4  # scattering length densities full deuteration
     dbicelle = js.ff.multiShellDisc(q, radialthickness=[R,head], shellthickness=[th-2*head,head],
                                                     shellSLD=dsld,solventSLD=6.335*1e-4)
     dsld67 = np.r_[6.65,6.65]*1e-4  # scattering length densities DMPC d67
     dbicelle67 = js.ff.multiShellDisc(q, radialthickness=[R,head], shellthickness=[th-2*head,head],
...
def multiShellCylinder(q, L, shellthickness, shellSLD, solventSLD=0, alpha=None, h=None, nalpha=60, ncap=31):
    ...
    This allows to approximate continous profiles by step profiles with large number of shells.
...
def pearlNecklace(Q, N, Rc=None, l=None, A1=None, A2=None, A3=None, ms=None, mr=None, vmonomer=None,
    ...
    For Rc==0 we have no pearls, but we have a neglace of linear linkers like connected infinitly thin rods.
    ...
    The formfactor describes a short neglace e.g. from magnetic nanoparticles building a chain
    or spheres connected by bonds.
    In the graph the modulations in mid Q range are interferences between neigboring beads.
    ::
...
def _idealhelix(qz, qp, N, R, P, n):
    dn = np.r_[1:int(N) - 1]
    fh = 1 + (2*(1-dn/N) * np.cos(qz[..., None]*dn*P/n) * special.j0(2 * qp[..., None] * R * np.sin(dn * np.pi / n)))
    return fh.sum(axis=-1)
...
def idealHelix(q, L=3, R=0.3, P=0.54, n=3.6*3, Rot=None):
    ...
    the α-helix [2]_ ::
     ...
     p = js.grace()
     for L in [1, 2, 5,10]:
         fq = js.ff.idealHelix(q,L=L, R=0.3, P=0.54, n=3.6*3)
         p.plot(fq)
    ...
    ::
     ...
     fq = js.ff.idealHelix(qe, L=3, R=0.3, P=0.54, n=3.6*3, Rot=R)  # same as Rot=[90,0]
     fq.Y=fq.Y-1
     fig=js.mpl.contourImage(fq, scale='log', invert_yaxis=1)
