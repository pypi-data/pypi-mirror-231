...
class HuaWeiHttpRequest:
    ...
    def handle_response(self, response):
        ...
        if status_code >= 200 and status_code < 300:
            ...
            logging.error(f"response status code: {status_code}, detail error message: {response.text}")
...
def invoke(ak, sk, method, url, data=None, headers=None):
    ...
    logging.info(f"第{times}次请求, response: {response}")
    return response
...
def loop_invoke_check_condition(ak, sk, method, url, condition, data=None, headers=None):
    ...
    while True:
        ...
        if ((symbol == "==") and (tmp_resp == value)) or ((symbol == "!=") and (tmp_resp != value)):
            logging.warning(f"第{times}次请求结果, 判断条件: {tmp_resp}{symbol}{value}成立, 退出循环")
            del tmp_resp
            break
        logging.info(f"第{times}次请求结果, 判断条件: {tmp_resp}{symbol}{value}不成立, 等待下次请求...")
        time.sleep(10)
        times += 1
    return response
