...
class HuaWeiHttpRequest:
    ...
    def handle_response(self, response):
        ...
        if status_code >= 200 and status_code < 300:
            ...
            logging.error(f"response status code: {status_code}", response.reason)


def get_invoke_response(ak, sk, method, url, data=None, headers=None):
    hw_request = HuaWeiHttpRequest(ak, sk)
    resp = hw_request.request(method, url, data, headers)
    return resp
