...
class IoK8sApiCoreV1ServiceAccountTokenProjection(BaseModel):
    ...
    """
    audience: Optional[StrictStr] = Field(None, description="Audience is the intended audience of the token. A recipient of a token must identify itself with an identifier specified in the audience of the token, and otherwise should reject the token. The audience defaults to the identifier of the apiserver.")
    expiration_seconds: Optional[StrictInt] = Field(None, alias="expirationSeconds", description="ExpirationSeconds is the requested duration of validity of the service account token. As the token approaches expiration, the kubelet volume plugin will proactively rotate the service account token. The kubelet will start trying to rotate the token if the token is older than 80 percent of its time to live or if the token is older than 24 hours.Defaults to 1 hour and must be at least 10 minutes.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> IoK8sApiCoreV1ServiceAccountTokenProjection:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> IoK8sApiCoreV1ServiceAccountTokenProjection:
        """Create an instance of IoK8sApiCoreV1ServiceAccountTokenProjection from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return IoK8sApiCoreV1ServiceAccountTokenProjection.parse_obj(obj)

        _obj = IoK8sApiCoreV1ServiceAccountTokenProjection.parse_obj({
