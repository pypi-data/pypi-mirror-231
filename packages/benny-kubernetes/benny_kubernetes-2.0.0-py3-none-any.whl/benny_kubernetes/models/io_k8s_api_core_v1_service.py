...
class IoK8sApiCoreV1Service(BaseModel):
    """
    Service is a named abstraction of software service (for example, mysql) consisting of local port (for example 3306) that the proxy listens on, and the selector that determines which pods will answer requests sent through the proxy.  # noqa: E501
    """
    ...
    spec: Optional[IoK8sApiCoreV1ServiceSpec] = None
    status: Optional[IoK8sApiCoreV1ServiceStatus] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> IoK8sApiCoreV1Service:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> IoK8sApiCoreV1Service:
        """Create an instance of IoK8sApiCoreV1Service from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return IoK8sApiCoreV1Service.parse_obj(obj)

        _obj = IoK8sApiCoreV1Service.parse_obj({
            ...
            "spec": IoK8sApiCoreV1ServiceSpec.from_dict(obj.get("spec")) if obj.get("spec") is not None else None,
            "status": IoK8sApiCoreV1ServiceStatus.from_dict(obj.get("status")) if obj.get("status") is not None else None
        })
        return _obj
