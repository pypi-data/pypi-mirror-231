...
class IoK8sApiCoreV1Endpoints(BaseModel):
    ...
    subsets: Optional[conlist(IoK8sApiCoreV1EndpointSubset)] = Field(None, description="The set of all endpoints is the union of all subsets. Addresses are placed into subsets according to the IPs they share. A single address with multiple ports, some of which are ready and some of which are not (because they come from different containers) will result in the address being displayed in different subsets for the different ports. No address will appear in both Addresses and NotReadyAddresses in the same subset. Sets of addresses and ports that comprise a service.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> IoK8sApiCoreV1Endpoints:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> IoK8sApiCoreV1Endpoints:
        """Create an instance of IoK8sApiCoreV1Endpoints from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return IoK8sApiCoreV1Endpoints.parse_obj(obj)

        _obj = IoK8sApiCoreV1Endpoints.parse_obj({
            ...
            "subsets": [IoK8sApiCoreV1EndpointSubset.from_dict(_item) for _item in obj.get("subsets")] if obj.get("subsets") is not None else None
        })
        return _obj
