...
class IoK8sApimachineryPkgApisMetaV1APIVersions(BaseModel):
    ...
    server_address_by_client_cidrs: conlist(IoK8sApimachineryPkgApisMetaV1ServerAddressByClientCIDR) = Field(..., alias="serverAddressByClientCIDRs", description="a map of client CIDR to server address that is serving this group. This is to help clients reach servers in the most network-efficient way possible. Clients can use the appropriate server address as per the CIDR that they match. In case of multiple matches, clients should use the longest matching CIDR. The server returns only those CIDRs that it thinks that the client can match. For example: the master will return an internal IP CIDR only, if the client reaches the server using an internal IP. Server looks at X-Forwarded-For header or X-Real-Ip header or request.RemoteAddr (in that order) to get the client IP.")
    versions: conlist(StrictStr) = Field(..., description="versions are the api versions that are available.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> IoK8sApimachineryPkgApisMetaV1APIVersions:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> IoK8sApimachineryPkgApisMetaV1APIVersions:
        """Create an instance of IoK8sApimachineryPkgApisMetaV1APIVersions from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return IoK8sApimachineryPkgApisMetaV1APIVersions.parse_obj(obj)

        _obj = IoK8sApimachineryPkgApisMetaV1APIVersions.parse_obj({
