...
def _get_stream_resolver_input_type(
    ...
    if isinstance(param, StreamResolverParamMessage):
        ...
        elif isinstance(param.typ, DataFrameMeta):
            stream_input_annotation = cast(Type[DataFrame], param.typ)
...
def _validate_keys(
    stream_fn: Callable[P, T], keys: dict[str, Any], params: Sequence[StreamResolverParam]
) -> dict[str, Any]:
    stream_fqn = get_resolver_fqn(function=stream_fn)
...
def _validate_timestamp(stream_fn: Callable[P, T], timestamp: str, params: Sequence[StreamResolverParam]):
    stream_fqn = get_resolver_fqn(function=stream_fn)
    input_model_arg = next(
        param for param in params if isinstance(param, (StreamResolverParamMessage, StreamResolverParamMessageWindow))
    )
