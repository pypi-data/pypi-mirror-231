...
class View3dWindow:
  ...
  def __createInternal(self):
    ...
    Also check for GLES supported extension
    '''
    ...
    if glfw.extension_supported('GL_ARB_ES3_compatibility'):
      logging.info('Extension supported:%s', 'GL_ARB_ES3_compatibility')
      View3dWindow.__GLES_LEVEL = 3      
    else:       
      logging.info('Using GLES 120 Extension')
      View3dWindow.__GLES_LEVEL = 1
    ...
    #     NumberOfExtensions = glGetIntegerv(GL_NUM_EXTENSIONS)
    #     for i in range(0, NumberOfExtensions):
    #       ccc = glGetStringi(GL_EXTENSIONS, i)
    #       print(str(ccc))
  ...
  def ogl_draw(self):
    ...
    for mesh in self.meshes.values():
      if mesh.isVisible:        
        mesh.rendering(self.win, self._MVP, self.ViewMatrix, self.ProjectionMatrix, View3dWindow.__GLES_LEVEL)
