...
from .consume import RabbitMQConsumerForExchangePool
...
class RpcServer:
    ...
    def __init__(
        ...
        self._producer: RabbitMQProducerForExchangePool = RabbitMQProducerForExchangePool(
            pool_size, url, self._exchange_name
        )

        self._consumer: RabbitMQConsumerForExchangePool = RabbitMQConsumerForExchangePool(
            pool_size, url, self._queue_name, self._exchange_name
        )
...
class RpcClient:
    ...
    def __init__(self, pool_size: int, url: yarl.URL, exchange_name: str, request_timeout: int = 60):
