    ...
    if member is not None:
        ...
        for role in reversed(sorted(member.roles)):
            if role in BUILTIN_ROLES:
                li.insert(idx, f"qqguild:{BUILTIN_ROLES[role]}")
                li.insert(idx, f"qqguild:g{guild_id}:{BUILTIN_ROLES[role]}")
                if role == 5:  # 频道管理员
                    li.insert(idx, f"qqguild:c{channel_id}:{BUILTIN_ROLES[role]}")
                    li.insert(idx, f"qqguild:g{guild_id}:c{channel_id}:{BUILTIN_ROLES[role]}")
            else:
