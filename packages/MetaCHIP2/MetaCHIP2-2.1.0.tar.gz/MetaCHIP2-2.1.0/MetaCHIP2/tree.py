...
MetaCHIP2 tree -o get_tree -i gnm_dir -x fna -t 12 -f -db /home-user/wzsong/DB/GTDB
MetaCHIP2 tree -o get_tree -i gnm_dir -x fna -t 12 -f -db /home-user/wzsong/DB/GTDB -c gnm_taxon.txt

# you need to have the following files in the db dir (-db)
ar53_r214.tree
bac120_r214.tree
ar53_metadata_r214.tsv
bac120_metadata_r214.tsv
...
def RootTree(db_dir, user_gnm_taxon, user_gnm_tree_bac, user_gnm_tree_ar, user_tree_rooted):
    ...
    if (contain_bac_gnm is True) and (contain_ar_gnm is False):
        root_tree_by_gtdb(gtdb_ref_tree_bac, gtdb_gnm_meta_bac, user_gnm_taxon, user_gnm_tree_bac, user_tree_rooted, 'bac')

    # root archaeal tree
    elif (contain_bac_gnm is False) and (contain_ar_gnm is True):
        root_tree_by_gtdb(gtdb_ref_tree_ar,  gtdb_gnm_meta_ar,  user_gnm_taxon,  user_gnm_tree_ar,  user_tree_rooted, 'ar')

    elif (contain_bac_gnm is True) and (contain_ar_gnm is True):

        # root bacterial tree
        user_tree_rooted_bac = '%s.bac' % user_tree_rooted
        root_tree_by_gtdb(gtdb_ref_tree_bac, gtdb_gnm_meta_bac, user_gnm_taxon, user_gnm_tree_bac, user_tree_rooted_bac, 'bac')

        # root archaeal tree
        user_tree_rooted_ar  = '%s.ar'  % user_tree_rooted
        root_tree_by_gtdb(gtdb_ref_tree_ar,  gtdb_gnm_meta_ar,  user_gnm_taxon,  user_gnm_tree_ar,  user_tree_rooted_ar, 'ar')
...
def tree(args):
    ...
    gtdb_gnm_meta_ar        = '%s/ar53_metadata_r214.tsv'                   % db_dir
    gtdb_gnm_meta_bac       = '%s/bac120_metadata_r214.tsv'                 % db_dir
    gtdb_ar53_summary_tsv   = '%s/gtdb.ar53.summary.tsv'                    % output_dir
    gtdb_bac120_summary_tsv = '%s/gtdb.bac120.summary.tsv'                  % output_dir
    gtdb_summary_tsv        = '%s/gtdb_classifications.tsv'                 % output_dir
    msa_bac120_gz           = '%s/align/gtdb.bac120.user_msa.fasta.gz'      % output_dir
    msa_bac120              = '%s/align/gtdb.bac120.user_msa.fasta'         % output_dir
    msa_ar53_gz             = '%s/align/gtdb.ar53.user_msa.fasta.gz'        % output_dir
    msa_ar53                = '%s/align/gtdb.ar53.user_msa.fasta'           % output_dir
    user_tree_rooted        = '%s/rooted.tree'                              % output_dir
    ...
    if gnm_tax_txt is None:
        ...
        # run classify_wf
        cmd_classify_wd = 'gtdbtk classify_wf --cpus %s --pplacer_cpus 1 --skip_ani_screen --genome_dir %s --extension %s --out_dir %s --prefix gtdb' % (num_threads, input_gnm_dir, file_extension, output_dir)
    ...
    if os.path.isfile(msa_bac120_gz) is True:
        cmd_infer_bac120 = 'gunzip %s; gtdbtk infer --msa_file %s --out_dir %s --cpus %s --prefix bac120' % (msa_bac120_gz, msa_bac120, output_dir, num_threads)
        with open(log_txt, 'a') as log_txt_handle:
            log_txt_handle.write(cmd_infer_bac120 + '\n')
        os.system(cmd_infer_bac120)

    # infer ar tree
    if os.path.isfile(msa_ar53_gz) is True:
        cmd_infer_ar53 = 'gunzip %s; gtdbtk infer --msa_file %s --out_dir %s --cpus %s --prefix ar53' % (msa_ar53_gz, msa_ar53, output_dir, num_threads)
        with open(log_txt, 'a') as log_txt_handle:
            log_txt_handle.write(cmd_infer_ar53 + '\n')
        os.system(cmd_infer_ar53)

    ######################################## root tree ########################################

    if gnm_tax_txt is None:
        gnm_tax_txt = gtdb_summary_tsv
    RootTree(db_dir, gnm_tax_txt, inferred_bac120_tree, inferred_ar53_tree, user_tree_rooted)
...
submitHPC.sh --cmd "python3 tree.py -o gnm_5bac_5ar_get_tree_wd -i gnm_5bac_5ar -x fna -t 12 -f -db /home-user/wzsong/DB/GTDB" -n 9 -c tree
