...
class DAG(nx.DiGraph):
    ...
    def Cutnodes(self,output=None):
      ...
      output_gate = list(flow_graph.pred[output])[0]
      #cutnodes.add(output_gate)
      queue.append(output_gate)
      ...
      output gate
      """
      nets = list(flow_graph.pred[output_gate])
      for net in nets:
          flow_graph.remove_edge(net,output_gate)
      # find `immediate dominators` of each node for 
      # the `flow graph`
      idom = nx.immediate_dominators(flow_graph,
                                     output_gate)
      """each node is a child of `immediate dominator`
      in the `dominator tree`"""
      dominator_tree = DAG()
      # remove output gate as its immediate dominator is 
      # itself
      del idom[output_gate]
