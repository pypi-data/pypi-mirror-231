...
class Database(object):
    ...
    def batch_insert_live_production(self,database, table, df):
        try:
            conn, cur = self.connect(database)
            if conn is None or cur is None:
                return

            tuples = [tuple(x) for x in df.to_numpy()]
            cols = ','.join(list(df.columns))
            query = f"INSERT INTO {table}({cols}) VALUES %s ON CONFLICT (start_time, identifiant, prm, element, provider, type, model_type, data_type) DO NOTHING"
            extras.execute_values(cur, query, tuples)
            conn.commit()
            print("Data inserted")
        except Exception as error:
