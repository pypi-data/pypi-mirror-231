...
class SyncScheduleSnowflakeTask(SubscriptableBaseModel):
    ...
    Args:
        ...
        task_warehouse_type (Literal["fixed", "serverless"]): The type of warehouse to use for the sync's task.
        warehouse (Optional[str]): The Snowflake warehouse which this task uses, if task_warehouse_type=fixed
    ...
    task_warehouse_type: Literal["fixed", "serverless"] = "serverless"
...
class SyncScheduleDbt(SubscriptableBaseModel):
    ...
    Args:
        ...
        task_warehouse_type (Literal["fixed", "serverless"]): The type of warehouse to use for the sync's task. If task_warehouse_dbt_defined is True, this is only an initial setting before being overwritten by the dbt run
        warehouse (Optional[str]): The Snowflake warehouse which this task uses, if task_warehouse_type=fixed
...
class SyncScheduleManual(SubscriptableBaseModel):
    ...
    Args:
        task_warehouse_type (Literal["fixed", "serverless"]): The type of warehouse to use for the sync's task.
        warehouse (str): The Snowflake warehouse which this task uses, if task_warehouse_type=fixed
    """
