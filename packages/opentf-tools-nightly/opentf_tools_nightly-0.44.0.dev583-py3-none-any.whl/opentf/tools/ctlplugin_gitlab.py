...
TARGET_API_PATH = '/api/v4/projects/{project_id}/{target}/{target_id}/notes'
...
def publish_results(
    ...
    - 'target_id': issue_iid or merge_request_iid ($CI_MERGE_REQUEST_ID)
    - ['token'],
    - ['update'],
    - ['cats']
    ...
    """
    if not all(map(gl_params.get, ('server', 'project', 'target', 'target_id'))):
        _error(
    ...
    target = gl_params['target']
    target_id = gl_params['target_id']
    update = gl_params.get('update') is not None
    token = gl_params.get('token')
    cats = gl_params.get('cats') is not None

    path = TARGET_API_PATH.format(
        project_id=project_id, target=target, target_id=target_id
    )
