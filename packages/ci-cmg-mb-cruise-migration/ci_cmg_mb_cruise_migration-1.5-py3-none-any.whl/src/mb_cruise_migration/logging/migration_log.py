...
class MigrationLog(object):
    ...
    def log_survey_start(survey_crate: CruiseSurveyCrate):
        ...
        MigrationLog.log.info(f"{LogConsts.BEGIN} -- Survey Migration: {survey_name}")
    ...
    def log_cruise_processor_unhandled_error(cruise_cargo: [CruiseCargo], error_message):
        ...
        MigrationLog.log.error(f"Unhandled error occurred during dataset insertion. Potentially affected datasets include {datasets}. ERROR: {error_message}")

    @staticmethod
    def log_mb_survey_query(query):
        MigrationLog.log.info(f"Querying MB.SURVEY for next batch of surveys: \n\t\t{query}")
