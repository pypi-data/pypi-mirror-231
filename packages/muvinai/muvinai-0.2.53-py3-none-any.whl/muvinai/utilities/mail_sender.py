...
def send_mail_with_attachment(
    ...
    msg = {
        ...
        'to': to_mail,
        'global_merge_vars': global_vars if global_vars else [],
...
def send_mail_inactivo(receiver: dict, *, brand: dict, sede: str = None, test_mail='matias@muvinai.com'):
    ...
    if brand["name"].lower() != "sportclub":
        ...
        return send_mail(receiver["email"], global_vars, template, brand=brand, sede=sede, test_mail=test_mail)
    ...
    if receiver.get('status') == 'baja':
        if receiver.get('plan_corporativo', None):
            ...
            if (merchant := receiver.get('seller_merchant_id')) or (merchant := receiver.get('merchant_id')):
...
def send_mail(receiver_mail: str, params: list, template: str, *, brand: dict, sede: dict = None,
              test_mail="matias@muvinai.com", send_at=None):
    ...
    if not 'email_contacto' in [v['name'] for v in params]:
...
def send_mail_carrito_abandonado(receiver_mail: str, client_name: str, plan: dict,
    ...
    return send_mail(receiver_mail, global_vars, template, brand=brand, sede=sede, send_at=send_at, test_mail=test_mail)
...
def send_mail_cambio_tarjeta(receiver: dict, *, brand: dict, sede: dict = None):
...
def send_mail_bienvenida(receiver: dict, *, plan: dict, brand: dict, sede: dict = None,
                         test_mail: str = "ignacio@muvinai.com"):
...
def send_mail_exitoso(receiver: dict, *, plan: dict, brand: dict, sede: dict = None):
    global_vars = [
        {"name": "nombre", "content": receiver["nombre"]},
        {"name": "apellido", "content": receiver["apellido"]},
        {"name": "plan", "content": plan["name"]},
...
def send_mail_pago_pendiente(receiver: dict, *, brand: dict, sede: dict = None, test_mail="ignacio@muvinai.com"):
    global_vars = [
        {"name": "nombre", "content": receiver["nombre"]}
    ]
    if brand["name"].lower() == "sportclub":
...
def send_mail_cambio_tarjeta_aon(receiver: dict, *, brand: dict, sede: dict = None):
    """ Enviar mail indicando al cliente que debe cambiar la tarjeta.
...
def send_mail_vencimiento_anual(receiver: dict, *, brand: dict, sede: dict = None):
    """ Enviar mail indicando al cliente que debe cambiar la tarjeta.
...
def send_mail_cambio_plan_herenecia(receiver: dict, *, brand: dict, sede: dict = None):
...
def send_mail_baja(receiver: dict, *, template: str, brand: dict, sede: dict = None):
