...
def configure_tailwind(content_glob: str) -> int:
    ...
    if os.path.exists(TAILWIND_CONFIG.name):
        print("Detected existing tailwind.config.js, trying to update it")
    ...
    pkgprint("Tailwindcss configuration file created")
...
def main() -> int:
    ...
    if tailwind_code != 0:
        pkgprint("Remember to add the following glob to the content config:")
        pkgprint(f"'{config.full_templates_glob}'")
