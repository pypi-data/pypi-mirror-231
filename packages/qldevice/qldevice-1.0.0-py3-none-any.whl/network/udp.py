...
class UDPSender(object):
    ...
    def send(self, message):
        # print(f"UDPSender send content: {message}")
        try:
            self._socket.sendto(message, self._address)
        except Exception as e:
            print("UDPSender send 出现异常：")
            print(e)
...
class UDPReceiver(object):
    ...
    def accept(self):
        if self._port is None:
            print(f"UDPReceiver未设置监听端口！")
            return

        if self._run:
            print(f"UDPReceiver正在监听端口{self._port}, 本次操作已忽略。")
            return
