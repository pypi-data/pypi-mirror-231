...
class ThreadWorker(object):
        ...
        while not ciocore.common.SIGINT_EXIT:
            try:
                ...
                try:
                    ...
                    logger.debug("Worker in queue raised Empty (_job_counter={}, task_count={}".format(self._job_counter.value, self.task_count))
    ...
    def put_job(self, job):
        ...
        if job is None:
            logger.debug("Attempting to put job 'None' in out_queue ({} -> {}). Skipping.".format(self, self.out_queue))
            return
...
class JobManager():
    ...
    def add_task(self, task):
        ...
        for worker in self.workers:            
            worker.task_count += 1
            logger.debug("Incremented task count on worker {} to {}".format(worker, worker.task_count))
        
        self.task_count += 1
        logger.debug("Incremented task count on manager to {}".format(self.task_count))
