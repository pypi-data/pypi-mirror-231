...
def add_storage_node(cluster_id, iface_name, data_nics):
    ...
    for index, nvme in enumerate(nvme_devs):
        ...
        if not ret:
            ...
            continue
        p_size = int(nvme.size/split_count)
        for nvm_part in ret:
            ...
            for iface in snode.data_nics:
                if iface.ip4_address:
                    ...
                    IPs.append(iface.ip4_address)
            logger.info(f"add {pt_name} to subsystem")
            ret = rpc_client.nvmf_subsystem_add_ns(subsystem_nqn, pt_name)
            cluster_devices.append({
                "id": alceml_id,
                "dev_id": nvme.get_id(),
                "node_id": nvme.node_id,
                "name": alceml_name,
                "name_pt": pt_name,
                "size": p_size,
                "nqn": subsystem_nqn,
                "IPs": IPs, #  list of string
                "port": "4420",
                "status": "online"
            })
    ...
    for node_index, node in enumerate(snodes):
        if node.get_id() == snode.get_id() or node.status == node.STATUS_OFFLINE:
            continue
        for index, dev in enumerate(node.cluster_devices):
            name = f"rem_{dev['name']}"
            logger.info(f"Connecting to {name}")
            for ip in dev['IPs']:
                ret = rpc_client.bdev_nvme_attach_controller_tcp(name, dev['nqn'], ip, dev['port'])
            dev["remote_name"] = f"{name}n1"
    ...
    for node_index, node in enumerate(snodes):
        ...
        count = 0
        for dev in snode.cluster_devices:
            name = f"rem_{dev['name']}"
            for ip in dev['IPs']:
                ret = rpc_client.bdev_nvme_attach_controller_tcp(name, dev['nqn'], ip, dev['port'])
                if not ret:
    ...
    if snodes:
        snodes = snodes[::-1]
    cluster_map_data = get_distr_cluster_map(snodes)
    for snode in snodes:
...
def get_distr_cluster_map(snodes):
    map_cluster = {}
    map_prob = []
    for snode in snodes:
        dev_map = {}
        ...
        """
        index = 0
        for i, dev in enumerate(snode.cluster_devices):
            ...
            dev_w_map.append({
                ...
                "id": index})
            index += 1
        for i, dev in enumerate(snode.remote_devices):
            dev_w = int(dev['size'] / (1024 * 1024 * 1024)) or 1
            node_w += dev_w
            dev_map[f"{index}"] = {
                "UUID": dev['id'],
                "bdev_name": dev['remote_name'],
                "status": "online"}
            dev_w_map.append({
                "weight": dev_w,
                "id": index})
