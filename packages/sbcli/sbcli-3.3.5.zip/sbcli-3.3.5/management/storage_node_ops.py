...
def add_storage_node(cluster_id, iface_name, data_nics):
    ...
    for index, nvme in enumerate(nvme_devs):
        ...
        split_count = 4
        ret = rpc_client.bdev_split(nvme.nvme_bdev, split_count)
        if not ret:
            logger.error(f"Error splitting nvme bdev: {nvme.nvme_bdev}")
            continue
        p_size = nvme.size/split_count
        for nvm_part in ret:
            alceml_name = f"alceml_{nvm_part}"
            alceml_id = str(uuid.uuid4())
            logger.info(f"adding alceml: {alceml_name}")
            ret = rpc_client.bdev_alceml_create(alceml_name, nvm_part, alceml_id)
            cluster_devices.append({
                "id": alceml_id,
                "nvme_dev_id": nvme.get_id(),
                "name": alceml_name,
                "size": p_size,
                "status": "online"
            })
    snode.cluster_devices = cluster_devices

        # nvme.alceml_bdev = alceml_name

        # subsystem_nqn = snode.subsystem + ":dev:" + nvme.get_id()
        # logger.info("creating subsystem %s", subsystem_nqn)
        # ret = rpc_client.subsystem_create(subsystem_nqn, 'sbcli-cn', nvme.get_id())
        ...
        # logger.info(f"add {alceml_name} to subsystem")
        # ret = rpc_client.nvmf_subsystem_add_ns(subsystem_nqn, alceml_name)
    ...
    #         continue
    #     for index, nvme in enumerate(node.nvme_devices):
    #         nqn = node.subsystem + ":dev:" + nvme.get_id()
    #         alceml_name = f"rem_{(node_index + 1)}_alceml_{(index + 1)}"
    #         for iface in node.data_nics:
    #             if iface.ip4_address:
    #                 ret = rpc_client.bdev_nvme_attach_controller_tcp(alceml_name, nqn, iface.ip4_address)
    #                 nvme.alceml_bdev = f"{alceml_name}n1"
    #                 remote_devices.append(nvme)
    ...
    snodes = db_controller.get_storage_nodes()
    send_distr_cluster_map(snodes, rpc_client, snode.get_id())

    logger.info("Done")
    return "Success"


def send_distr_cluster_map(snodes, rpc_client, target_node_id):
    map_cluster = {}
    map_prob = []
    for snode in snodes:
        dev_map = {}
        ...
        """
        for index, dev in enumerate(snode.cluster_devices):
