...
def add_storage_node(cluster_id, iface_name, data_nics):
    ...
    for index, nvme in enumerate(nvme_devs):
        ...
        alceml_name = f"alceml_{(index + 1)}"
        logger.info(f"adding alceml: {alceml_name}")
        ret = rpc_client.bdev_alceml_create(alceml_name, nvme.nvme_bdev, nvme.get_id())
    ...
    for node_index, node in enumerate(snodes):
        if node.get_id() == snode.get_id():
            continue
        for index, nvme in enumerate(node.nvme_devs):
...
def send_distr_cluster_map(snodes, rpc_client, target_node_id):
    ...
    for snode in snodes:
        ...
        dev_w_map = []
        node_w = 0
        for index, nvme in enumerate(snode.nvme_devs):
            dev_w = int(nvme.size/(1024*1024*1024)) or 1
            node_w += dev_w
            dev_map[f"{index}"] = {
                "UUID": nvme.get_id(),
                "bdev_name": nvme.alceml_bdev,
                "status": "online"}
            dev_w_map.append({
                "weight": dev_w,
                "id": index})
