    ...
    *stop_event.lock().unwrap() = false;
    ...
    loop {
        ...
        if data_available>0 {
            ...
            data_available = (reader.read(&mut read_buff, conf.fifo_reading_buffer as u32).unwrap() / conf.min_packet)*conf.min_packet ;
...
impl NifpgaFastFifoRecv {
    ...
    #[new]
    #[pyo3(signature = (bitfile = "X", signature = "ABCD12345678", ni_address = "RI0", run = false, close_on_reset = false, fifo = 0, dma_buffer_size=50000, fifo_reading_buffer=10000, min_packet=1))]
    fn new(bitfile: &str,
           signature: &str,
           ni_address:  &str,
           run: bool,
    ...
    fn get_data(&mut self, py: Python) -> PyResult<PyObject> {
        ...
        {
            Ok(x) => Ok(x.to_object(py).into()),
            Err(_) => Ok(empty_vec.to_object(py).into())
        }
    }
