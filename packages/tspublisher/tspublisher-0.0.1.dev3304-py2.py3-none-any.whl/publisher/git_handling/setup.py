...
def setup_users_machine():
    ...
    while not login_success:
        email = get_email()
        if login(email, get_password(), create_rsa_return_pub_key()):
            login_success = True
    ...
    for repository, checkout_directory in get_repositories_to_checkout():
        clone_repo(repository, checkout_directory, name, email)
...
def get_email():
    print("Please enter TSE username here:")
    sys.stdout.flush()
    email = get_input()
    return email
...
def get_password():
    print("Please enter your TSE password here:")
    sys.stdout.flush()
...
def clone_repo(repository, directory, name, email):
    if not os.path.exists(directory):
        call_command_and_print_exception(
            [
                ...
                "protocol.version=2",
                "--config",
                "lfs.url=https://live.touchsurgery.com/api/v3/lfs",
                "--config",
...
def pull_repo(directory):
    with WorkingDirectory(directory):
        call_command_and_print_exception(
            ["git", "pull", "origin", "master"], "Git pull failure"
        )
