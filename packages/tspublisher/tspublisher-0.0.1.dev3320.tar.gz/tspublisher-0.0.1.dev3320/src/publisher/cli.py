...
from publisher import lock, procedure_content
...
class PublisherCLI:
    ...
    def __init__(self, args):
        ...
        if not tspublisher_up_to_date():
            ...
            if not get_yes_no(
                "Do you wish to continue with this outdated version? [y/N]: "
            ):
                sys.exit()
        ...
        if lock.acquire(blocking=False):
            ...
            print(
                "Another application is blocking use of tspub commands\n "
                "Most likely this will be the simulation editor\n "
                "If unable to resolve this issue contact pipeline"
            )
            sys.exit()
    ...
    def publish(self, args):
        ...
        self._validate_distgroups(distgroup)
    ...
    def generate_review_doc(self, args):
        ...
        parser.add_argument(
            "--output_dir",
            help="Directory to output generated report to",
            required=False,
        )
        parser.add_argument(
            "--number",
        ...
        args = parser.parse_args(args)
        procedure_content.generate_review_document_with_display(args.output_dir, args.number)
    ...
    def _get_user_decision():
        ...
        while user_input not in ['y', 'n']:
            print(
                "You have unsaved changes to {}. Would you save them now? "
                "If you do not save them they will be deleted".format(current_procedure)
            )
            print("> y or n:")
            sys.stdout.flush()
            user_input = input('')
    ...
    @staticmethod
    def _validate_distgroups(distgroups):
        if distgroups is None:
            return True

        groups = distgroups.split(",")

        if "Public" in groups and len(groups) != 1:
            raise ValueError(
                "Can not publish to Public and another distribution group "
                "at the same time"
            )
        elif "TS-Testing" in groups and len(groups) != 1:
            raise ValueError(
                "Can not publish to TS-Testing and another distribution group "
                "at the same time"
            )
        return True
