...
class ParserTests(unittest.TestCase):
    ...
    def test_tsToNum(self):
        parser = Parser(None)
        now = datetime.datetime.now()
        custom_property_repository = CustomPropertyRepository()
        custom_property_repository.add_custom_property(CustomProperty('cp1', CustomPropertyType.DATETIME, now))
        roxx_properties_extensions = PropertiesExtensions(parser, custom_property_repository)
        roxx_properties_extensions.extend()
        self.assertEqual(now.timestamp(), parser.evaluate_expression('tsToNum(property("cp1"))').value)

        custom_property_repository.add_custom_property(CustomProperty('cp2', CustomPropertyType.DATETIME, 'stringInsteadOfDate'))
        self.assertEqual(None, parser.evaluate_expression('tsToNum(property("cp2"))').value)

        self.assertEqual(None, parser.evaluate_expression('tsToNum(property("cp3"))').value)
