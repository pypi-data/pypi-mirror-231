...
class StateSender:
    ...
    include_platform_changes: bool

    def __init__(self, device_properties, request, flag_repository, custom_property_repository, error_reporter, environment, include_platform_changes = False):
        ...
        self.include_platform_changes = include_platform_changes if include_platform_changes is not None else False
    ...
    def serialize_feature_flags(self):
        ...
        for v in self.flag_repository.get_all_flags().values():
            ...
            f['name'] = v.name
            f['options'] = v.options
            if self.include_platform_changes:
                f['externalType'] = v.external_type()
            flags.append(f)
    ...
    def serialize_custom_properties(self):
        ...
        for v in self.custom_property_repository.get_all_custom_properties().values():
            ...
            p['name'] = v.name
            if not self.include_platform_changes:
                p['type'] = v.type.type
            properties.append(p)
    ...
    def get_cdn_path(self, properties):
        return '%s/%s' % (self.environment.CDN_STATE_PATH(), self.get_relative_path(properties))

    def get_api_path(self, properties):
        api_state_path = self.environment.API_STATE_PATH()
