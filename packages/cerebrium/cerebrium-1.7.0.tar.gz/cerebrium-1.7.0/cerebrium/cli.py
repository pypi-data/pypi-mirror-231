...
def model_scaling(
    ...
    cooldown: int = typer.Option(
        None,
        help="Update the cooldown period in seconds before an inactive replica of your deployment is scaled down.",
    ),
    ...
    if min_replicas is not None:
        assert min_replicas >= 0, "Minimum replicas must be a natural number."
    if max_replicas is not None:
        assert max_replicas >= 0, "Maximum replicas must be a natural number."
        if min_replicas is not None:
            assert (
