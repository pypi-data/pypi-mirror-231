...
class Meter(object):
    ...
    def decode(self, context: Context, index=0):
        ...
        # 异常判断 - 协议服务返回结果
        if data.get('error') == 1:
            raise ProtocolError(data.get('result'))
    ...
    def exec(self, context: Context, retry_times: int = 3):
        self.encode(context)
        result = base.send(context,
        ...
        try:
            ...
            retry_times -= 1
            if str(pe).find('FrameError') < 0 or retry_times <= 0:
                raise ProtocolError(context.tester.api)
            else:
                base.send(context, todo={'app:error': {'msg': base.FRAME_ERROR}})
                base.sleep(1)
                self.exec(context, retry_times=retry_times)
...
class ATS(object):
    ...
    def jump(self, context: Context):
        if isinstance(self._jp, dict) and len(self._jp) > 0:
            ...
            result = base.jump_result(context.runtime.sas[context.runtime.step], context.runtime.jump_times, times,
                                      step)
            base.send(context, todo={'app:show': {'msg': result}})
            context.runtime.sas[context.runtime.step] = result
