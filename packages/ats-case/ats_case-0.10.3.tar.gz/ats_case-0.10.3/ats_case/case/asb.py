...
def todo_channel(context: Context, data):
    ch = data.get('channel')
    if isinstance(ch, dict):
        if ch.get('type') == '485' and isinstance(context.runtime.channel.rs485, int):
            ch['baudrate'] = context.runtime.channel.rs485
        if ch.get('type') == '红外' and isinstance(context.runtime.channel.ir, int):
            ch['baudrate'] = context.runtime.channel.ir
        if ch.get('type') == '载波' and isinstance(context.runtime.channel.plc, int):
            ch['baudrate'] = context.runtime.channel.plc
