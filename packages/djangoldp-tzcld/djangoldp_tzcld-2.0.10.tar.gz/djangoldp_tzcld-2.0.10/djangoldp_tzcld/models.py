...
class TzcldProfilesMembership(Model):
    ...
    class Meta(Model.Meta):
        ...
        verbose_name_plural = _("TZCLD Membership types")
        anonymous_perms = []
...
class TzcldTerritoriesTrainingPromotion(Model):
    ...
    class Meta(Model.Meta):
        ...
        verbose_name_plural = _("TZCLD Training Promotions")
        anonymous_perms = []
        authenticated_perms = ['view']
        owner_perms = ['inherit']
        superuser_perms = ['inherit']
...
class TzcldCommunityIdentity(Model):
    ...
    class Meta(Model.Meta):
        ...
        serializer_fields = ['@id', 'community', 'deputy', 'circonscription', 'origin_mobilization', 'application_date', 'signatory_structure', 'birth_date', 'last_contribution_date', 'territories_project_team_memebers', 'territories_trainings']
...
class TzcldCommunityEvaluationPointAnswer(Model):
    ...
    class Meta(Model.Meta):
        ...
        serializer_fields = ['@id', 'answer', 'answer_community_deliberation', 'answer_other_community_deliberation', 'answer_concil_department_deliberation', 'comment', 'evaluation_point', 'community']
...
class TzcldSharedNote(Model):
    ...
    VISIBILITY_REFERENTS = 'referents'
    VISIBILITY_CHOICES = [
        (VISIBILITY_ADMINS, 'Territory administrators'),
        (VISIBILITY_REFERENTS, 'Referents'),
    ]

    user = models.ForeignKey(settings.AUTH_USER_MODEL, null=True, on_delete=models.SET_NULL)
    longdesc = models.TextField(blank=True, null=True)
    community = models.ForeignKey(Community, on_delete=models.CASCADE, related_name='tzcld_community_shared_notes', null=True, blank=True)
    conversations = models.ManyToManyField(Conversation, blank=True, related_name='tzcld_shared_notes')
    visible = models.CharField(
        max_length=25,
        choices=VISIBILITY_CHOICES,
        default=VISIBILITY_REFERENTS,
    )
    ...
    class Meta(Model.Meta):
        ...
        serializer_fields = ['@id', 'user', 'longdesc', 'community', 'visible', 'conversations', 'date']
...
class TzcldTerritoryRequest(Model):
    ...
    class Meta(Model.Meta):
        ...
        serializer_fields = ['@id', 'user', 'date', 'contactType', 'subject', 'community']
...
class TzcldTerritorySynthesisFollowed(Model):
    ...
    community = models.ForeignKey(Community, on_delete=models.DO_NOTHING,related_name='tzcld_community_synthesis_followed', blank=False, null=True)
    ...
    class Meta(Model.Meta):
        ...
        serializer_fields = ['@id', 'context', 'strongPoints', 'questions', 'needs', 'targetdate', 'community']
...
class TzcldTerritorySharedFile(Model):
    ...
    community = models.ForeignKey(Community, on_delete=models.DO_NOTHING,related_name='tzcld_community_shared_files', blank=False, null=True)
    visible = models.CharField(
        max_length=25,
        choices=VISIBILITY_CHOICES,
        default=VISIBILITY_REFERENTS,
