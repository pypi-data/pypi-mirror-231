...
class AbstractOktaUser(AbstractBaseUser, PermissionsMixin):
	...
	def __str__(self):
		...
		Used by the admin application to get the "ID" column for each record.
		'''

		return self.login
	
	@classmethod
	def _pre_process_okta_attribute(cls, okta_attribute, model_field):
		'''Okta attribute pythonic value
		Further converts Okta attributes into the types required by the model (over the basic type assignment that the okta module does).

		Returns the "converted" Okta attribute.
		'''

		if isinstance(okta_attribute, str):
			if isinstance(model_field, models.fields.DateTimeField):
				return datetime.datetime.fromisoformat(okta_attribute.rstrip('Z'))
		return okta_attribute

	@classmethod
	def attributes_from_okta_profile(cls, okta_profile):
		'''Filter Okta profile attributes
		Goes through the attributes on the profile provided and maps those that are present on the current model. It also performs the conversion of the values that needs it.

		Returns a dictionary with attribute:value that can be directly "updated" into a model.
		'''

		attributes = {}
		for field in cls._meta.fields:
			if hasattr(okta_profile, field.name):
				okta_attr = cls._pre_process_okta_attribute(getattr(okta_profile, field.name), field)
				if okta_attr is not None:
					attributes[field.name] = okta_attr
		return attributes

	@classmethod
	def from_okta_profile(cls, okta_profile):
		'''Constructor from Okta profile
		Builds a model instance from the provided Okta profile.
		'''

		return cls(**cls.attributes_from_okta_profile(okta_profile))

	def get_full_name(self):
		'''Django magic
		Used by Django authentication mechanism, mostly for UI purposes.
		'''
	...
	def update(self, **updated_values):
		'''Simple update
		The default is to create a QuerySet and then use its update method. This method allows the direct update of the model. It filters out unsupported attributes (with a warning message).
		'''
	...
	def update_from_okta_profile(self, okta_profile):
		'''Update from Okta profile
		Uses the attributes on the provided Okta profile to update the current model.
		'''

		return self.update(**self.attributes_from_okta_profile(okta_profile))
