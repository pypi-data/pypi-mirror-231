...
class SPConfig:
	'''SAML config
	Configuration structure for the "saml2" package.
	'''
	
	class OktaConfig(dict):
		'''Okta configuration
		Specific configuration for Okta SAML
		'''
		
		def __init__(self, request, django_settings = settings):
			'''Magic initialization
			Load the required values from the Django config and set some hard coded values.
			'''
	...
	def __new__(cls, request, django_config = settings):
		'''Magic factory
		Factory method that creates the configuration with the settings.
		'''
	...
	def next_url(cls, request, django_config = settings):
		'''Next URL logic
		An extension of the regular mechanism using also the SAML configuration.
		'''
...
def acs(request):
	...
	Final destination for the SAML 2.0 assertion.
	'''
...
def login_(request):
	'''Login view
	Implements support for SP initiated SAML logins.
	'''
...
def logout_(request):
	'''Logout view
	Simply logout the current user (if any).
	'''
...
def index(request):
	'''Sample page
	Just a "Lorem ipsum" page that requires the user to login.
	'''
