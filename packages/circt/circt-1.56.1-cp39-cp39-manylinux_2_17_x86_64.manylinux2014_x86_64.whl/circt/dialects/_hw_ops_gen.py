...
class HWModuleExternOp(_ods_ir.OpView):
  ...
  @builtins.property
  def portLocs(self):
    return self.operation.attributes["portLocs"]

  @portLocs.setter
  def portLocs(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["portLocs"] = value
...
class HWTestModuleOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, module_type, parameters, *, port_attrs=None, port_locs=None, comment=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_564')) else
      _ods_ir.AttrBuilder.get('anonymous_564')(module_type, context=_ods_context))
