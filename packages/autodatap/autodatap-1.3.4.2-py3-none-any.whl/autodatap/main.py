...
def mainMethod(linkToData):
    ...
    if linkToData == "":
        ...
        def detect_encoding(linkToData):
            with open(linkToData, 'rb') as f:
                result = chardet.detect(f.read())
                return result['encoding']

        def read_csv_with_encoding(linkToData):
            encoding = detect_encoding(linkToData)
            data = pd.read_csv(linkToData, delimiter=",", encoding=encoding)
            return data
