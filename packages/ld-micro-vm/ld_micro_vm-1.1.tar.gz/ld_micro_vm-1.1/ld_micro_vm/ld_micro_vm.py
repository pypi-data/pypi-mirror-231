...
class ld_micro_vm:
    def __init__ (self, plc_address, GPIO_OUT = [399,400,401],BGA = ["PI.00","PI.01","PI.02"]):
        self.lock = threading.Lock()
        file_check, self.plc_object, self.cycle_time = self.load_program(plc_address)
        self.pins = GPIO_OUT
        self.bga = BGA_Dict = dict(zip(GPIO_OUT,BGA))
        self.gpio_setup()
    ...
    def gpio_functions(self, bits):
        for i in range(len(pins)):
            if(bits[32+i]): #IO_BIT_OUT0 = 32
                gpio_set(self.pins[i])
            else:
                gpio_clear(self.pins[i])
    
    def gpio_setup(self):
    	for i in self.pins:
        	os.system("echo " + str(i) + ">/sys/class/gpio/export")
        	os.system("echo out > /sys/class/gpio/" + self.bga[i] + "/direction")
    
    def gpio_set(self,pin):
	    os.system("echo 1 >/sys/class/gpio/" + self.bga[i] + "/value")
    def gpio_clear(self,pin):
        os.system("echo 0 >/sys/class/gpio/" + self.bga[i] + "/value")
