...
from pitrix.fixture import db_config, db_cache, db_schema
from pitrix.utils.common import genson
from pitrix.exceptions import HttpRequestError
from pitrix.extension.retry import PitrixRetry
from pitrix.constants.path import project
from pitrix.extension.template import api_template
...
def response_callback(payload: dict):
    def inner(response: requests.models.Response, *args, **kwargs):
        ...
        if response.status_code >= 400:
            logger.error(_render_template(api_template, print_info))
            ...
            print_info = _render_template(api_template, print_info)
            std_logger(print_info)

            to_schema = genson(resp_body)
            db_schema.set(caller_of_method, to_schema)
...
def _get_api_frame():
    ...
    for frame_info in outer_frames[8:]:
        ...
        if str(project.apis_dis) in filename:
            return frame
...
class BaseApi:
    ...
    def request(self, method, url, **kwargs):
        ...
        if self.IS_HTTP_RETRY:
            for attempt in PitrixRetry(counts=self.HTTP_RETRY_COUNTS,
                                       interval=self.HTTP_RETRY_INTERVAL,
