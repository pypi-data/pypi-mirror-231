...
class PitrixException(Exception):
    pass


class NotFoundError(PitrixException):
    pass


class FileNotFound(FileNotFoundError, PitrixException):
...
class ConfKeyError(PitrixException):
...
class YamlKeyError(PitrixException):
...
class LoginError(PitrixException):
    def __str__(self):
        return "用例启动函数run未传入Login对象"


class HttpRequestError(PitrixException):
...
class JsonPathExtractFailed(PitrixException):
