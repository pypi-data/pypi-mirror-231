...
def import_file(
    ...
    if error_count > 0:
        raise ExitCodeException(
            ...
            exit_code=client.EXIT_CODE_IMPORT_FAILED,
        )
...
def main():
    ...
    try:
        ...
        for file in files:
            try:
                ...
                if err.exit_code == hubspot.EXIT_CODE_IMPORT_FAILED:
    ...
    if failed_uploads:
        ...
        for failed_upload in failed_uploads:
            hubspot.logger.error(failed_upload)
            sys.exit(hubspot.EXIT_CODE_IMPORT_FAILED)
