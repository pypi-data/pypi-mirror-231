...
class signlanguage:
    ...
    def Predict(self, data=None):
        try:
            ...
            return self.model_object_result(value_result=results)
        except Exception as e:
    ...
    def Evaluate(self, data=None):
        ...
        if isinstance(data, list):
            for idx, data_ in enumerate(data):
                rst_tmp = self.Predict(data=data_)
                
                if rst_tmp['found']:
                    rst_ejecucion.clear()
                    rst_tmp['state_evaluation'] = idx
                else: 
                    rst_ejecucion.append(data_)                    

                results_data.append(rst_tmp)

            if len(rst_ejecucion) > 0:
                results_data.append(self.Predict(data=rst_ejecucion))
        else:
            results_data.append(self.Predict(data=data))

        end_time = time.time()
        print(f"Evaluation ended in {(end_time - start_time):.2f} seconds.")
        print(f"------------------------------------------------------------------------------------")
