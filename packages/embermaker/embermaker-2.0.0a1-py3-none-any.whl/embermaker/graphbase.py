...
def vaxis(el: Element):
    ...
    drawaxis(egr, cx, cy, axname=axname, axunit=axunit, axside='left', has_vaxis=el.has_vaxis,
             has_grid=has_grid, nmticks=egr.gp['haz_axis_minorticks'])
    ...
    tbx = cx.c0 - egr.txspace - egr.lxticks * has_vline  # right-end of text strings
    if egr.gp.lst('haz_grid_lines'):
        ...
        for ic, haz in enumerate(egr.gp.lst('haz_grid_lines')):
            ...
            if hlp.hasindex(glends, ic):
                ...
                egr.c.rect(cx.c0 - egr.lxticks * has_vline, yp, cx.c + egr.lxticks * has_vline, sy,
                           stroke=None, fill=glcolor)
            else:
                # Grid-lines
                sy = 0.0
                egr.c.line(cx.c0 - egr.lxticks * has_vline, yp, cx.c1, yp, stroke=glcolor)
            if el.has_vaxis:
                ...
                # Add label
                egr.c.paragraph(tbx, sby, gllabel, align='right', valign="center", color='dgrey',
...
def drawaxis(egr, cx, cy, altaxfct=None, axname='', axunit='', axside='left',
    ...
    if axside == 'right':
        tbx = orbx + egr.txspace + egr.lxticks * has_vline  # start of text strings
        ollx = olbx
        orlx = orbx + egr.lxticks * has_vline  # end of lines (= drawing area edge + tick mark)
        talign = 'left'
    else:  # Left axis
        tbx = olbx - egr.txspace - egr.lxticks * has_vline  # end of text strings
...
def showboxes(el: Element):
    ...
    if gettrace and gettrace():  # and el.egr:  # If debug & this is not the root graph element
        ...
        if el.egr is not None:
            ...
            el.logger.addinfo(f"Element (not attached): {el._type}, parent: {el.parent_type}, children: {len(el.elements)}")
