...
class DataRobotExportBase:
    ...
    def __init__(
        ...
        if self._export_type == ExportType.PREDICTIONS:
            self._start_export = self._api.start_prediction_data_export
            self._get_export_dataset_ids = self._api.get_prediction_export_dataset_ids
        elif self._export_type == ExportType.ACTUALS:
            self._start_export = self._api.start_actuals_export
            self._get_export_dataset_ids = self._api.get_actuals_export_dataset_ids
        else:
            raise Exception(
                f"Unknown export type, allowed export types: {ExportType.all()}"
            )
    ...
    def _get_time_bucket_chunks(self) -> TimeBucketChunks:
        ...
        if self._use_cache:
            ...
            dataset_ids = (
                self._get_export_dataset_ids(self._deployment_id, data_export)
                if data_export
                else None
            )
