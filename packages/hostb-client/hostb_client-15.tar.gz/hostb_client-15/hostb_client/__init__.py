...
class Client(object):
    ...
    def upload_chunks(self, url, filename, data=None):
        ...
        if 'uploadUrl' in data:
            ...
            while chunk:
                ...
                remaining = ""
                if done:
                    bytes_uploaded = done - resume_offset
                    bytes_remaining = fsize - resume_offset
                    if bytes_uploaded and bytes_remaining:
                        r = math.ceil((elapsed / (bytes_uploaded/bytes_remaining) - elapsed)/60) * 60
                ...
                if len(chunk) < CHUNK_SIZE or f.tell() == fsize:
                    data['done'] = '1'
                try:
                    data = self._json_request(uploadUrl, data, files)
    ...
    def _json_request(self, url, data, files=None):
        result = {}
        try:
            request = self.s.post(url, data=data, files=files)
            result = request.json()
            return result
    ...
    def _json_request_2(self, url, form):
        result = {}
        try:
            body = form.body()
