from os import environ

if "PYGAME_HIDE_SUPPORT_PROMPT" in environ:
    import pygame
else:
    environ["PYGAME_HIDE_SUPPORT_PROMPT"] = "1"
    import pygame

del environ


letters = """qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM!@#$%^&*()'"_+=-/*.?\\|"""


class ScreenManager:
    def __init__(self, wh: tuple[int, int], title: str, _type: str = "2D"):
        """
        _type can be 2D/3D/All

        :param wh:
        :param title:
        :param _type:
        """
        pygame.init()
        pygame.font.init()
        if _type in ["2D", "3D", "All"]:
            self._type = _type
            self._levels = {}
        else:
            raise KeyError(f"_type must be 2D, 3D or All and not: {_type}")

        self._wh = wh
        self._cScreen = "start"
        self._running = True
        self._screens = {"screen:start": {}, "screen:all": {}}
        self._root = pygame.display.set_mode(wh)
        self._press = {"keyboard": {"start": {}}, "mouse": {"start": {}}}

        pygame.display.set_caption(title)

    def switchScreen(self, screen: str):
        self._cScreen = screen

    def quit(self):
        self._running = False

    def newScreen(self, _id: str) -> None:
        """
        Makes new screen.

        :param _id:
        :return:
        """
        self._screens[f"screen:{_id}"] = {}
        self._press["keyboard"][_id] = {}
        self._press[_id] = {}

    def CopyItem(self, source_id: str, dest_id: str):
        """
        Copy an item from the source screen to the destination screen.

        :param source_id: The ID of the item to copy (e.g., "screen:start.centerRect").
        :param dest_id: The ID of the destination item (e.g., "screen:main.centerRect").
        """
        source_path = source_id.split(".")
        dest_path = dest_id.split(".")

        # Check if both source and destination screens exist
        if len(source_path) == 2 and len(dest_path) == 2:
            source_screen = source_path[0]
            dest_screen = dest_path[0]

            if source_screen in self._screens and dest_screen in self._screens:
                source_item_id = source_path[1]

                # Check if the source item exists
                if source_item_id in self._screens[source_screen]:
                    source_item = self._screens[source_screen][source_item_id]

                    # Copy the item to the destination screen with a new ID
                    dest_item_id = dest_path[1]
                    self._screens[dest_screen][dest_item_id] = source_item.copy()

                else:
                    raise KeyError(f"Source item '{source_id}' not found.")
            else:
                raise KeyError("Source or destination screen not found.")
        else:
            raise ValueError("Invalid source or destination ID format.")

    def bind(self, screenOrLevel: str, key: str, command) -> None:
        """
        device arg can be "keyboard" or "mouse".
        :param command:
        :param screenOrLevel:
        :param key:
        :return:
        """

        path = screenOrLevel.split(":", 1)
        if path[0].find("screen") == 0:
            if screenOrLevel in self._screens:
                if key in letters+",0123456789 ":
                    self._press["keyboard"][path[1]][key] = command
        elif path[0].find("level") == 0:
            if screenOrLevel in self._levels:
                if key in letters+",0123456789 ":
                    self._press["keyboard"][path[1]][key] = command

    def Shape(self, level, _id, path: str = "shape:Box", size: tuple[int, int, int] = (1, 1, 1)):
        if self._type in ["3D", "All"]:
            self._levels[level][_id] = {"shape": path, "size": size}

    def Rect(self,
             parent, _id: str,
             xy: tuple[int, int] = (0, 0),
             wh: tuple[int, int] = (50, 50),
             _in: str = "color:white"
             ) -> None:
        """
        Adds rect to parent and binds it to an id.

        :param parent:
        :param _id:
        :param xy:
        :param wh:
        :param _in:
        :return:
        """
        if parent in self._screens:
            _rect = pygame.Rect(xy, wh)
            self._screens[parent][_id] = {"type": "rect", "data": _rect, "bg": _in, "show": False}
        else:
            raise KeyError("Screen not found in screen manager")

    def _font(self, font, size, text, fg):
        try:
            tmp = pygame.font.Font(font, size)
        except FileNotFoundError:
            tmp = pygame.font.SysFont(font, size)
        return tmp.render(text, True, self._getColor(fg))

    def Image(self, parent, _id, img, xy: tuple[int, int] = (0, 0), wh: tuple[int, int] = None):
        """
        Adds Image to parent and binds it to id.
        :return:
        """

        if parent in self._screens:
            img = pygame.image.load(img)
            if wh:
                img = pygame.transform.scale(img, wh)
            self._screens[parent][_id] = {"type": "img", "data": img, "pos": xy, "show": False}
        else:
            raise KeyError("Screen not found in screen manager")

    def Video(self):
        """
        (Not done yet)
        :return:
        """

    def Text(self,
             font: str = "Arial",
             text: str = "Sample Text.",
             color: str = "color:black",
             size: int = 12
             ) -> pygame.Surface:
        """

        :param font:
        :param text:
        :param color:
        :param size:
        :return:
        """
        try:
            return pygame.font.Font(font, size).render(text, False, self._getColor(color))
        except FileNotFoundError:
            try:
                return pygame.font.SysFont(font, size, True).render(text, False, self._getColor(color))
            except ValueError:
                return pygame.font.SysFont("Ariel", 12, True).render("Sample Text", False, self._getColor(color))

    def Button(self,
               parent,
               _id: str,
               xy: tuple[int, int] = (0, 0),
               wh: tuple[int, int] = (100, 50),
               _in: str = "color:white",
               _text: pygame.Surface = None,
               command=None) -> None:
        """
        Adds button to parent and binds it to an id.

        :param _text:
        :param parent:
        :param _id:
        :param xy:
        :param wh:
        :param _in:
        :param command:
        :return:
        """

        if parent in self._screens:
            _rect = pygame.Rect(xy, wh)
            if _in.find("img") == 0:
                d = _in.split(":", 1)[1]
                img = pygame.image.load(d)
                img = pygame.transform.scale(img, wh)
                self._screens[parent][_id] = {"type": "button", "data": img, "command": command, "show": False,
                                              "pos": xy, "rect": _rect, "text": _text}
            else:
                self._screens[parent][_id] = {"type": "button", "data": _rect, "command": command, "show": False,
                                              "bg": _in, "text": _text}

            if _text:
                text_rect = _text.get_rect()
                text_rect.center = _rect.center
                self._screens[parent][_id]["text_rect"] = text_rect

        else:
            raise KeyError("Screen not found in screen manager")

    def show(self,
             _id: str
             ) -> None:
        """
        Shows anything that the id is.

        :param _id:
        :return:
        """

        path = _id.split(".", 1)
        if len(path) == 2:
            self._screens[path[0]][path[1]]["show"] = True
        else:
            raise KeyError

    def hide(self,
             _id: str
             ) -> None:
        """
        Hides anything that the id is.

        :param _id:
        :return:
        """

        path = _id.split(".", 1)
        if len(path) == 2:
            self._screens[path[0]][path[1]]["show"] = False
        else:
            raise KeyError

    def delete(self,
               _id: str
               ) -> None:
        """
        Deletes anything that id is.

        :param _id:
        :return:
        """

        path = _id.split(".", 1)
        if len(path) == 2:
            del self._screens[path[0]][path[1]]
        else:
            raise KeyError

    def mainloop(self) -> None:
        while self._running:
            self._root.fill((0, 0, 0))
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self._running = False
                elif event.type == pygame.KEYDOWN:
                    if event.unicode in self._press["keyboard"][self._cScreen]:
                        self._press["keyboard"][self._cScreen][event.unicode]()
                elif event.type == pygame.MOUSEBUTTONUP:
                    for item in self._screens[f"screen:{self._cScreen}"]:
                        data = self._screens[f"screen:{self._cScreen}"][item]
                        if data["type"] == "button":
                            if type(data["data"]) == pygame.Rect:
                                if data["data"].collidepoint(event.pos):
                                    data["command"]()
                            else:
                                if data["rect"].collidepoint(event.pos):
                                    data["command"]()
            for s in [self._cScreen, "all"]:
                for item in self._screens[f"screen:{s}"]:
                    data = self._screens[f"screen:{s}"][item]
                    if data["show"]:
                        if data["type"] == "rect":
                            pygame.draw.rect(self._root, self._getColor(data["bg"]), data["data"])
                        elif data["type"] == "text":
                            self._root.blit(data["data"], data["pos"])
                        elif data["type"] == "button":
                            if type(data["data"]) == pygame.Rect:
                                pygame.draw.rect(self._root, self._getColor(data["bg"]), data["data"])
                                self._root.blit(data["text"], data["text_rect"].topleft)
                            else:
                                self._root.blit(data["data"], data["pos"])
                                self._root.blit(data["text"], data["text_rect"].topleft)
                        elif data["type"] == "img":
                            self._root.blit(data["data"], data["pos"])

            pygame.display.update()

        pygame.quit()
        pygame.font.quit()

    @staticmethod
    def _getColor(color: str) -> tuple[int, int, int]:
        if color.find("color") == 0:
            color = color.split(":")[1]
            colors = {
                "red": (255, 0, 0),
                "green": (0, 255, 0),
                "blue": (0, 0, 255),

                "purple": (255, 0, 255),
                "yellow": (255, 255, 0),
                "cyan": (0, 255, 255),

                "white": (255, 255, 255),
                "gray": (127, 127, 127),
                "black": (0, 0, 0)
            }
            finalColor = colors.get(color, (0, 0, 0))
        elif color.find("RGB") == 0:
            color = color.split(":")[1]
            safe = True
            for letter in letters:
                if letter in color:
                    safe = False

            if safe:
                finalColor = eval(f"({color})")
            else:
                finalColor = (0, 0, 0)
        elif color.find("HEX") == 0:
            try:
                color = color.split(":")[1]
                red = int(color[1:3], 16)
                green = int(color[3:5], 16)
                blue = int(color[5:7], 16)

                finalColor = (red, green, blue)
            except ValueError:
                finalColor = (0, 0, 0)

        else:
            finalColor = (0, 0, 0)

        return finalColor


def test():
    gui = ScreenManager((700, 500), "Test App")

    gui.Rect("screen:all", "bg", (500, 0), (200, 500), "HEX:#555555")
    gui.show("screen:all.bg")

    gui.Rect("screen:start", "bg", wh=(500, 500), _in="color:gray")
    gui.show("screen:start.bg")

    gui.mainloop()


if __name__ == '__main__':
    test()
