...
class BacSeg(QWidget):
    ...
    def __init__(self, viewer: napari.Viewer):
        ...
        self.undrift_progressbar = self.findChild(
            QProgressBar, "undrift_progressbar"
        )
        ...
        self.cellpose_invert_images = self.findChild(
            QCheckBox, "cellpose_invert_images"
        )
        ...
        self.upload_initial.currentTextChanged.connect(
            self.populate_upload_combos
        )

        self.upload_content.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_microscope.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_antibiotic.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_abxconcentration.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_treatmenttime.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_mount.currentTextChanged.connect(self.update_upload_combos)
        self.upload_protocol.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_strain.currentTextChanged.connect(
            self.update_upload_combos
        )
        self.upload_phenotype.currentTextChanged.connect(
            self.update_upload_combos
        )
    ...
    def _undrift_images(self):
        ...
        worker.signals.progress.connect(
            partial(self._Progresbar, progressbar="undrift")
        )
    ...
    def _undrift_postprocesing(self):
        try:
            ...
            boundary_image = np.min(
                self.viewer.layers[undrift_channel].data.copy(), axis=0
            )
            ...
            layer_names = [
                ...
                if layer.name not in ["center_lines"]
            ]

            for layer in layer_names:
                self.viewer.layers[layer].data = self.viewer.layers[
                    layer
                ].data[:, boundary[1] : boundary[3], boundary[0] : boundary[2]]
                image_shape = self.viewer.layers[layer].data.shape

                for i in range(image_shape[0]):
                    if layer not in ["center_lines"]:
                        try:
                            self.viewer.layers[layer].metadata[i]["dims"] = [
                                image_shape[-1],
                                image_shape[-2],
                            ]
                            self.viewer.layers[layer].metadata[i]["crop"] = [
                                0,
                                image_shape[-2],
                                0,
                                image_shape[-1],
        ...
        try:
            ...
            if layer_names != []:
                ...
                if len(image_shape) == 3:
                    if image_shape[0] > 1:
                        anchor_binary = self._undrift_preprocesing(
                            self.viewer.layers[undrift_channel].data[0]
                        )

                        for i in range(image_shape[0] - 1):
                            progress = int(
                                (i + 1) / (image_shape[0] - 1) * 100
                            )
                            progress_callback.emit(progress)

                            target_image = self.viewer.layers[
                                undrift_channel
                            ].data[i + 1]
                            target_binary = self._undrift_preprocesing(
                                target_image
                            )
                        ...
                        for layer in layer_names:
                            for i in range(image_shape[0] - 1):
                                shifted_image = scipy.ndimage.shift(
                                    self.viewer.layers[layer].data[i + 1],
                                    shift_list[i],
                                    cval=-1,
                                )

                                self.viewer.layers[layer].data[
                                    i + 1
                                ] = shifted_image
                        ...
                        #     frame = self.viewer.layers[layer].data[0].copy()
                        #
                        #     for i in range(image_shape[0] - 1):
                        #         if layer not in ["Segmentations", "Nucleoid", "Classes"]:
                        #             self.viewer.layers[layer].metadata[i]["dims"] = [frame.shape[-1], frame.shape[-2]]
                        #             self.viewer.layers[layer].metadata[i]["crop"] = [0, frame.shape[-2], 0, frame.shape[-1]]
        ...
        def _event(viewer):
            ...
            if os.path.isdir(path):
                ...
                worker.signals.result.connect(
                    partial(self.process_cell_statistics, path=path)
                )
                self.threadpool.start(worker)
