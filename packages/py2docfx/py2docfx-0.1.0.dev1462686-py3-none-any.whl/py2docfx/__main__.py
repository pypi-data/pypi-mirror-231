...
from py2docfx.convert_prepare.post_process.generate_xrefmap import generate_xrefmap
...
def donwload_package_generate_documents(
    ...
    for idx, package in enumerate(package_info_list):
        try:
            ...
            print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
            print("Caught exception as below")
            print(exception)
    if output_doc_folder:
...
def prepare_out_dir(output_root: str | os.PathLike) -> os.PathLike | None:
    # prepare output_root (if folder contains files, raise exception)
    if output_root:
...
def main(argv) -> int:
    ...
    generate_xrefmap(output_doc_folder or os.path.join("target_repo", "docs-ref-autogen"))
    return 0
