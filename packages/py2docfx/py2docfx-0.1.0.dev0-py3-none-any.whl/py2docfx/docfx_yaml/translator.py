            ...
            for class_node in class_nodes:
                if isinstance(class_node.children[0], desc_signature):
                    ...
                    if node_class == signature_child['fullname'] and node_module == signature_child['module']:
                        if (isinstance(class_node.children[1], desc_content)):
                            return class_node.children[1]
                        else:
                            raise Exception('Second child of class node isn\'t content, node: {0}'.format(class_node.astext()))
                else:
