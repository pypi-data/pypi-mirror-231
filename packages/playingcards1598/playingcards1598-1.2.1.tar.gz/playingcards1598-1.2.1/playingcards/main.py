...
class CardCollection:
    ...
    def ranking_counts_inv(self) -> dict[int, set[Rank]]:
        ...
        """
        rci = dict()
        for rank in self.ranking_counts:
            if rank in rci:
                rci[rank].add(rank)
            else:
                rci[rank] = {rank}
    ...
    def normalize_suits(self, suit_order: tuple[Suit, Suit, Suit, Suit] = (
            Suit('s', '♠'),
            Suit('h', '♥'),
            Suit('c', '♣'),
            Suit('d', '♦')
    ), inplace=False) -> 'CardCollection':
        """
        Returns a new CardCollection with the suits normalized to the suit_order
        ...
        if inplace:
            self.cards = [Card(card.rank, mapper[card.suit]) for card in self.cards]
        else:
            return self.__class__([Card(card.rank, mapper[card.suit]) for card in self.cards],
    ...
    def order_cards(self):
        if self.ordered:
            ...
            for i in range(len(self.cards)):  # Put pairs in order of mosuit
                if self.cards.count(self.cards[i]) > 1:
                    if self.suit_counts[self.cards[i]] < self.suit_counts[self.cards[i+1]]:
    ...
    def __add__(self, other) -> 'CardCollection':
        if other is None:  # TODO: HIGH PRIORITY Mutating the original object is such a bad idea. Need to find all usages of this and fix
            return self
        elif isinstance(other, Card):
            self.add_cards([other], position=len(self.cards))
            return self
        elif isinstance(other, CardCollection):
            # If the other collection is different to this one, then we need to return an instance of CardCollection.
            # Otherwise we can return an instance of this class
            if other.__class__ != self.__class__:
                return self.__class__(self.cards + other.cards, maximum=self.maximum, ordered=self.ordered, reverse_order=self.reverse_order)
            else:
                self.add_cards(other.cards, position=len(self.cards))
                return self
        else:
...
def main():
    deck = Deck()
    sample = deck.draw_top_n(3)
    print(sample + deck.draw_top_n(2))
