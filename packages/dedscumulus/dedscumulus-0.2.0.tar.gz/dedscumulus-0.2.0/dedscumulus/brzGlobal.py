...
def getSetting(CREDS, version, settingName, logger):
    try:
        ...
        log_text="brzGlobal.getSetting Failed, error: "+str(e1)
        logger.error(f"ERROR: {log_text}")
        return None

def getAssetIds(CREDS, logger):#deprecated, to be removed in time
    try:
        errors=0
        while errors<3:
            try:
                ...
                if len(assetIds)>0:
                    return assetIds
                log_text="brzGlobal.getAssetIds returned zero results"
                logger.warning(f"WARNING: {log_text}")
                return None
            except pyodbc.Error as pe:
                log_text="brzGlobal.getAssetIds Failed ("+str(errors)+"), error code: "+str(pe.args[0])+", retrying. Error text: "+str(pe)
                logger.debug(f"WARNING: {log_text}")
                errors+=1
                time.sleep(50)
    except Exception as e1:
        log_text="brzGlobal.getAssetIds Failed, error: "+str(e1)
        logger.error(f"ERROR: {log_text}")
        return None
...
def getAssetIds(CREDS, assetType, logger):
    try:
            ...
            try:
                ...
                with pyodbc.connect(conn_string) as conn:
                    with conn.cursor() as cursor:
                        ...
                        sqlSelect+=f"SELECT [AssetId], CAST(JSON_VALUE([SystemIds], '$.brz') AS INT), JSON_VALUE([Info], '$.ratedPower') AS [RatedPower], "
                        sqlSelect+=f"JSON_VALUE([Info], '$.installedCapacity') AS [installedCapacity] FROM [dbo].[mstAssets] WHERE [AssetTypeId]=@assetid AND JSON_VALUE([SystemIds], '$.brz') IS NOT NULL;"
        ...
        log_text="brzGlobal.getAssetIds (type) Failed, error: "+str(e1)
        logger.error(f"ERROR: {log_text}")
        return None
...
def getAssetPerSiteIds(CREDS, siteIds, logger):
    try:
        ...
        log_text="brzGlobal.getAssetPerSiteIds Failed, error: "+str(e1)
        logger.error(f"ERROR: {log_text}")
        return None
...
def getPowerCurve(CREDS, assetId, logger):
    try:
        ...
        log_text="brzGlobal.getPowerCurve Failed, error: "+str(e1)
        logger.error(f"ERROR: {log_text}")
        return None
