    ...
    for _ in range(5):
        ...
        try:
            ...
            if dt.datetime.strptime(result['time'], "%Y-%m-%d %H:%M:%S") > (dt.datetime.utcnow() + dt.timedelta(hours=8)):
                ...
                str_wind_info = get_cwb_wind_data(location['lat'], location['lng'], result["time"], lang)
                print('eco_ask_3w_handler/fetched wind data, fetching aiot data', flush=True)
                str_device_info = get_aiot_rawdata(location['lat'], location['lng'], result["time"], image_name, lang)
