...
import ayena
from ayena import version
from ayena.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    if args.api_key is not None:
        ayena.api_key = args.api_key
    if args.api_base is not None:
        ayena.api_base = args.api_base
    if args.organization is not None:
        ayena.organization = args.organization
    if args.proxy is not None:
        ayena.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                ayena.proxy['https'] = proxy
            elif proxy.startswith('http'):
                ayena.proxy['http'] = proxy

    try:
        args.func(args)
    except ayena.error.OpenAIError as e:
        display_error(e)
        return 1
