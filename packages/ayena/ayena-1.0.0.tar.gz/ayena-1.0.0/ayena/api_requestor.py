...
import ayena
from ayena import error, util, version
from ayena.openai_response import OpenAIResponse
from ayena.util import ApiType
...
def _requests_proxies_arg(proxy) -> Optional[Dict[str, str]]:
    ...
    if proxy is None:
        ...
        raise ValueError(
            "'ayena.proxy' must be specified as either a string URL or a dict with string URL under the https and/or http keys."
        )
...
def _make_session() -> requests.Session:
    if ayena.requestssession:
        if isinstance(ayena.requestssession, requests.Session):
            return ayena.requestssession
        return ayena.requestssession()
    if not ayena.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; ayena always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(ayena.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or ayena.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(ayena.api_type)
        )
        self.api_version = api_version or ayena.api_version
        self.organization = organization or ayena.organization
    ...
    def request_headers(
        ...
        if ayena.app_info:
            user_agent += " " + self.format_app_info(ayena.app_info)
    ...
    def _prepare_request_raw(
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "OpenAI bindings. Please contact us through our help center at help.ayena.com for "
                "assistance." % (method,)
            )
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(ayena.proxy),
            "timeout": timeout,
        }
        try:
...
async def aiohttp_session() -> AsyncIterator[aiohttp.ClientSession]:
    user_set_session = ayena.aiosession.get()
    if user_set_session:
