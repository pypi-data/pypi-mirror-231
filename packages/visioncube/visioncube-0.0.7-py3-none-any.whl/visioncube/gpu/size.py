...
class Resize(AbstractTransform):
    ...
    def __init__(
        ...
        Args:
            height: Height, 缩放高度, [0, 1000], 224
            width: Width, 缩放宽度, [0, 1000], 224
    ...
    def __call__(self, sample):
        ...
        if sample.image is None or self.to_h is None or self.to_w is None:
            return sample
        ...
        if sample.bboxes is not None:
            sample.bboxes = self._augment_bboxes(sample.bboxes, sample.shape, to_size)
        ...
        if sample.keypoints is not None:
            sample.keypoints = self._augment_keypoints(sample.keypoints, sample.shape, to_size)
...
class CenterCrop(AbstractTransform):
    ...
    def __init__(self, height: int = 224, width: int = 224) -> None:
        ...
        Args:
            height: Height, 裁剪高度, [0, 5000], 224
            width: Width, 裁剪宽度, [0, 5000], 224
        """
        super().__init__()
        self.to_h = eval_arg(height, None)
        self.to_w = eval_arg(width, None)
    ...
    def __call__(self, sample):
        ...
        if sample.bboxes is not None:
            sample.bboxes = self._augment_bboxes(sample.bboxes, sample.shape)
        ...
        if sample.keypoints is not None:
            sample.keypoints = self._augment_keypoints(sample.keypoints, sample.shape)
...
class Crop(AbstractTransform):
    ...
    def __init__(
        ...
        Args:
            top: Vertical distance to the top, 向上的垂直距离, [0, 1000], 0
            right: Horizontal distance to the right, 向右的水平距离, [0, 1000], 0
            bottom: Vertical distance to the bottom, 向下的垂直距离, [0, 1000], 0
            left: Horizontal distance to the left, 向左的水平距离, [0, 1000], 0
        """
        super().__init__()
...
class Pad(AbstractTransform):
    ...
    def _get_params(self, img_size):
        if self.px is not None:
            return self.px
        elif self.px is None and self.pct is not None:
            _, h, w = img_size
...
class RandomResize(Resize):
    ...
    def __init__(
        ...
        Args:
            rnd_resize_x: Random resize x-axis ratio, x轴随机裁剪比例, [0.0, 10.0], 0.0
            rnd_resize_y: Random resize y-axis ratio, y轴随机裁剪比例, [0.0, 10.0], 0.0
            interpolation: Interpolation, 插值模式, {"nearest", "cubic", "linear"}, "linear"
        """
...
class RandomCrop(AbstractTransform):
    ...
    def __init__(
        ...
        Args:
            height: Resize height, 裁剪高度, [0, 5000], 224
            width: Resize width, 裁剪宽度, [0, 5000], 224
            cval: Color value, 填充颜色, [0, 255], 127
...
class RandomResizedCrop(AbstractTransform):
    def __init__(
        ...
        Args:
            height: Resize height, 裁剪高度, [0, 5000], 224
            width: Resize width, 裁剪宽度, [0, 5000], 224
            scale_min: Crop area scale minimum, 裁剪区域比例最小值, [0.08, 1.0], 0.08
            scale_max: Crop area scale maximum, 裁剪区域比例最大值, [0.08, 1.0], 1.0
            ratio_min: Crop ratio minimum, 裁剪纵横比最小值, [0.75, 1.33], 0.75
            ratio_max: Crop ratio maximum, 裁剪纵横比最大值, [0.75, 1.33], 0.75
            interpolation: Interpolation, 插值模式, {"nearest", "bicubic", "linear"}, "linear"
        """
        super().__init__()
