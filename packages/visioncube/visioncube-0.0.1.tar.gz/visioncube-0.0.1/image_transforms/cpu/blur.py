...
class GaussianBlur(ImgaugAdapter):
    ...
    def __init__(self, sigma: float = 3.0) -> None:
        ...
        Args:
            sigma: Sigma, 标准差, (0.0, 3.0), 3.0
        """
        sigma = eval_arg(sigma, None)
        super().__init__(iaa.GaussianBlur(sigma=sigma))
...
class AverageBlur(ImgaugAdapter):
    ...
    def __init__(self, kernel_size: int = 5) -> None:
        ...
        Args:
            kernel_size: Kernel size, 核数, [1, 7], 5
        """
        kernel_size = eval_arg(kernel_size, int)
        if kernel_size % 2 == 0:
            kernel_size += 1
        super().__init__(iaa.AverageBlur(k=kernel_size))
...
class MedianBlur(ImgaugAdapter):
    ...
    def __init__(self, kernel_size: int = 5):
        ...
        if kernel_size % 2 == 0:
            kernel_size += 1
        super().__init__(iaa.MedianBlur(k=kernel_size))
...
class BilateralBlur(ImgaugAdapter):
    ...
    def __init__(self, d: int = 5, sigma: float = 11.0):
        ...
        Args:
            d: Diameter, 直径, [1, 101], 5
            sigma: Sigma, 标准差, (0.0, 100.0], 11.0
        """
        d = eval_arg(d, int)
        sigma = eval_arg(sigma, None)
        if d % 2 == 0:
            d += 1
        super().__init__(iaa.BilateralBlur(d=d, sigma_color=sigma, sigma_space=sigma))
...
class MotionBlur(ImgaugAdapter):
    def __init__(
        ...
        Args:
            kernel_size: Kernel size, 核数, [1, 9], 3
            degree: Rotate angle, 旋转角度, [-180.0, 180.0], 45.0
            direction: Direction, 方向, [-1.0, 1.0], 1.0
        """
        kernel_size = eval_arg(kernel_size, int)
        degree = eval_arg(degree, None)
        direction = eval_arg(direction, None)

        super().__init__(iaa.MotionBlur(k=kernel_size, angle=degree, direction=direction))
...
class MeanShiftBlur(ImgaugAdapter):
    def __init__(self, spatial_radius: float = 5.0, color_radius: float = 5.0):
        ...
        Args:
            spatial_radius: Spatial radius, 空间半径, [5.0, 40.0], 5.0
            color_radius: Color radius, 颜色半径, [5.0, 40.0], 5.0
        """
...
class RandomGaussianBlur(ImgaugAdapter):
    ...
    def __init__(
        ...
        Args:
            sigma_min: Sigma minimum, 标准差最小值, [0.0, 3.0], 0.1
            sigma_max: Sigma maximum, 标准差最大值, [0.0, 3.0], 3.0
        """
        sigma_min = eval_arg(sigma_min, float)
        sigma_max = eval_arg(sigma_max, float)

        super().__init__(iaa.GaussianBlur(sigma=(sigma_min, sigma_max)))
...
class RandomAverageBlur(ImgaugAdapter):
    ...
    def __init__(self, kernel_size_min: int = 1, kernel_size_max: int = 7) -> None:
        ...
        Args:
            kernel_size_min: Kernel size minimum, 核数最小值, [1, 7], 1
            kernel_size_max: Kernel size maximum, 核数最大值,, [1, 7], 7
        """
        kernel_size_min = eval_arg(kernel_size_min, int)
        kernel_size_max = eval_arg(kernel_size_max, int)

        super().__init__(iaa.AverageBlur(k=(kernel_size_min, kernel_size_max)))
...
class RandomMedianBlur(ImgaugAdapter):
    ...
    def __init__(self, kernel_size_min: int = 1, kernel_size_max: int = 7) -> None:
        ...
        super().__init__(iaa.MedianBlur(k=(k_min, k_max)))
...
class RandomBilateralBlur(ImgaugAdapter):
    ...
    def __init__(
        ...
        Args:
            d_min: Diameter min value, 直径最小值, [1, 9], 1
            d_max: Diameter max value, 直径最大值, [1, 9], 9
            sigma_min: Sigma min value, 标准差最小值, [10.0, 250.0], 10.0
            sigma_max: Sigma max value, 标准差最大值, [10.0, 250.0], 250.0
        """
        ...
        super().__init__(iaa.BilateralBlur(d=(d_min, d_max),
                                           ...
                                           sigma_space=(sigma_min, sigma_max)))
...
class RandomMotionBlur(ImgaugAdapter):
    def __init__(
        ...
        Args:
            kernel_size_min: Kernel size minimum, 核数最小值, [3, 7], 3
            kernel_size_max: Kernel size maximum, 核数最小值, [3, 7], 7
            degree_min: Angle minimum, 角度最小值, [0.0, 360.0], 0.0
            degree_max: Angle maximum, 角度最大值, [0.0, 360.0], 360.0
            direction_min: Direction minimum, 方向最小值, [-1.0, 1.0], -1.0
            direction_max: Direction maximum, 方向最大值, [-1.0, 1.0], 1.0
        """
        kernel_size_min = eval_arg(kernel_size_min, int)
        kernel_size_max = eval_arg(kernel_size_max, int)
...
class RandomMeanShiftBlur(ImgaugAdapter):
    def __init__(
        ...
        Args:
            spatial_radius_min: Spatial radius minimum, 空间半径最小值, [5.0, 40.0], 5.0
            spatial_radius_max: Spatial radius maximum, 空间半径最大值, [5.0, 40.0], 40.0
            color_radius_min: Color radius minimum, 颜色半径最小值, [5.0, 40.0], 5.0
            color_radius_max: Color radius maximum, 颜色半径最大值, [5.0, 40.0], 40.0
        """
        ...
        super().__init__(iaa.MeanShiftBlur(spatial_radius=(spatial_radius_min, spatial_radius_max),
