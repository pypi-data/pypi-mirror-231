...
from .common import AbstractTransform, SampleCuda, SampleCPU
from . import gpu
from . import cpu
...
class TransformPipeline(object):
    ...
    def add_transform(
        ...
        if self.device == 'gpu':
            Processing = getattr(gpu, name)
        else:
            Processing = getattr(cpu, name)
    ...
    def __call__(self, doc):
        ...
        sample = SampleCuda(doc) if self.device == 'gpu' else SampleCPU(doc)
