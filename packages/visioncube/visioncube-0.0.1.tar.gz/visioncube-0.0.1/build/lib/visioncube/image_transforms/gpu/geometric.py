...
from ..common import AbstractTransform, INTERPOLATION, eval_arg
...
class Affine(AbstractTransform):
    ...
    def __init__(
        ...
        Args:
            degree: Rotate angle, 旋转角度, [-180.0, 180.0], 0.0
            translate_x: Horizontal translations, 水平平移像素, [0, 1000], 0
            translate_y: Vertical translations, 垂直平移像素, [0, 1000], 0
            scale: Overall scale, 缩放比例, [0.1, 1000.0], 1.0
            shear_x: Horizontal shear value, 水平错切, [-180.0, 180.0], 0.0
            shear_y: Vertical shear value, 垂直错切, [-180.0, 180.0], 0.0
            cval: Color value, 填充颜色, [0, 255], 0
            interpolation: Interpolation, 插值模式, {"nearest", "linear"}, "nearest"
        """
        super().__init__()
        ...
        self.interp = INTERPOLATION.get(interpolation)
...
class Rotate(Affine):
    ...
    def __init__(
        ...
        Args:
            degree: Rotate angle, 旋转角度, [-360.0, 360.0], 20.0
            cval: Color Value, 填充颜色, [0, 255], 127
            interpolation: Interpolation, 插值模式, {'nearest', 'linear'}, 'nearest'
        """
        super().__init__(
            degree=degree,
...
class RandomPerspective(AbstractTransform):
    ...
    def __init__(
        ...
        Args:
            scale_min: Minimum distortion degree, 最小失真程度, [0.0, 1.0], 0.0
            scale_max: Maximum distortion degree, 最大失真程度, [0.0, 1.0], 1.0
    ...
    def _augment_images(self, image, start_points, end_points, cval):
        ...
        return TF.perspective(image, start_points, end_points,
                              fill=cval, interpolation=self.interpolation)
    ...
    def __call__(self, sample):
        ...
        if sample.heatmap is not None:
            sample.heatmap = self._augment_images(sample.heatmap,
                                                  start_points,
                                                  end_points,
                                                  self.cval)

        if sample.keypoints is not None:
            sample.keypoints = self._augment_keypoints(sample.keypoints,
                                                       start_points,
                                                       end_points,
                                                       sample.shape)
...
class RandomRotate(Affine):
    ...
    def __init__(
        ...
        Args:
            degree_min: Degree minimum, 角度最小值, [-360.0, 360.0], -10.0
            degree_max: Degree maximum, 角度最大值, [-360.0, 360.0], 10.0
            cval: Color Value, 填充颜色, [0, 255], 127
            interpolation: Interpolation, 插值模式, {'nearest', 'linear'}, 'nearest'
        """
...
class RandomAffine(Affine):
    ...
    def __init__(
        ...
        Args:
            p_scale: Scale probability, 缩放变换概率, [0.0, 1.0], 0.5
            rnd_scale_min: Scale minimum, 缩放比例最小值, [0.1, 1000.0], None
            rnd_scale_max: Scale maximum, 缩放比例最最大值, [0.1, 1000.0], None
            p_shear: Shear probability, 缩放变换概率, [0.0, 1.0], 0.5
            rnd_shear_x: Horizontal shear value, 水平错切, [-180.0, 180.0], None
            rnd_shear_y: Vertical shear value, 垂直错切, [-180.0, 180.0], None
            p_rotate: Rotate probability, 缩放变换概率, [0.0, 1.0], 0.5
            rnd_rotate: Rotate angle, 旋转角度, [-180.0, 180.0], 0.0
            p_translate: Translate probability, 缩放变换概率, [0.0, 1.0], 0.5
            rnd_translate_x_pct: Horizontal translations percent, 水平平移百分比, [0.0, 1.0], None
            rnd_translate_y_pct: Vertical translations percent, 垂直平移百分比, [0.0, 1.0], None
            cval: Color value, 填充颜色, [0, 255], 127
            interpolation: Interpolation, 插值模式, {"nearest", "linear"}, "nearest"
        """
        ...
        if (rnd_scale_min or rnd_scale_max) and torch.rand(1) < p_scale:
            scale = [rnd_scale_min, rnd_scale_max]
        else:
            scale = 1.0

        if (rnd_shear_x or rnd_shear_y) and torch.rand(1) < p_shear:
            shear_x, shear_y = (0 - rnd_shear_x, rnd_shear_x), (0 - rnd_shear_y, rnd_shear_y)
        else:
            shear_x, shear_y = 0, 0

        if (rnd_translate_x_pct or rnd_translate_y_pct) and torch.rand(1) < p_translate:
            assert 0 < rnd_translate_x_pct <= 1
            assert 0 < rnd_translate_y_pct <= 1
            translate_x, translate_y = (0.1, rnd_translate_x_pct), (0.1, rnd_translate_y_pct)
        else:
