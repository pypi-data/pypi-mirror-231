...
class AdjustColorLevels(AbstractTransform):
    ...
    def __init__(self,
                 ...
                 gamma: float = 1.0,
                 out_black: int = 0,
                 out_white: int = 255) -> None:
        """AdjustColorLevels, 调整颜色级别, 颜色变换

        Args:
            in_black: Input black, 输入截止黑色, [0, 255], 0
            in_white: Input white, 输入截止白色, [0, 255], 255
            gamma: Gamma, Gamma值, [0.1, 3.0], 1.0
            out_black: Output black, 输出截止黑色, [0, 255], 0
            out_white: Output white, 输出截止白色, [0, 255], 255
        """
...
class Brightness(ImgaugAdapter):
    ...
    def __init__(self, factor: float = 1.0) -> None:
        ...
        Args:
            factor: Brightness factor, 亮度系数, [0.7, 1.3], 1.0
        """
...
class Saturation(ImgaugAdapter):
    ...
    def __init__(self, factor=1.0):
        ...
        Args:
            factor: Saturation factor, 饱和度系数, [0.0, 2.0], 1.0
        """
        factor = eval_arg(factor, None)
        super().__init__(iaa.MultiplySaturation(factor))
...
class Contrast(ImgaugAdapter):
    ...
    def __init__(self, factor: float = 1.0) -> None:
        ...
        Args:
            factor: Contrast factor, 对比度系数, [0.0, 2.0], 1.0
        """
        factor = eval_arg(factor, None)
        super().__init__(iaa.LinearContrast(alpha=factor))
...
class Hue(ImgaugAdapter):
    ...
    def __init__(self, factor: float = 0.0) -> None:
        ...
        Args:
            factor: Hue factor, 色调系数, [-0.5, 0.5], 0.0
        """
        factor = eval_arg(factor, None)
        factor = int(factor * 255)

        super().__init__(iaa.AddToHue(factor))
...
class Sharpness(AbstractTransform):
    ...
    def __init__(self, factor: float = 1.0) -> None:
        ...
        Args:
            factor: Sharpness factor, 锐度系数, [0.0, 2.0], 1.0
        """
...
class Gamma(AbstractTransform):
    ...
    def __init__(
        ...
        Args:
            gamma: Gamma, 伽玛校正系数, [0.1, 3.0], 1.0
            gain: Gain, 乘数, [0.1, 10], 1.0
        """
...
class Equalize(ImgaugAdapter):
    ...
    def __init__(self) -> None:
        ...
        """
        super().__init__(iaa.HistogramEqualization())
...
class Posterize(ImgaugAdapter):
    ...
    def __init__(self, bits: int = 2) -> None:
        ...
        Args:
            bits: Channel color bits, 通道颜色位数, [1, 8], 2
        """
        bits = eval_arg(bits, int)
        super().__init__(iaa.Posterize(nb_bits=bits))
...
class KMeansColor(ImgaugAdapter):
    ...
    def __init__(self, n_colors: int = 5, interpolation: str = 'nearest') -> None:
        ...
        Args:
            n_colors: Number of clusters in k-Means, 簇数, [2, 16], 5
...
class CLAHE(AbstractTransform):
    ...
    def __init__(self, clip_limit: float = 1.0, gw: int = 2, gh: int = 2) -> None:
        ...
        Args:
            clip_limit: Clip limit, 截止对比度, [0.1, 8.0], 1.0
            gh: Grid width, 网格宽度, [1, 16], 2
            gw: Grid height, 网格高度, [1, 16], 2
        """
        super().__init__()
...
class RandomSaturation(ImgaugAdapter):
    ...
    def __init__(self, factor_min: float = 0.0, factor_max: float = 2.0) -> None:
        ...
        Args:
            factor_min: Saturation minimum, 饱和度系数最小值, [0.0, 3.0], 0.0
            factor_max: Saturation maximum, 饱和度系数最大值, [0.0, 3.0], 2.0
        """
        factor_min = eval_arg(factor_min, None)
        factor_max = eval_arg(factor_max, None)
        super().__init__(iaa.MultiplySaturation((factor_min, factor_max)))
...
class RandomColor(ImgaugAdapter):
    ...
    def __init__(
        ...
        Args:
            rnd_brightness: Brightness factor, 亮度系数, [0.0, 2.0], 0.2
            rnd_contrast: Contrast factor, 对比度系数, [0.0, 2.0], 0.2
            rnd_saturation: Saturation factor, 饱和度系数, [0.0, 2.0], 0.2
            rnd_hue: Hue factor, 色调系数, [-0.5, 0.5], 0.05
        """
...
class RandomBrightness(ImgaugAdapter):
    ...
    def __init__(self, factor_min: float = 0.0, factor_max: float = 2.0) -> None:
        ...
        Args:
            factor_min: Brightness minimum, 亮度系数最小值, [0.0, 2.0], 0.0
            factor_max: Brightness maximum, 亮度系数最大值, [0.0, 2.0], 2.0
        """
        factor_min = eval_arg(factor_min, None)
        factor_max = eval_arg(factor_max, None)
...
class RandomContrast(ImgaugAdapter):
    ...
    def __init__(self, factor_min: float = 0.0, factor_max: float = 2.0) -> None:
        ...
        Args:
            factor_min: Contrast minimum, 对比度系数最小值, [0.0, 2.0], 0.0
            factor_max: Contrast maximum, 对比度系数最大值, [0.0, 2.0], 2.0
        """
        factor_min = eval_arg(factor_min, None)
        factor_max = eval_arg(factor_max, None)
        super().__init__(iaa.LinearContrast(alpha=(factor_min, factor_max)))
...
class RandomHue(ImgaugAdapter):
    ...
    def __init__(self, factor_min: float = -0.5, factor_max: float = 0.5) -> None:
        ...
        Args:
            factor_min: Hue minimum, 色调系数最小值, [-0.5, 0.5], -0.5
            factor_max: Hue maximum, 色调系数最大值, [-0.5, 0.5], 0.5
        """
        factor_min = eval_arg(factor_min, None)
        factor_max = eval_arg(factor_max, None)
        factor = (int(factor_min * 255), int(factor_max * 255))

        super().__init__(iaa.AddToHue(value=factor))
...
class RandomSharpness(Sharpness):
    ...
    def __init__(self, factor: float = 1.0, p: float = 0.5) -> None:
        ...
        Args:
            factor: Sharpness factor, 锐度系数, [0, 2], 1.0
...
class RandomGamma(Gamma):
    ...
    def __init__(
        ...
        Args:
            gamma_min: Gamma minimum, 伽玛校正系数最小值, [0.1, 3.0], 1.0
            gamma_max: Gamma maximum, 伽玛校正系数最大值, [0.1, 3.0], 3.0
            gain_min: Gain minimum, 乘数最小值, [0.1, 10], 0.1
            gain_max: Gain maximum, 乘数最大值, [0.1, 10], 10.0
        """
...
class RandomEqualize(ImgaugAdapter):
    ...
    def __init__(self, p: float = 0.5) -> None:
        ...
        """
        p = eval_arg(p, None)
        super().__init__(iaa.HistogramEqualization() if np.random.rand(1) < p else iaa.Identity())
...
class RandomPosterize(ImgaugAdapter):
    ...
    def __init__(self, bits_min: int = 1, bits_max: int = 8, p: float = 0.5) -> None:
        ...
        Args:
            bits_min: Channel color bits minimum, 通道颜色位数最小值, [1, 8], 1
            bits_max: Channel color bits maximum, 通道颜色位数最大值, [1, 8], 8
            p: Probability, 变换概率, [0, 1.0], 0.5
        """
...
class RandomKMeansColor(ImgaugAdapter):
    ...
    def __init__(
        ...
        Args:
            n_colors_min: Minimum of clusters in k-Means, 簇数最小值, [2, 16], 2
            n_colors_max: Maximum of clusters in k-Means, 簇数最小值, [2, 16], 16
            interpolation: Interpolation, 插值模式, {"nearest", "linear", "cubic", "area"}, "nearest"
        """
        ...
        super().__init__(
            iaa.KMeansColorQuantization(n_colors=(k_min, k_max),
                                        from_colorspace=iaa.CSPACE_RGB,
                                        to_colorspace=iaa.CSPACE_RGB,
                                        interpolation=interpolation))
