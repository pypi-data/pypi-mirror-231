...
class Normalize(AbstractTransform):
    ...
    def __init__(self) -> None:
        """Normalize, 标准化, 算术变换
        """
        mean = torch.tensor([0.485, 0.456, 0.406], dtype=torch.float32) * 255
        std = torch.tensor([0.229, 0.224, 0.225], dtype=torch.float32) * 255
        self.operator = transforms.Normalize(mean=mean, std=std)
...
class AdditiveGaussianNoise(AbstractTransform):
    ...
    def __init__(self, mean: float = 0.0, std: float = 3.0) -> None:
        ...
        Args:
            mean: Mean, 平均值, [0.0, 1.0], 0.0
            std: Standard deviation, 标准差, [0.0, 15.0], 3.0
        """
