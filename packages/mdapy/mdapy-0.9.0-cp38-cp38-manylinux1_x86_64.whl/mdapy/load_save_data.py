...
class SaveFile:
        ...
        with pa.OSFile(output_name, "wb") as op:
            ...
            if data_format == "atomic":
                ...
                if "q" not in data.columns:
                    table = pa.Table.from_pandas(data[["id", "type", "x", "y", "z"]])
                    table = table.add_column(2, "q", pa.array(np.zeros(data.shape[0])))
                else:
    ...
    @staticmethod
    def write_dump(
        output_name, box, boundary, data=None, pos=None, type_list=None, timestep=0
    ):
        assert isinstance(output_name, str)
        assert isinstance(box, np.ndarray)
        assert len(boundary) == 3
...
class BuildSystem:
    ...
    def read_dump(filename):
        ...
        if if_space:
            read_options = csv.ReadOptions(
                column_names=col_names + ["drop"], skip_rows=9
            )
            parse_options = csv.ParseOptions(delimiter=" ")
            data = (
                csv.read_csv(
            ...
            read_options = csv.ReadOptions(column_names=col_names, skip_rows=9)
            parse_options = csv.ParseOptions(delimiter=" ")
            data = csv.read_csv(
                filename, read_options=read_options, parse_options=parse_options
            ).to_pandas()
