import openpyxl, gzip, datetime, psutil, time, os
...
def demultiplex(
    primerset, tagging_scheme_header, tagging_scheme_line, output_folder, tag_removal
):
    ## create a dictionary for the in_handles which are the two files to demultiplex
    in_handles = {
        "fwd": FastqGeneralIterator(gzip.open(Path(tagging_scheme_line[0]), "rt")),
    ...
    combinations = [
        (primerset[primername[0]], primerset[primername[1]])
        for primername in combinations
    ]

    ## connect combinations and sites if site was used
    tag_to_site = {
        tag: site for tag, site in zip(combinations, tagging_scheme_line[4:]) if site
    }
    ...
    nomatch_fwd = Path(output_folder).joinpath(
        "no_match_{}_r1.fastq.gz".format(
            Path(tagging_scheme_line[2]).with_suffix("").with_suffix("")
        )
    )
    nomatch_rev = Path(output_folder).joinpath(
        "no_match_{}_r2.fastq.gz".format(
            Path(tagging_scheme_line[3]).with_suffix("").with_suffix("")
        )
    )
    out_handles["nomatch"] = (
        gzip.open(nomatch_fwd, "wt", compresslevel=6),
        gzip.open(nomatch_rev, "wt", compresslevel=6),
    )
    ...
    for (title_f, seq_f, qual_f), (title_r, seq_r, qual_r) in zip(
        in_handles["fwd"], in_handles["rev"]
    ):
        ...
        for combination in tag_to_site.keys():
            if seq_f.startswith(combination[0]) and seq_r.startswith(combination[1]):
                ...
                out_handles[tag_to_site[combination]][0].write(
                    "@{}\n{}\n+\n{}\n".format(
                        title_f, seq_f[fwd_tag_len:], qual_f[fwd_tag_len:]
                    )
                )
                out_handles[tag_to_site[combination]][1].write(
                    "@{}\n{}\n+\n{}\n".format(
                        title_r, seq_r[rev_tag_len:], qual_r[rev_tag_len:]
                    )
                )
                count += 1
                break
    ...
    print(
        "{}: {} - {}: {} reads demultiplexed.".format(
            ...
            tagging_scheme_line[2],
            tagging_scheme_line[3],
            count,
        )
    )
...
def main(primerset, tagging_scheme, output_folder, tag_removal, print_handle, window):
    ...
    primerset = {
        line.split(",")[0]: line.split(",")[1] for line in open(primerset, "r")
    }
    ...
    print_handle.print(
        "{}: Starting to demultiplex {} file pairs. Output will be routed to the terminal. The window will freeze during this process".format(
            datetime.datetime.now().strftime("%H:%M:%S"), len(rows) - 1
        )
    )
    window.Refresh()
    Parallel(n_jobs=cores_to_use)(
        delayed(demultiplex)(primerset, rows[0], rows[i], output_folder, tag_removal)
...
def main_cl(
    ...
    ## run the demultiplex function on every line in the tagging scheme in parallel
    print(
        "{}: Starting to demultiplex {} file pairs.".format(
            datetime.datetime.now().strftime("%H:%M:%S"), len(rows) - 1
        )
    )
    Parallel(n_jobs=cores_to_use)(
