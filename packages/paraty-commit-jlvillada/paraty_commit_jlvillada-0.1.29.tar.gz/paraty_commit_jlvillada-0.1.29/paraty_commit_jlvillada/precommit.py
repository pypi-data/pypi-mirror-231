...
def main():
    ...
    print(Fore.GREEN + "Installing Paraty Pre-Commit...")
    ...
    print(Fore.GREEN + f"Project Path: {project_path}")
    ...
    print(Fore.GREEN + "Paraty Pre-Commit Installed!!")


def execute_command(comando):
    print(Fore.GREEN + ">>", comando)
    proceso = subprocess.Popen(comando, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    salida_stdout, salida_stderr = proceso.communicate()
    codigo_salida = proceso.returncode
    if salida_stderr:
        print(Fore.RED + salida_stderr.decode('utf-8'))
        print(Fore.RED +"<<<<<<<<<<<<<<<<<<<<<<<")
    if salida_stdout:
        print(Fore.GREEN + salida_stdout.decode('utf-8'))
        print(Fore.GREEN +"<<<<<<<<<<<<<<<<<<<<<<<")
