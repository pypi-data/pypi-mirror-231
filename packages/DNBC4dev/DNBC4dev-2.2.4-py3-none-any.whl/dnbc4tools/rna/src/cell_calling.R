...
cell_numbers <- as.numeric(gsub("CELL(\\d+)_.*", "\\1", beads_barcodes))
sorted_indices <- order(cell_numbers)
beads_barcodes <- beads_barcodes[sorted_indices]
write.table(beads_barcodes,file = paste(args$outdir,"/beads_barcodes.txt",sep=""), quote = FALSE, row.names=FALSE, col.names=FALSE)
