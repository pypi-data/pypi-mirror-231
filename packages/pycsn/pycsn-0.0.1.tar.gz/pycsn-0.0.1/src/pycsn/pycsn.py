        ...
        match format:
            case 'cds':
                ...
                with open(filename, 'w') as fp:
                    fp.write(cds)
                print(f"{C.green}CDS-file written: {C.red}{filename}{C.n}")
            case 'csn':
                with open(filename, 'w') as js:
                    json.dump(self.csn, js, indent=4)
                print(f"{C.green}CSN-file written: {C.red}{filename}{C.n}")
            case _:
...
def main():
    ...
    for i, f in enumerate(args.filenames):
        print(f"{C.green}Read data-file:{C.red} {f}{C.n}")
        fp = Path(f)
        df = pd.read_csv(fp)
        dfs[str(fp.stem)] = df
        print(f"{C.green}Table name:{C.red} {fp.stem}{C.n}")
        print(df.head(3))
    csn=PyCSN(dfs)
    print(csn)
