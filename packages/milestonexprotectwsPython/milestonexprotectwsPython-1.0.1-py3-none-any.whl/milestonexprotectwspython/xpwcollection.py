...
class XPWCollection(Iterable):
    ...
    def __init__(self, itemType:type=None) -> None:
        ...
        # validations.
        if (itemType == None):
            raise Exception(XPWAppMessages.ARGUMENT_REQUIRED_ERROR.format("itemType"))
        if (not isinstance(itemType, type)):
            raise Exception(XPWAppMessages.ARGUMENT_TYPE_ERROR.format("itemType", "type", type(itemType).__name__))
    ...
    def _ValidateItemType(self, item:object, methodName:str, argumentName:str) -> None:
        ...
        with self._fLock:
            ...
            raise TypeError(XPWAppMessages.COLLECTION_ARGUMENT_TYPE_ERROR.format(methodName, argumentName, self._fItemType.__name__, type(item).__name__))
