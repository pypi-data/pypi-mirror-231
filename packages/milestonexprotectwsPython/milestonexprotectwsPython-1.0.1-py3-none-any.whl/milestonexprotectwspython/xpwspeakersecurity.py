...
@export
class XPWSpeakerSecurity:
    """
    Speaker device Security information.

    Supplemental documentation can be found at:
    https://doc.milestonesys.com/2023R2/en-US/standard_features/sf_mc/sf_ui/mc_roles_security.htm?cshid=9902#Speakerrelatedpermissions
    ...
    def __eq__(self, other):
        try:
            return self.Browse == other.Browse
        except Exception as ex:
            if (isinstance(self, XPWSpeakerSecurity)) and (isinstance(other, XPWSpeakerSecurity)):
                return self.Browse == other.Browse
            return False
    ...
    def __lt__(self, other):
        try:
            ...
            # epColl.sort(Browse=lambda x: x.Browse or "", reverse=False)     <- GOOD syntax
            # epColl.sort(Browse=lambda x: x.Browse, reverse=False)           <- BAD syntax, as the "x.Browse" property may be None, and will cause this to fail!
            return self.Browse < other.Browse
        except Exception as ex:
            if (isinstance(self, XPWSpeakerSecurity)) and (isinstance(other, XPWSpeakerSecurity)):
                return self.Browse < other.Browse
