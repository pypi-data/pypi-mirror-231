...
def barcodeTranslatefile(combined_file: str, 
                         select_barcode: str, 
                         barcodeTranslate: str,
                         barcodeTranslate_hex: str,
                         cellid: str
                         ) -> None:
    """
    ...
    None
    """
    multibeads_filter_dict = collections.defaultdict(list)
    one_barcodedict = {}
    
    with open(combined_file, 'r') as multi_beads:
        for line in multi_beads:
            line = line.strip()
            if line and int(line.split('\t')[-1].split('_N')[-1]) <= 9:
                barcode, cell = line.split('\t')[0], line.split('\t')[-1]
                multibeads_filter_dict[cell].append(barcode)

    # Use set for faster barcode lookup
    multibead_barcodes = set(barcode for barcodes in multibeads_filter_dict.values() for barcode in barcodes)
                
    with open(select_barcode, 'r') as selectbarcode:
        num = int(list(multibeads_filter_dict.keys())[-1].split('CELL')[-1].split('_')[0]) if multibeads_filter_dict else 0
        next(selectbarcode)  # Skip the header
        for line in selectbarcode:
            barcode = line.strip().split('\t')[0]
            if barcode not in multibead_barcodes:
                num += 1
                cellName = f'CELL{num}_N1'
                one_barcodedict[barcode] = cellName
    ...
    with open(barcodeTranslate, 'w') as barcode_cell, \
        ...
        for cell, barcodes in multibeads_filter_dict.items():
            for barcode in barcodes:
                barcode_cell.write(f'{barcode}\t{cell}\n')
                celllist.append(cell)
                barcode_hex = seq_comp(barcode)
                barcode_cell_hex.write(f'{barcode_hex}\t{cell}\n')

        for barcode, cell in one_barcodedict.items():
            barcode_cell.write(f'{barcode}\t{cell}\n')
            celllist.append(cell)
            barcode_hex = seq_comp(barcode)
            barcode_cell_hex.write(f'{barcode_hex}\t{cell}\n')
...
def summary_count(barcodeTranslate, beads_stat, 
                  beads_barcodes, cellCount, outdir):
    """
    ...
    selectbarcode = pd.read_table(beads_barcodes,sep='\t', header=None)
    barcodeTranslate.columns = ['BARCODE', 'CELL']
    filtered_barcodeTranslate = barcodeTranslate[barcodeTranslate['CELL'].isin(selectbarcode[0])]
    beads_stat = pd.merge(raw_beads_stat, filtered_barcodeTranslate, how='inner', on='BARCODE')
