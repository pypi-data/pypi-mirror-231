...
tss_set = read.table(args$tss,header = F,sep="\t",fill = TRUE,comment.char = "$")
...
qc1[5,1]="Mean fraction of fragments overlapping peaks"
qc1[5,2]=paste0(100*round(sum(my_meta$FRIP*my_meta$uniqueNuclearFrags)/sum(my_meta$uniqueNuclearFrags),4),"%")
qc1[6,1]="Mean fraction of fragments overlapping TSSs"
qc1[6,2]=paste0(100*round(sum(my_meta$tssProportion*my_meta$uniqueNuclearFrags)/sum(my_meta$uniqueNuclearFrags),4),"%")
qc1[7,1]="Called peak number"
qc1[7,2]=prettyNum(nrow(counts),big.mark = ",")
qc1[8,1]="Percent duplicates"
qc1[8,2]=paste0(100*round(1-(sum(my_meta$uniqueNuclearFrags)/sum(my_meta$totalFrags)),4),"%")
